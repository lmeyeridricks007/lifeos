...
import { DentistsNetherlandsView } from "@/src/components/health/DentistsNetherlandsView";
import { dentistsNetherlandsPage as page } from "@/src/components/health/dentistsNetherlandsPageModel";
...
function MedicalWebPageJsonLd() {
  const jsonLd = {
    ...
    about: {
      ...
      name: "Dentistry",
      description:
        "Orientation for expats on dental care (tandarts) in the Netherlands — finding a dentist, insurance, check-ups, treatments, children's dentistry, orthodontics and emergencies.",
    },
    audience: {
      "@type": "PeopleAudience",
...
export default function DentistsNetherlandsPage() {
  return (
    <>
      ...
      <HowToJsonLd
        ...
        steps={page.finding.howToSteps.map((step) => ({ name: step.name, text: step.text }))}
        urlPath={`${path}${page.howToSchema.anchor}`}
      />
      <DentistsNetherlandsView />
    </>
  );
}
