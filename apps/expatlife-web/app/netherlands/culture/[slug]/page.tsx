...
export function generateStaticParams() {
  return getClusterSlugsForStaticParams("culture").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const path = normalizeSitePath(`/netherlands/culture/${slug}`);
  const entry = getClusterPageByPath(path);
  if (!entry || isContentHidden(entry.contentStatus)) return { title: "Culture in the Netherlands" };
...
export default async function NetherlandsCultureTopicPage({ params }: Props) {
  const { slug } = await params;
  const path = normalizeSitePath(`/netherlands/culture/${slug}`);
  const entry = getClusterPageByPath(path);
  if (!entry || entry.cluster !== "culture" || entry.slug === "" || isContentHidden(entry.contentStatus))
    notFound();
