...
import { PhoneProvidersNetherlandsView } from "@/src/components/services/phone-providers/PhoneProvidersNetherlandsView";
import { phoneProvidersNetherlandsPage as page } from "@/src/components/services/phone-providers/phoneProvidersNetherlandsPageModel";
...
function ProviderDirectoryJsonLd() {
  const jsonLd = {
    ...
    "@type": "ItemList",
    name: "Phone providers in the Netherlands for expats — prepaid, SIM-only and eSIM orientation",
    description:
      "Structured directory of Dutch mobile / SIM / eSIM provider types and comparison approaches for expats. Informational soft discovery — not a ranking.",
    numberOfItems: page.providers.length,
    itemListElement: page.providers.map((provider, index) => ({
      "@type": "ListItem",
      position: index + 1,
      item: {
        "@type": ["MobilePhoneStore", "LocalBusiness", "ProfessionalService"],
        name: provider.name,
        url: provider.website,
        areaServed: provider.region,
        serviceType: "Dutch mobile / SIM / eSIM / prepaid plan orientation",
...
export default function PhoneProvidersNetherlandsPage() {
  return (
    <>
