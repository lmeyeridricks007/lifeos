...
import { InternetProvidersNetherlandsView } from "@/src/components/services/internet-providers/InternetProvidersNetherlandsView";
import { internetProvidersNetherlandsPage as page } from "@/src/components/services/internet-providers/internetProvidersNetherlandsPageModel";
...
function ProviderDirectoryJsonLd() {
  const jsonLd = {
    ...
    "@type": "ItemList",
    name: "Internet providers in the Netherlands for expats — fibre, cable and DSL orientation",
    description:
      "Structured directory of Dutch fixed broadband provider types and comparison approaches for expats. Informational soft discovery — not a ranking.",
    numberOfItems: page.providers.length,
    itemListElement: page.providers.map((provider, index) => ({
      "@type": "ListItem",
      position: index + 1,
      item: {
        "@type": ["InternetServiceProvider", "LocalBusiness", "ProfessionalService"],
        name: provider.name,
        url: provider.website,
        areaServed: provider.region,
        serviceType: "Dutch fixed broadband / fibre / cable / DSL orientation",
...
export default function InternetProvidersNetherlandsPage() {
  return (
    <>
      ...
      <InternetProvidersNetherlandsView />
    </>
  );
}
