...
import type { BsoProviderRecord } from "./afterSchoolCareNetherlandsPageModel";
...
const CARE_FILTERS = ["After school", "Before school", "Holiday care"] as const;

type Props = {
  providers: readonly BsoProviderRecord[];
};

export function BsoProviderDirectory({ providers }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    if (careFilter === "Before school") {
      list = list.filter((p) => !p.beforeSchool.toLowerCase().startsWith("limited") && p.beforeSchool.toLowerCase().includes("yes"));
    }
    ...
    <div className="space-y-5">
      <div className={cn("relative overflow-hidden rounded-3xl border border-slate-200/90 bg-white/95 p-4 shadow-sm ring-1 ring-slate-900/[0.04] sm:p-5", movingNlCardMicroLiftClass)}>
        ...
        <div className="mt-4 space-y-3">
          <FilterPills label="City" options={BSO_CITIES} active={cityFilter} onChange={(v) => { setCityFilter(v); resetPage(); }} />
      ...
      <div className="w-full overflow-hidden rounded-3xl border border-slate-200/90 bg-white shadow-card">
          ...
          <table className="w-full min-w-[900px] divide-y divide-slate-200 text-left text-sm">
            <thead className="bg-slate-50 text-xs uppercase tracking-[0.12em] text-slate-500">
              <tr>
                ...
                <th scope="col" className="px-4 py-3 font-bold">Before school</th>
                <th scope="col" className="px-4 py-3 font-bold">After school</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {paginated.map((row) => (
                <tr key={row.provider}>
                  ...
                  <td className="px-4 py-4 text-foreground-muted">{row.beforeSchool}</td>
        ...
        <p className="border-t border-slate-100 px-4 py-3 text-xs leading-relaxed text-foreground-muted">
          Orientation only — not rankings. Confirm availability, LRK registration and fees on each provider&apos;s website.
        </p>
      </div>
