...
import {
  estateAgentsNetherlandsPage as page,
  type EstateAgentLink,
  type EstateAgentProvider,
} from "./estateAgentsNetherlandsPageModel";
...
function LinkCard({ item, iconIndex = 0 }: { item: EstateAgentLink; iconIndex?: number }) {
...
function ProviderCard({ provider }: { provider: EstateAgentProvider }) {
  return (
    <article className={cn("relative overflow-hidden rounded-3xl border border-slate-200/90 bg-white p-5 shadow-expatos-xl ring-1 ring-slate-900/[0.04]", movingNlCardMicroLiftClass)}>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className={providerTypeClass}>{provider.agentType}</span>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-[920px] divide-y divide-slate-200 text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-[0.12em] text-slate-500">
            <tr>
              {["Agent", "Cities Served", "Expat Focus", "Languages", "Remote", "Agent Type"].map((heading) => (
...
export function EstateAgentsNetherlandsView() {
  const breadcrumbCrumbs = [
    { name: "Home", item: new URL("/", baseUrl).toString() },
    { name: "Netherlands", item: new URL("/netherlands", baseUrl).toString() },
    { name: "Services", item: new URL("/netherlands/services/", baseUrl).toString() },
    { name: "Estate Agents", item: new URL(page.path, baseUrl).toString() },
  ];
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          <nav aria-label="Breadcrumb" className="mb-6 text-sm text-copilot-text-secondary">
            <ol className="flex flex-wrap items-center gap-x-2 gap-y-1">
              ...
              <li aria-hidden className="text-copilot-text-muted">/</li>
              <li className="font-medium text-copilot-text-primary" aria-current="page">Estate Agents</li>
            </ol>
          </nav>
            ...
            <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1fr)_0.78fr] lg:items-center">
              <div>
                ...
                <p className="mt-5 max-w-3xl rounded-2xl border border-amber-200/70 bg-amber-50/70 px-4 py-3 text-sm leading-relaxed text-amber-950 shadow-sm ring-1 ring-amber-200/40">
                  Provider inclusion is informational soft discovery — not a ranking. Always verify mandate scope, fees, dual-agency rules and fit directly with the makelaar.
                </p>
              </div>
              <HeroImage />
            </div>
          ...
          <nav
            ...
            aria-label="Estate agents guide sections"
          >
          ...
          <div className={sectionStackClass}>
              ...
              <section id="differentiate" className={sectionClass}>
                <SectionIntro eyebrow="Provider types" title="Estate Agents Are Not the Same As…">
                  <p>Start here so you do not instruct the wrong provider. Makelaars, rental agencies, housing platforms and mortgage advisors solve different problems.</p>
                </SectionIntro>
              ...
              <section id="snapshot" className={sectionClass}>
                <SectionIntro eyebrow="Snapshot" title="Estate Agents at a Glance">
              ...
              <section id="agent-role" className={sectionClass}>
                <SectionIntro eyebrow="Makelaar role" title="What Estate Agents Actually Help With">
                  <p>Scope varies by mandate. The common pattern is search or listing support, viewings, bidding or sale negotiation, and coordination with mortgage advisors and notaries — not rental mediation.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
                  {page.agentServices.map((item, idx) => <FeatureCard key={item.title} {...item} iconIndex={idx} />)}
                </div>
                <VisualFigure visual={page.visuals.agentServices} className={sectionVisualMtClass} />
              </section>

              <section id="agent-types" className={sectionClass}>
                <SectionIntro eyebrow="Makelaar models" title="Types of Estate Agents (Makelaars)">
                  <p>Buyer agents, seller agents, NVM offices, franchises and boutiques solve different problems. Match the model to buy vs sell goals before comparing brand names.</p>
                </SectionIntro>
                <div className="mt-6 grid gap-4">
                  {page.agentTypeComparison.map((item) => (
                ...
                <VisualFigure visual={page.visuals.agentTypes} className={sectionVisualMtClass} />
              </section>

              <section id="buy-sell" className={sectionClass}>
                <SectionIntro eyebrow="Buy vs sell pathways" title="Buyer and Seller Situations for Expats">
                  <p>Buyer and seller mandates are different jobs. Use these profiles to decide whether you need an aankoopmakelaar, a verkoopmakelaar, or both — and keep mortgage advice on the Mortgage advisors page.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
                    rows={page.expatBuyerSellerProfiles}
                    columns={[
                ...
                <VisualFigure visual={page.visuals.buySell} className={sectionVisualMtClass} />
              </section>

              <section id="fees" className={sectionClass}>
                <SectionIntro eyebrow="Costs" title="Estate Agent Fees">
                  <p>Fee models vary for buyers and sellers. Confirm inclusions, exclusivity and what happens if a deal falls through — before you sign a mandate.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
              ...
              <section id="documents" className={sectionClass}>
                <SectionIntro eyebrow="Documents" title="Documents Estate Agents Commonly Request">
                  <p>Requirements vary by buy vs sell path. Preparing ID, financing proof and property documents early reduces avoidable delays once viewings start.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
              ...
              <section id="challenges" className={sectionClass}>
                <SectionIntro eyebrow="Common challenges" title="Challenges Expats Often Face">
                  <p>These friction points are common in Dutch buy/sell markets — they are reasons to clarify mandate and bidding strategy early.</p>
                </SectionIntro>
              ...
              <section id="cities" className={sectionClass}>
                <SectionIntro eyebrow="City coverage" title="Estate Agents Across the Netherlands">
                  <p>Local advisors may understand regional housing markets, expat communities and price pressure. At the same time, many estate agents work online nationwide, so compare both city experience and digital process quality.</p>
                </SectionIntro>
              ...
              <section id="directory" className={sectionClass}>
                <SectionIntro eyebrow="Directory" title="Estate Agents for Expats in the Netherlands">
                  <p>This structured directory uses real provider types and neutral descriptions. It does not rank makelaars, invent reviews or guarantee bids or sale prices.</p>
                </SectionIntro>
              ...
              <section id="comparison" className={sectionClass}>
                <SectionIntro eyebrow="Comparison table" title="Compare Estate Agents">
              ...
              <section id="related-housing" className={sectionClass}>
                <SectionIntro eyebrow="Housing guides" title="Related Housing Guides">
                  <p>Makelaar research should sit inside the broader buy/sell decision: financing, bidding, platforms, storage timing and city choice.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <section id="lead-cta" className={cn(CITIES_FUNNEL_SECTION_SCROLL_MARGIN, "scroll-mt-24 overflow-hidden rounded-[2rem] bg-slate-950 p-6 text-white shadow-expatos-xl sm:p-8 lg:p-10")}>
                ...
                <div className="mt-6 rounded-3xl border border-white/10 bg-white/10 p-5 ring-1 ring-white/10">
                  ...
                  <ul className="mt-4 grid gap-3 text-sm leading-relaxed text-slate-300 sm:grid-cols-3">
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Confirm buy vs sell mandate and dual-agency rules.</li>
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Get written fee, exclusivity and cancellation terms.</li>
              ...
              <section id="services-links" className={sectionClass}>
                <SectionIntro eyebrow="Services ecosystem" title="Related Services for Home Buyers">
                  <p>Makelaar work often overlaps with mortgage advice, listing platforms, storage between completions and relocation timing.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <footer className="rounded-2xl border border-slate-200/80 bg-white/80 px-5 py-4 text-sm leading-relaxed text-foreground-muted shadow-sm">
                Some links on this page may be affiliate or referral links. If you use them, we may earn a commission at no extra cost to you. Ordering reflects relevance to expat estate-agent discovery and related buy/sell support — not pay-to-rank placement of makelaars. This is not legal, mortgage, property or financial advice.
              </footer>
          </div>
        </Container>
      </main>
