...
import {
  recruitmentAgenciesServicesNetherlandsPage as page,
  type RecruitmentAgencyLink,
  type RecruitmentAgencyProvider,
} from "./recruitmentAgenciesNetherlandsPageModel";
...
function FeatureCard({ title, body, iconIndex = 0 }: { title: string; body: string; iconIndex?: number }) {
  const icons = [Briefcase, Users, ClipboardList, FileText, ShieldCheck, Languages, WalletCards, Globe2] as const;
...
function LinkCard({ item, iconIndex = 0 }: { item: RecruitmentAgencyLink; iconIndex?: number }) {
  const icons = [Briefcase, Building2, Users, FileText, Globe2, ShieldCheck] as const;
  const Icon = icons[iconIndex % icons.length];
...
function ProviderCard({ provider }: { provider: RecruitmentAgencyProvider }) {
  return (
    <article className={cn("relative overflow-hidden rounded-3xl border border-slate-200/90 bg-white p-5 shadow-expatos-xl ring-1 ring-slate-900/[0.04]", movingNlCardMicroLiftClass)}>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className={providerTypeClass}>{provider.agencyType}</span>
      </div>
      ...
      <div className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-3">
          ...
          <p className="mt-1 text-foreground-muted">{provider.remoteSupport ? "Remote screening available. " : ""}{provider.inPersonAvailability}</p>
        </div>
      </div>
      <dl className="mt-4 grid gap-2 text-sm text-foreground-muted sm:grid-cols-2">
        ...
        <div><dt className="font-semibold text-foreground">Sectors</dt><dd>{provider.sectors.join(", ")}</dd></div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-[920px] divide-y divide-slate-200 text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-[0.12em] text-slate-500">
            <tr>
              {["Agency", "Cities Served", "Expat Focus", "Languages", "Remote", "Agency Type"].map((heading) => (
          ...
          <tbody className="divide-y divide-slate-100 bg-white">
            {page.comparisonTable.map((row) => (
              <tr key={row.agency} className="align-top">
                <th scope="row" className="px-4 py-4 font-bold text-foreground">{row.agency}</th>
                ...
                <td className="px-4 py-4 text-foreground-muted">{row.remoteSupport}</td>
                <td className="px-4 py-4 text-foreground-muted">{row.agencyType}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export function RecruitmentAgenciesNetherlandsView() {
  const breadcrumbCrumbs = [
    { name: "Home", item: new URL("/", baseUrl).toString() },
    { name: "Netherlands", item: new URL("/netherlands", baseUrl).toString() },
    { name: "Services", item: new URL("/netherlands/services/", baseUrl).toString() },
    { name: "Recruitment Agencies", item: new URL(page.path, baseUrl).toString() },
  ];
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          <nav aria-label="Breadcrumb" className="mb-6 text-sm text-copilot-text-secondary">
            <ol className="flex flex-wrap items-center gap-x-2 gap-y-1">
              ...
              <li aria-hidden className="text-copilot-text-muted">/</li>
              <li className="font-medium text-copilot-text-primary" aria-current="page">Recruitment Agencies</li>
            </ol>
          </nav>
            ...
            <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1fr)_0.78fr] lg:items-center">
              <div>
                ...
                <p className="mt-5 max-w-3xl rounded-2xl border border-amber-200/70 bg-amber-50/70 px-4 py-3 text-sm leading-relaxed text-amber-950 shadow-sm ring-1 ring-amber-200/40">
                  Provider inclusion is informational soft discovery — not a ranking or placement guarantee. Always verify agency type, fees, languages and fit directly. For how the channel works as a job seeker, use the Career guide.
                </p>
              </div>
              <HeroImage />
            </div>
          ...
          <nav
            ...
            aria-label="Recruitment agencies directory sections"
          >
          ...
          <div className={sectionStackClass}>
              ...
              <section id="snapshot" className={sectionClass}>
                <SectionIntro eyebrow="Snapshot" title="Recruitment Agencies at a Glance">
                  <p>Practical orientation points before you shortlist providers or reply to recruiter outreach.</p>
                </SectionIntro>
              ...
              <section id="agency-types" className={sectionClass}>
                <SectionIntro eyebrow="Agency models" title="Types of Recruitment Agencies">
                  <p>Generalist, specialist, interim/staffing, executive search and expat-focused desks solve different problems. Match the model to your seniority and sector before comparing brand names.</p>
                </SectionIntro>
                <div className="mt-6 grid gap-4">
                  {page.agencyTypeComparison.map((item) => (
                ...
                <VisualFigure visual={page.visuals.agencyTypes} className={sectionVisualMtClass} />
              </section>

              <section id="agency-role" className={sectionClass}>
                <SectionIntro eyebrow="Agency role" title="What Recruitment Agencies Actually Help With">
                  <p>Scope varies by desk. The common pattern is matching, market feedback, client introductions and process coordination — not guaranteed placement.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
                  {page.agencyServices.map((item, idx) => <FeatureCard key={item.title} {...item} iconIndex={idx} />)}
                </div>
                <VisualFigure visual={page.visuals.agencyServices} className={sectionVisualMtClass} />
              </section>

              <section id="compare" className={sectionClass}>
                <SectionIntro eyebrow="Comparison criteria" title="How to Compare Recruitment Agencies">
                  <p>Use criteria that reveal fit — sector depth, languages, commercial model, visa awareness and transparency — rather than star ratings or invented rankings.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
              ...
              <section id="fees" className={sectionClass}>
                <SectionIntro eyebrow="Costs" title="Fees and Commercial Models">
                  <p>Permanent placement fees are usually employer-paid. Interim and executive search models differ. Confirm who pays and what you sign before exclusive arrangements.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
                    rows={page.feeExamples}
                    columns={[
                      { key: "item", label: "Model / item" },
              ...
              <section id="documents" className={sectionClass}>
                <SectionIntro eyebrow="Documents" title="What to Prepare Before Agency Outreach">
                  <p>Requirements vary by role and client. A clean, consistent file helps agencies pitch you accurately without over-promising.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
                    rows={page.documentChecklist}
                    columns={[
                      { key: "document", label: "Document / prep item" },
              ...
              <section id="challenges" className={sectionClass}>
                <SectionIntro eyebrow="Common challenges" title="Challenges Expats Often Face">
                  <p>These friction points are common and do not mean agencies are useless — they are reasons to ask better questions early.</p>
                </SectionIntro>
              ...
              <section id="cities" className={sectionClass}>
                <SectionIntro eyebrow="City coverage" title="Recruitment Agencies Across the Netherlands">
                  <p>Many desks work online nationwide. Local presence can still matter for certain sectors, languages and branch-based staffing.</p>
                </SectionIntro>
              ...
              <section id="directory" className={sectionClass}>
                <SectionIntro eyebrow="Directory" title="Recruitment Agencies for Expats in the Netherlands">
                  <p>This editorial directory uses real companies and neutral descriptions. It does not rank providers, invent reviews or guarantee job or visa outcomes.</p>
                </SectionIntro>
              ...
              <section id="comparison" className={sectionClass}>
                <SectionIntro eyebrow="Comparison table" title="Compare Recruitment Agencies">
                  <p>Use this as a starting point for your shortlist. Coverage, language support and desks can change — verify details directly.</p>
                </SectionIntro>
                <div className={sectionStackClass}>
                  <ComparisonTable />
                </div>
              ...
              {showAffiliateProviders && affiliateProviderData ? (
                <section id="affiliate-providers" className={sectionClass}>
                  <SectionIntro eyebrow="Related support providers" title="Providers That May Help Around Your Search">
                    <p>
                      These listings support overlapping needs — tax, housing, insurance and relocation — not a ranking of recruitment agencies. Confirm services and credentials before you commit.
                    </p>
                  </SectionIntro>
                  <div className="mt-6">
                    <AffiliateBlockView
              ...
              <section id="questions" className={sectionClass}>
                <SectionIntro eyebrow="Agency interview" title="Questions Expats Should Ask">
                  <p>Use these questions before partnering with a desk. The answers should reveal sector depth, fee model and process quality.</p>
                </SectionIntro>
              ...
              <section id="lead-cta" className={cn(CITIES_FUNNEL_SECTION_SCROLL_MARGIN, "scroll-mt-24 overflow-hidden rounded-[2rem] bg-slate-950 p-6 text-white shadow-expatos-xl sm:p-8 lg:p-10")}>
                ...
                <div className="mt-6 rounded-3xl border border-white/10 bg-white/10 p-5 ring-1 ring-white/10">
                  <p className="text-sm font-bold text-white">Before you share documents</p>
                  <ul className="mt-4 grid gap-3 text-sm leading-relaxed text-slate-300 sm:grid-cols-3">
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Confirm agency type, sector desk and languages.</li>
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Ask who pays fees and when your CV will be sent.</li>
              ...
              <section id="faq" className={sectionClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently Asked Questions">
                  <p>Use these answers to separate provider discovery from career tactics — and to spot fee or process red flags early.</p>
                </SectionIntro>
              ...
              <section id="sources" className={sectionClass}>
                <SectionIntro eyebrow="Trust" title="Official & Trusted Resources">
                  <p>Employment and immigration context should be checked against official sources — not agency marketing alone.</p>
                </SectionIntro>
              ...
              <section id="related-guides" className={sectionClass}>
                <SectionIntro eyebrow="Related guides" title="Continue Your Research">
                  <p>Connect provider shortlists to the Career how-to guide, multi-channel job search and related services.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <section id="services-links" className={sectionClass}>
                <SectionIntro eyebrow="Services ecosystem" title="Related Services Around Job Search">
                  <p>Recruitment often overlaps with immigration, tax, coaching and relocation support.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <section id="explore-next" className={sectionClass}>
                <SectionIntro eyebrow="Explore next" title="Plan the Next Step">
                  <p>Move from provider discovery into channel tactics, visa context and city choice.</p>
                </SectionIntro>
              ...
              <footer className="rounded-2xl border border-slate-200/80 bg-white/80 px-5 py-4 text-sm leading-relaxed text-foreground-muted shadow-sm">
                Some links on this page may be affiliate or referral links. If you use them, we may earn a commission at no extra cost to you. Ordering reflects relevance to expat recruitment-agency discovery and related support — not pay-to-rank placement of recruitment agencies. This is not employment, immigration or recruitment advice.
              </footer>
          </div>
        </Container>
      </main>
