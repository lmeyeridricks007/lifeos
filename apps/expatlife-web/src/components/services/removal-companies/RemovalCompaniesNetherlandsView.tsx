...
import {
  removalCompaniesNetherlandsPage as page,
  type RemovalCompanyLink,
  type RemovalCompanyProvider,
} from "./removalCompaniesNetherlandsPageModel";
...
function LinkCard({ item, iconIndex = 0 }: { item: RemovalCompanyLink; iconIndex?: number }) {
...
function ProviderCard({ provider }: { provider: RemovalCompanyProvider }) {
  return (
    <article className={cn("relative overflow-hidden rounded-3xl border border-slate-200/90 bg-white p-5 shadow-expatos-xl ring-1 ring-slate-900/[0.04]", movingNlCardMicroLiftClass)}>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className={providerTypeClass}>{provider.moverType}</span>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-[920px] divide-y divide-slate-200 text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-[0.12em] text-slate-500">
            <tr>
              {["Mover", "Cities Served", "Expat Focus", "Languages", "Packing", "Mover Type"].map((heading) => (
          ...
          <tbody className="divide-y divide-slate-100 bg-white">
            {page.comparisonTable.map((row) => (
              <tr key={row.mover} className="align-top">
                <th scope="row" className="px-4 py-4 font-bold text-foreground">{row.mover}</th>
                ...
                <td className="px-4 py-4 text-foreground-muted">{row.packing}</td>
                <td className="px-4 py-4 text-foreground-muted">{row.moverType}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export function RemovalCompaniesNetherlandsView() {
  const breadcrumbCrumbs = [
    { name: "Home", item: new URL("/", baseUrl).toString() },
    { name: "Netherlands", item: new URL("/netherlands", baseUrl).toString() },
    { name: "Services", item: new URL("/netherlands/services/", baseUrl).toString() },
    { name: "Removal Companies", item: new URL(page.path, baseUrl).toString() },
  ];
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          <nav aria-label="Breadcrumb" className="mb-6 text-sm text-copilot-text-secondary">
            <ol className="flex flex-wrap items-center gap-x-2 gap-y-1">
              ...
              <li aria-hidden className="text-copilot-text-muted">/</li>
              <li className="font-medium text-copilot-text-primary" aria-current="page">Removal Companies</li>
            </ol>
          </nav>
            ...
            <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1fr)_0.78fr] lg:items-center">
              <div>
                ...
                <p className="mt-5 max-w-3xl rounded-2xl border border-amber-200/70 bg-amber-50/70 px-4 py-3 text-sm leading-relaxed text-amber-950 shadow-sm ring-1 ring-amber-200/40">
                  Provider inclusion is informational soft discovery — not a ranking or price guarantee. Always verify access, packing scope, insurance and fit directly. For international household goods use Removal companies; for full packages use Relocation agencies.
                </p>
              </div>
              <HeroImage />
            </div>
          ...
          <nav
            ...
            aria-label="Moving companies directory sections"
          >
          ...
          <div className={sectionStackClass}>
              ...
              <section id="differentiate" className={sectionClass}>
                <SectionIntro eyebrow="Provider types" title="Removal Companies Are Not the Same As…">
                  <p>Start here so you do not collect the wrong quotes. International removers, domestic movers, relocation packages and freight shipping solve different problems.</p>
                </SectionIntro>
                <div className="mt-6 grid gap-4 sm:grid-cols-2">
                  {page.differentiateCards.map((card, idx) => (
              ...
              <section id="snapshot" className={sectionClass}>
                <SectionIntro eyebrow="Snapshot" title="Removal Companies at a Glance">
                  <p>Practical orientation points before you shortlist international removers or request surveys.</p>
                </SectionIntro>
              ...
              <section id="mover-types" className={sectionClass}>
                <SectionIntro eyebrow="Removal models" title="Types of Domestic Removal Companies">
                  <p>Door-to-door, groupage, air priority, corporate mobility and destination unpack solve different timeline and volume problems. Match the model before comparing brand names.</p>
                </SectionIntro>
                <div className="mt-6 grid gap-4">
                  {page.moverTypeComparison.map((item) => (
                ...
                <VisualFigure visual={page.visuals.moverTypes} className={sectionVisualMtClass} />
              </section>

              <section id="mover-role" className={sectionClass}>
                <SectionIntro eyebrow="Remover role" title="What International Removers Actually Help With">
                  <p>Scope varies by company. The common pattern is export packing, transit modes, customs orientation and destination delivery — not within-NL van-only jobs.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
                  {page.moverServices.map((item, idx) => <FeatureCard key={item.title} {...item} iconIndex={idx} />)}
                </div>
                <VisualFigure visual={page.visuals.moverServices} className={sectionVisualMtClass} />
              </section>

              <section id="compare" className={sectionClass}>
                <SectionIntro eyebrow="Comparison criteria" title="How to Compare Removal Companies">
                  <p>Use criteria that reveal logistics fit — written scope, access surveys, packing inclusions, insurance and cancellation terms — rather than star ratings.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
              ...
              <section id="costs" className={sectionClass}>
                <SectionIntro eyebrow="Costs" title="Cost Drivers for International Removals">
                  <p>Volume, mode, seasonality, packing labour and destination access drive international removal prices. Ask for written assumptions before you compare totals.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
              ...
              <section id="prep" className={sectionClass}>
                <SectionIntro eyebrow="Prep" title="What to Prepare Before Move Day">
                  <p>Building access and parking permits often matter as much as van size. Share stairs photos and key dates early.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
              ...
              <section id="challenges" className={sectionClass}>
                <SectionIntro eyebrow="Common challenges" title="Challenges Expats Often Face on International Removals">
                  <p>These friction points are common on overseas household moves — they are reasons to ask better questions early, not to skip written surveys.</p>
                </SectionIntro>
              ...
              <section id="cities" className={sectionClass}>
                <SectionIntro eyebrow="City coverage" title="Removal Companies Across the Netherlands">
                  <p>International removers cover Dutch origins and destinations nationwide. Destination stairs and parking still matter after ocean transit.</p>
                </SectionIntro>
              ...
              <section id="directory" className={sectionClass}>
                <SectionIntro eyebrow="Directory" title="Removal Companies for Expats in the Netherlands">
                  <p>This editorial directory uses real companies and neutral descriptions focused on domestic moves. It does not rank providers, invent reviews or guarantee prices.</p>
                </SectionIntro>
              ...
              <section id="comparison" className={sectionClass}>
                <SectionIntro eyebrow="Comparison table" title="Compare Domestic Removal Companies">
                  <p>Use this as a starting point for your shortlist. Coverage, packing options and language support can change — verify details directly.</p>
                </SectionIntro>
                <div className={sectionStackClass}>
                  <ComparisonTable />
                </div>
              ...
              {showAffiliateProviders && affiliateProviderData ? (
                <section id="affiliate-providers" className={sectionClass}>
                  <SectionIntro eyebrow="Related support providers" title="Providers That May Help Around Your Move">
                    <p>
                      These listings support overlapping needs — housing search, insurance, tax and relocation coordination — not a ranking of removal companies. Confirm services and credentials before you commit.
                    </p>
                  </SectionIntro>
                  <div className="mt-6">
                    <AffiliateBlockView
              ...
              <section id="questions" className={sectionClass}>
                <SectionIntro eyebrow="Mover interview" title="Questions Expats Should Ask">
                  <p>Use these questions before paying a deposit. The answers should reveal access assumptions, insurance and packing scope.</p>
                </SectionIntro>
              ...
              <section id="lead-cta" className={cn(CITIES_FUNNEL_SECTION_SCROLL_MARGIN, "scroll-mt-24 overflow-hidden rounded-[2rem] bg-slate-950 p-6 text-white shadow-expatos-xl sm:p-8 lg:p-10")}>
                ...
                <div className="mt-6 rounded-3xl border border-white/10 bg-white/10 p-5 ring-1 ring-white/10">
                  ...
                  <ul className="mt-4 grid gap-3 text-sm leading-relaxed text-slate-300 sm:grid-cols-3">
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Confirm domestic-only scope vs international removals.</li>
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Get written access, packing and insurance assumptions.</li>
              ...
              <section id="faq" className={sectionClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently Asked Questions">
                  <p>Use these answers to separate domestic movers from international removals and relocation packages — and to spot quote red flags early.</p>
                </SectionIntro>
              ...
              <section id="sources" className={sectionClass}>
                <SectionIntro eyebrow="Trust" title="Official & Trusted Resources">
                  <p>Consumer and housing context should be checked against official sources — not mover marketing alone.</p>
                </SectionIntro>
              ...
              <section id="related-guides" className={sectionClass}>
                <SectionIntro eyebrow="Related guides" title="Continue Your Research">
                  <p>Connect domestic mover shortlists to international removals, relocation packages, housing and tax support.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <section id="services-links" className={sectionClass}>
                <SectionIntro eyebrow="Services ecosystem" title="Related Services Around a Dutch House Move">
                  <p>Domestic movers often overlap with housing search, relocation packages, tax timing and — when goods cross borders — international removals.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <section id="explore-next" className={sectionClass}>
                <SectionIntro eyebrow="Explore next" title="Plan the Next Step">
                  <p>Move from provider discovery into international logistics, relocation packages, housing and city choice.</p>
                </SectionIntro>
              ...
              <footer className="rounded-2xl border border-slate-200/80 bg-white/80 px-5 py-4 text-sm leading-relaxed text-foreground-muted shadow-sm">
                Some links on this page may be affiliate or referral links. If you use them, we may earn a commission at no extra cost to you. Ordering reflects relevance to expat domestic-mover discovery and related support — not pay-to-rank placement of removal companies. This is not moving, legal, tax or consumer-rights advice.
              </footer>
          </div>
        </Container>
      </main>
