...
import {
  businessConsultantsNetherlandsPage as page,
  type BusinessConsultantLink,
  type BusinessConsultantProvider,
} from "./businessConsultantsNetherlandsPageModel";
...
function FeatureCard({ title, body, iconIndex = 0 }: { title: string; body: string; iconIndex?: number }) {
  const icons = [Calculator, Landmark, ClipboardList, FileText, WalletCards, Globe2, Building2, ShieldCheck] as const;
...
function LinkCard({ item, iconIndex = 0 }: { item: BusinessConsultantLink; iconIndex?: number }) {
  const icons = [Calculator, Landmark, WalletCards, FileText, Building2, Globe2] as const;
  const Icon = icons[iconIndex % icons.length];
...
function ProviderCard({ provider }: { provider: BusinessConsultantProvider }) {
  return (
    <article className={cn("relative overflow-hidden rounded-3xl border border-slate-200/90 bg-white p-5 shadow-expatos-xl ring-1 ring-slate-900/[0.04]", movingNlCardMicroLiftClass)}>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className={providerTypeClass}>{provider.consultantType}</span>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-[920px] divide-y divide-slate-200 text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-[0.12em] text-slate-500">
            <tr>
              {["Firm / model", "Cities Served", "Expat Focus", "Languages", "Remote", "Type"].map((heading) => (
...
export function BusinessConsultantsNetherlandsView() {
  const breadcrumbCrumbs = [
    { name: "Home", item: new URL("/", baseUrl).toString() },
    { name: "Netherlands", item: new URL("/netherlands", baseUrl).toString() },
    { name: "Services", item: new URL("/netherlands/services/", baseUrl).toString() },
    { name: "Business consultants", item: new URL(page.path, baseUrl).toString() },
  ];
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          <nav aria-label="Breadcrumb" className="mb-6 text-sm text-copilot-text-secondary">
            <ol className="flex flex-wrap items-center gap-x-2 gap-y-1">
              ...
              <li aria-hidden className="text-copilot-text-muted">/</li>
              <li className="font-medium text-copilot-text-primary" aria-current="page">Business consultants</li>
            </ol>
          </nav>
            ...
            <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1fr)_0.78fr] lg:items-center">
              <div>
                ...
                <p className="mt-5 max-w-3xl rounded-2xl border border-amber-200/70 bg-amber-50/70 px-4 py-3 text-sm leading-relaxed text-amber-950 shadow-sm ring-1 ring-amber-200/40">
                  Provider inclusion is informational soft discovery — not a ranking. Always verify scope, fees, language support and credentials directly with the firm. This is not tax, legal or financial advice.
                </p>
              </div>
              <HeroImage />
            </div>
          ...
          <nav
            ...
            aria-label="Business consultants guide sections"
          >
          ...
          <div className={sectionStackClass}>
              ...
              <section id="differentiate" className={sectionClass}>
                <SectionIntro eyebrow="Provider types" title="Business Consultants Are Not the Same As…">
                  <p>Start here so you do not use the wrong directory. Business consultants own strategy and growth discovery; Starting a business / ZZP guides own how the system works; accountants own bookkeeping; tax advisors own advice; financial advisors own wealth planning.</p>
                </SectionIntro>
              ...
              <section id="snapshot" className={sectionClass}>
                <SectionIntro eyebrow="Snapshot" title="Business Consultants at a Glance">
                  <p>These are practical orientation points to understand before comparing consultants or opening engagement letters.</p>
                </SectionIntro>
              ...
              <section id="consultant-role" className={sectionClass}>
                <SectionIntro eyebrow="Consultant role" title="What Business Consultants Actually Help With">
                  <p>Scope varies by consultant. Common expat touchpoints are strategy, market entry, KvK facilitation, operations design and founder coaching — not bookkeeping filings or wealth planning rebuilds.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
                  {page.consultantServices.map((item, idx) => <FeatureCard key={item.title} {...item} iconIndex={idx} />)}
                </div>
                <VisualFigure visual={page.visuals.consultantServices} className={sectionVisualMtClass} />
              </section>

              <section id="consultant-types" className={sectionClass}>
                <SectionIntro eyebrow="Consultant models" title="Types of Business Consultants">
                  <p>Strategy coaches, market-entry facilitators, ops consultants, KvK setup coaches and tax/admin-adjacent setup support solve different problems. Match the model to your need before comparing brand names.</p>
                </SectionIntro>
                <div className="mt-6 grid gap-4">
                  {page.consultantTypeComparison.map((item) => (
                ...
                <VisualFigure visual={page.visuals.consultantTypes} className={sectionVisualMtClass} />
              </section>

              <section id="when-to-use" className={sectionClass}>
                <SectionIntro eyebrow="Decision paths" title="When to Use a Business Consultant vs DIY Guides vs Accountants">
                  <p>System how-to often suits Starting a business / ZZP guides. Bookkeeping belongs on Accountants. Complex tax advice belongs on Tax advisors. Strategy and growth shortlists live here. Match the path to your profile.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
              ...
              <section id="credentials" className={sectionClass}>
                <SectionIntro eyebrow="Checklist" title="What to Verify Before You Instruct a Business Consultant">
                  <p>Ask about scope, fees, deliverables and language support early. Use official KvK, RVO, Belastingdienst and Business.gov.nl orientation to verify context — this page is not a substitute for professional searches.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
              ...
              <section id="challenges" className={sectionClass}>
                <SectionIntro eyebrow="Common challenges" title="Challenges Expats Often Face">
                  <p>These friction points are common when arranging Dutch business consulting — they are reasons to clarify scope, language and which page owns bookkeeping, tax advice or practice how-to.</p>
                </SectionIntro>
              ...
              <section id="directory" className={sectionClass}>
                <SectionIntro eyebrow="Directory" title="Business Consultants for Expats in the Netherlands">
                  <p>This structured directory uses known setup-adjacent providers, role-based discovery paths and neutral descriptions. It does not rank consultants, invent Big Four/McKinsey listings or guarantee commercial outcomes.</p>
                </SectionIntro>
              ...
              <section id="comparison" className={sectionClass}>
                <SectionIntro eyebrow="Comparison table" title="Compare Business Consultant Models">
                  <p>Use this as a starting point for your shortlist. Scope, language support and package inclusions can change, so verify all details directly.</p>
                </SectionIntro>
                <div className={sectionStackClass}>
                  <ComparisonTable />
                </div>
              ...
              {showAffiliateProviders && affiliateProviderData ? (
                <section id="affiliate-providers" className={sectionClass}>
                  <SectionIntro eyebrow="Comparison support" title="Affiliates and Providers That May Help">
                    <p>
                      Soft discovery for tax/admin and financial-planning support that often sits next to strategy work — including Blue Umbrella, BROADSTREET, TaxSavers and expat-oriented planners — not a ranking of business consultants. Confirm scope, credentials and fees before you instruct anyone. For bookkeeping, use Accountants; for tax advice, use Tax advisors.
                    </p>
                  </SectionIntro>
                  <div className="mt-6">
                    <AffiliateBlockView
              ...
              <section id="questions" className={sectionClass}>
                <SectionIntro eyebrow="Firm interview" title="Questions Expats Should Ask">
                  <p>Use these questions before choosing a business consultant. The answers should help you understand scope, fees, deliverables and where bookkeeping or tax advice starts.</p>
                </SectionIntro>
              ...
              <section id="related-business" className={sectionClass}>
                <SectionIntro eyebrow="Business guides" title="Related Business & Work Guides">
                  <p>Business consultant research should sit beside ZZP setup, starting a business, freelancing, consultancy and contractor-vs-employee orientation.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
                  {page.relatedBusinessGuides.map((item, idx) => <LinkCard key={item.href} item={item} iconIndex={idx} />)}
                </div>
                <VisualFigure visual={page.visuals.relatedBusiness} className={sectionVisualMtClass} />
              </section>
              ...
              <section id="lead-cta" className={cn(CITIES_FUNNEL_SECTION_SCROLL_MARGIN, "scroll-mt-24 overflow-hidden rounded-[2rem] bg-slate-950 p-6 text-white shadow-expatos-xl sm:p-8 lg:p-10")}>
                ...
                <div className="mt-6 rounded-3xl border border-white/10 bg-white/10 p-5 ring-1 ring-white/10">
                  ...
                  <ul className="mt-4 grid gap-3 text-sm leading-relaxed text-slate-300 sm:grid-cols-3">
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Confirm strategy vs bookkeeping vs tax-advice scope.</li>
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Ask how project fees and overruns work in writing.</li>
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Map deliverables for strategy, market entry and ops work.</li>
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Use Accountants and Tax advisors when admin or advice starts.</li>
              ...
              <section id="faq" className={sectionClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently Asked Questions">
                  <p>Use these FAQ answers to identify what you still need to verify: role boundaries, DIY guides vs consultants, KvK facilitation and official source orientation.</p>
                </SectionIntro>
              ...
              <section id="sources" className={sectionClass}>
                <SectionIntro eyebrow="Trust" title="Official Resources">
                  <p>Consumers should verify the consultant and understand engagement scope before proceeding. Official sources help you check entrepreneur orientation rather than relying on marketing claims.</p>
                </SectionIntro>
              ...
              <section id="related-guides" className={sectionClass}>
                <SectionIntro eyebrow="Related guides" title="Continue Your Business Consultant Research">
                  <p>Use these guides to connect consultant discovery to accountants, tax advice, wealth planning, ZZP setup and starting a business.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <section id="services-links" className={sectionClass}>
                <SectionIntro eyebrow="Services ecosystem" title="Related Services for Business Setup">
                  <p>Business consultant discovery often overlaps with accountants, tax advisors, financial advisors and mortgage timing.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <section id="explore-next" className={sectionClass}>
                <SectionIntro eyebrow="Explore next" title="Plan the Next Step">
                  <p>Move from business consultant discovery into the directories and guides that shape your tax advice, business setup and city context.</p>
                </SectionIntro>
              ...
              <footer className="rounded-2xl border border-slate-200/80 bg-white/80 px-5 py-4 text-sm leading-relaxed text-foreground-muted shadow-sm">
                Some links on this page may be affiliate or referral links. If you use them, we may earn a commission at no extra cost to you. Ordering reflects relevance to expat business-consultant discovery and adjacent setup support — not pay-to-rank placement of consultants. This is not tax, financial or legal advice.
              </footer>
          </div>
        </Container>
      </main>
