...
import {
  daycareProvidersNetherlandsPage as page,
  type DaycareProvidersLink,
  type DaycareProvider,
} from "./daycareProvidersNetherlandsPageModel";
...
function FeatureCard({ title, body, iconIndex = 0 }: { title: string; body: string; iconIndex?: number }) {
  const icons = [Baby, Users, ClipboardList, FileText, Globe2, Home, Landmark, ShieldCheck] as const;
...
function LinkCard({ item, iconIndex = 0 }: { item: DaycareProvidersLink; iconIndex?: number }) {
  const icons = [Baby, Users, Home, FileText, Building2, Globe2] as const;
  const Icon = icons[iconIndex % icons.length];
...
function ProviderCard({ provider }: { provider: DaycareProvider }) {
  return (
...
export function DaycareProvidersNetherlandsView() {
  const breadcrumbCrumbs = [
    { name: "Home", item: new URL("/", baseUrl).toString() },
    { name: "Netherlands", item: new URL("/netherlands", baseUrl).toString() },
    { name: "Services", item: new URL("/netherlands/services/", baseUrl).toString() },
    { name: "Daycare providers", item: new URL(page.path, baseUrl).toString() },
  ];
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          <nav aria-label="Breadcrumb" className="mb-6 text-sm text-copilot-text-secondary">
            <ol className="flex flex-wrap items-center gap-x-2 gap-y-1">
              ...
              <li aria-hidden className="text-copilot-text-muted">/</li>
              <li className="font-medium text-copilot-text-primary" aria-current="page">Daycare providers</li>
            </ol>
          </nav>
            ...
            <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1fr)_0.78fr] lg:items-center">
              <div>
                ...
                <p className="mt-5 max-w-3xl rounded-2xl border border-amber-200/70 bg-amber-50/70 px-4 py-3 text-sm leading-relaxed text-amber-950 shadow-sm ring-1 ring-amber-200/40">
                  Provider inclusion is informational soft discovery — not a ranking. Always verify places, hours, LRK registration and contract terms directly with each location. This is not childcare placement or financial advice.
                </p>
              </div>
              <HeroImage />
            </div>
          ...
          <nav
            ...
            aria-label="Daycare providers directory sections"
          >
          ...
          <div className={sectionStackClass}>
              ...
              <section id="differentiate" className={sectionClass}>
                <SectionIntro eyebrow="Provider types" title="Daycare Providers Directory Is Not the Same As…">
                  <p>Start here so you do not use the wrong page. This directory compares daycare providers; the education guide explains how Dutch childcare works; BSO/VSO and international schools own their own paths.</p>
                </SectionIntro>
              ...
              <section id="snapshot" className={sectionClass}>
                <SectionIntro eyebrow="Snapshot" title="Daycare Providers at a Glance">
                  <p>These orientation points help you compare locations before you request tours or pay deposits.</p>
                </SectionIntro>
              ...
              <section id="provider-types" className={sectionClass}>
                <SectionIntro eyebrow="Provider types" title="Care Formats Expats Compare">
                  <p>KDV centres, gastouderbureaus, employer daycare and search directories solve different scheduling and commute problems. Match the format to your hours — not to a generic “best daycare” claim.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
                  {page.providerTypeCards.map((item, idx) => <FeatureCard key={item.title} {...item} iconIndex={idx} />)}
                </div>
                <VisualFigure visual={page.visuals.providerTypes} className={sectionVisualMtClass} />
              </section>

              <section id="commute" className={sectionClass}>
                <SectionIntro eyebrow="Commute" title="Location & Drop-off Reality">
                  <p>Daycare choice and housing choice are linked. Map home, daycare and work before you sign a lease or accept a place that wrecks morning drop-offs.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
                    rows={page.commuteOrientation}
                    columns={[
                ...
                <VisualFigure visual={page.visuals.commute} className={sectionVisualMtClass} />
              </section>

              <section id="quality" className={sectionClass}>
                <SectionIntro eyebrow="LRK & quality" title="Registration & Inspection Orientation">
                  <p>Popular cities fill by age group. Use LRK registration and GGD inspection context as verification tools — then still visit and ask location-specific questions.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
                    rows={page.qualitySteps}
                    columns={[
                      { key: "step", label: "Step" },
                ...
                <VisualFigure visual={page.visuals.quality} className={sectionVisualMtClass} />
              </section>

              <section id="visit-checklist" className={sectionClass}>
                <SectionIntro eyebrow="Visit checklist" title="What to Check on Daycare Visits">
                  <p>Bring the same questions to every tour. Marketing visits are useful — structured checklists are better for comparing providers fairly.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
              ...
              <section id="challenges" className={sectionClass}>
                <SectionIntro eyebrow="Common challenges" title="Challenges Expats Often Face">
                  <p>These friction points are common when shortlisting daycare providers — they are reasons to verify waitlists, language and housing timing early.</p>
                </SectionIntro>
              ...
              <section id="directory" className={sectionClass}>
                <SectionIntro eyebrow="Directory" title="Daycare Provider Discovery for Expats">
                  <p>This structured directory uses care-format orientations, city clusters and public registers. It does not rank daycares, invent reviews or guarantee places or fees.</p>
                </SectionIntro>
              ...
              <section id="comparison" className={sectionClass}>
                <SectionIntro eyebrow="Comparison table" title="Compare Discovery Paths">
                  <p>Use this as a starting point for your shortlist. Care format, city coverage and verification sources can change, so confirm all details directly.</p>
                </SectionIntro>
                <div className={sectionStackClass}>
                  <ComparisonTable />
                </div>
              ...
              {showAffiliateProviders && affiliateProviderData ? (
                <section id="affiliate-providers" className={sectionClass}>
                  <SectionIntro eyebrow="Related support" title="Daycare Discovery and Related Support">
                    <p>
                      Soft discovery for location search, housing near commute corridors and household setup — not a ranking of daycares. Align drop-off and start dates before you sign a lease or pay a deposit.
                    </p>
                  </SectionIntro>
                  <div className="mt-6">
                    <AffiliateBlockView
              ...
              <section id="questions" className={sectionClass}>
                <SectionIntro eyebrow="Provider interview" title="Questions Expats Should Ask">
                  <p>Use these questions before signing a contract or paying a deposit. The answers should help you understand waitlists, language, illness rules and notice periods.</p>
                </SectionIntro>
              ...
              <section id="related-education" className={sectionClass}>
                <SectionIntro eyebrow="Education guides" title="Related Education & Family Guides">
                  <p>Provider research should sit beside the daycare system guide, BSO/VSO, childcare allowance and school pathways.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <section id="lead-cta" className={cn(CITIES_FUNNEL_SECTION_SCROLL_MARGIN, "scroll-mt-24 overflow-hidden rounded-[2rem] bg-slate-950 p-6 text-white shadow-expatos-xl sm:p-8 lg:p-10")}>
                ...
                <div className="mt-6 rounded-3xl border border-white/10 bg-white/10 p-5 ring-1 ring-white/10">
                  <p className="text-sm font-bold text-white">Before you sign</p>
                  <ul className="mt-4 grid gap-3 text-sm leading-relaxed text-slate-300 sm:grid-cols-3">
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Match care type to hours and commute.</li>
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Verify LRK registration for the location.</li>
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Check waitlists by age group and days.</li>
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Align housing with drop-off and start date.</li>
              ...
              <section id="faq" className={sectionClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently Asked Questions">
                  <p>Use these FAQ answers to identify what you still need to verify: care type, LRK status, waitlists, commute and official sources.</p>
                </SectionIntro>
              ...
              <section id="sources" className={sectionClass}>
                <SectionIntro eyebrow="Trust" title="Official Resources">
                  <p>Families should verify registration and public information before relying on marketing claims. Official sources help with system context — providers handle places and contracts.</p>
                </SectionIntro>
              ...
              <section id="related-guides" className={sectionClass}>
                <SectionIntro eyebrow="Related guides" title="Continue Your Family Childcare Research">
                  <p>Use these guides to connect daycare-provider discovery to system context, school-age care, allowance and housing.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <section id="services-links" className={sectionClass}>
                <SectionIntro eyebrow="Services ecosystem" title="Related Services for Family Setup">
                  <p>Daycare shortlists often overlap with international schools, health insurance, rental agencies, estate agents and relocation support.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <section id="explore-next" className={sectionClass}>
                <SectionIntro eyebrow="Explore next" title="Plan the Next Step">
                  <p>Move from daycare-provider discovery into the guides and directories that shape housing, school pathways and city choice.</p>
                </SectionIntro>
              ...
              <footer className="rounded-2xl border border-slate-200/80 bg-white/80 px-5 py-4 text-sm leading-relaxed text-foreground-muted shadow-sm">
                Some links on this page may be affiliate or referral links. If you use them, we may earn a commission at no extra cost to you. Ordering reflects relevance to expat daycare-provider discovery and related settling steps — not pay-to-rank placement of childcare centres. This is not a ranking of daycare providers, and not childcare, financial or placement advice.
              </footer>
          </div>
        </Container>
      </main>
