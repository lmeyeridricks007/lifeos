...
import {
  notariesNetherlandsPage as page,
  type NotaryLink,
  type NotaryProvider,
...
function ProviderCard({ provider }: { provider: NotaryProvider }) {
  return (
    <article className={cn("relative overflow-hidden rounded-3xl border border-slate-200/90 bg-white p-5 shadow-expatos-xl ring-1 ring-slate-900/[0.04]", movingNlCardMicroLiftClass)}>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className={providerTypeClass}>{provider.notaryType}</span>
      </div>
      ...
      <dl className="mt-4 grid gap-2 text-sm text-foreground-muted sm:grid-cols-2">
        ...
        <div><dt className="font-semibold text-foreground">Consultation type</dt><dd>{provider.engagementType}</dd></div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-[920px] divide-y divide-slate-200 text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-[0.12em] text-slate-500">
            <tr>
              {["Notary", "Cities Served", "Expat Focus", "Languages", "Remote", "Notary Type"].map((heading) => (
...
export function NotariesNetherlandsView() {
  const breadcrumbCrumbs = [
    { name: "Home", item: new URL("/", baseUrl).toString() },
    { name: "Netherlands", item: new URL("/netherlands", baseUrl).toString() },
    { name: "Services", item: new URL("/netherlands/services/", baseUrl).toString() },
    { name: "Notaries", item: new URL(page.path, baseUrl).toString() },
  ];
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          <nav aria-label="Breadcrumb" className="mb-6 text-sm text-copilot-text-secondary">
            <ol className="flex flex-wrap items-center gap-x-2 gap-y-1">
              ...
              <li aria-hidden className="text-copilot-text-muted">/</li>
              <li className="font-medium text-copilot-text-primary" aria-current="page">Notaries</li>
            </ol>
          </nav>
            ...
            <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1fr)_0.78fr] lg:items-center">
              <div>
                ...
                <p className="mt-5 max-w-3xl rounded-2xl border border-amber-200/70 bg-amber-50/70 px-4 py-3 text-sm leading-relaxed text-amber-950 shadow-sm ring-1 ring-amber-200/40">
                  Provider inclusion is informational soft discovery — not a ranking. Always verify deed scope, fees, language support and attendance rules directly with the notaris.
                </p>
              </div>
              <HeroImage />
            </div>
          ...
          <nav
            ...
            aria-label="Notaries guide sections"
          >
          ...
          <div className={sectionStackClass}>
              ...
              <section id="differentiate" className={sectionClass}>
                <SectionIntro eyebrow="Provider types" title="Notaries Are Not the Same As…">
                  <p>Start here so you do not instruct the wrong provider. Notaries, estate agents, mortgage advisors and tax advisors solve different problems in a Dutch purchase.</p>
                </SectionIntro>
              ...
              <section id="snapshot" className={sectionClass}>
                <SectionIntro eyebrow="Snapshot" title="Notaries at a Glance">
              ...
              <section id="notary-role" className={sectionClass}>
                <SectionIntro eyebrow="Notaris role" title="What Notaries Actually Help With">
                  <p>Scope varies by deed type. The common purchase pattern is drafting and executing the transfer deed (levering), the mortgage deed (hypotheekakte), identity checks and registration handoff — not property search or mortgage advice.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
                  {page.notaryServices.map((item, idx) => <FeatureCard key={item.title} {...item} iconIndex={idx} />)}
                </div>
                <VisualFigure visual={page.visuals.notaryServices} className={sectionVisualMtClass} />
              </section>

              <section id="notary-types" className={sectionClass}>
                <SectionIntro eyebrow="Notaris models" title="Types of Notaries (Notaris Offices)">
                  <p>Conveyancing offices, inheritance practices, family-deed specialists, KNB member offices and multi-city networks solve different problems. Match the model to your deed type before comparing brand names.</p>
                </SectionIntro>
                <div className="mt-6 grid gap-4">
                  {page.notaryTypeComparison.map((item) => (
                ...
                <VisualFigure visual={page.visuals.notaryTypes} className={sectionVisualMtClass} />
              </section>

              <section id="purchase-role" className={sectionClass}>
                <SectionIntro eyebrow="Purchase pathways" title="House Purchase Situations for Expats">
                  <p>Use these profiles to plan notary timing for first-time purchases, mortgage completions, remote signing and sale chains — and keep mortgage advice on the Mortgage advisors page.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
                    rows={page.purchaseScenarios}
                    columns={[
                ...
                <VisualFigure visual={page.visuals.purchaseRole} className={sectionVisualMtClass} />
              </section>

              <section id="costs" className={sectionClass}>
                <SectionIntro eyebrow="Costs" title="Notary Costs Orientation">
                  <p>Fee estimates vary by deed type and complexity. Confirm inclusions and third-party costs in writing before the completion diary is locked.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
              ...
              <section id="documents" className={sectionClass}>
                <SectionIntro eyebrow="Documents" title="Documents Notaries Commonly Request">
                  <p>Requirements vary by deed type. Preparing ID, marital-status proof, purchase agreement and mortgage details early reduces avoidable delays before signing.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
              ...
              <section id="challenges" className={sectionClass}>
                <SectionIntro eyebrow="Common challenges" title="Challenges Expats Often Face">
                  <p>These friction points are common on Dutch deed files — they are reasons to clarify language support, attendance and fee estimates early.</p>
                </SectionIntro>
              ...
              <section id="cities" className={sectionClass}>
                <SectionIntro eyebrow="City coverage" title="Notaries Across the Netherlands">
                  <p>Local notaries handle most residential deeds, while many offices also support remote intake. Compare city experience, English support and completion-week capacity.</p>
                </SectionIntro>
              ...
              <section id="directory" className={sectionClass}>
                <SectionIntro eyebrow="Directory" title="Notaries for Expats in the Netherlands">
                  <p>This structured directory uses real provider types, public networks and neutral descriptions. It does not rank notaries, invent reviews or guarantee fees or completion timing.</p>
                </SectionIntro>
              ...
              <section id="comparison" className={sectionClass}>
                <SectionIntro eyebrow="Comparison table" title="Compare Notaries">
                  <p>Use this as a starting point for your shortlist. Provider coverage, language support and fee estimates can change, so verify all details directly.</p>
                </SectionIntro>
                <div className={sectionStackClass}>
                  <ComparisonTable />
                </div>
              ...
              {showAffiliateProviders && affiliateProviderData ? (
                <section id="affiliate-providers" className={sectionClass}>
                  <SectionIntro eyebrow="Provider support" title="Affiliates and Providers That May Help">
                    <p>
                      Soft discovery for housing search and mortgage support often used alongside a notary purchase file — not a ranking of notaries. Compare scope, credentials and fees before sharing documents.
                    </p>
                  </SectionIntro>
                  <div className="mt-6">
                    <AffiliateBlockView
              ...
              <section id="questions" className={sectionClass}>
                <SectionIntro eyebrow="Advisor interview" title="Questions Expats Should Ask">
                  <p>Use these questions before choosing a notary. The answers should help you understand whether the office can handle your specific deed file.</p>
                </SectionIntro>
              ...
              <section id="related-housing" className={sectionClass}>
                <SectionIntro eyebrow="Housing guides" title="Related Housing Guides">
                  <p>Notary research should sit inside the broader purchase decision: financing, makelaar support, process timing and city choice.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <section id="lead-cta" className={cn(CITIES_FUNNEL_SECTION_SCROLL_MARGIN, "scroll-mt-24 overflow-hidden rounded-[2rem] bg-slate-950 p-6 text-white shadow-expatos-xl sm:p-8 lg:p-10")}>
                ...
                <div className="mt-6 rounded-3xl border border-white/10 bg-white/10 p-5 ring-1 ring-white/10">
                  ...
                  <ul className="mt-4 grid gap-3 text-sm leading-relaxed text-slate-300 sm:grid-cols-3">
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Confirm deed scope: levering, hypotheekakte or both.</li>
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Get a written fee estimate including third-party costs.</li>
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Ask who will explain the deed in English before signing.</li>
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Plan power of attorney early if you cannot attend.</li>
              ...
              <section id="faq" className={sectionClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently Asked Questions">
                  <p>Use these FAQ answers to identify what you still need to verify: deed scope, fee estimates, attendance rules, document requirements and whether your expat file needs specialist support.</p>
                </SectionIntro>
              ...
              <section id="sources" className={sectionClass}>
                <SectionIntro eyebrow="Trust" title="Regulation & Official Resources">
                  <p>Consumers should verify the assigned notaris and understand fee estimates before proceeding. Official sources help you check professional context rather than relying on marketing claims.</p>
                </SectionIntro>
              ...
              <section id="services-links" className={sectionClass}>
                <SectionIntro eyebrow="Services ecosystem" title="Related Services for Home Buyers">
                  <p>Notary work often overlaps with estate agents, mortgage advisors, financial advisors, tax advisors and insurance planning after purchase.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <footer className="rounded-2xl border border-slate-200/80 bg-white/80 px-5 py-4 text-sm leading-relaxed text-foreground-muted shadow-sm">
                Some links on this page may be affiliate or referral links. If you use them, we may earn a commission at no extra cost to you. Ordering reflects relevance to expat notary discovery and related purchase support — not pay-to-rank placement of notaries. This is not legal, mortgage, property or financial advice.
              </footer>
          </div>
        </Container>
      </main>
