...
import {
  storageCompaniesNetherlandsPage as page,
  type StorageCompanyLink,
  type StorageCompanyProvider,
} from "./storageCompaniesNetherlandsPageModel";
...
function LinkCard({ item, iconIndex = 0 }: { item: StorageCompanyLink; iconIndex?: number }) {
...
function ProviderCard({ provider }: { provider: StorageCompanyProvider }) {
  return (
    <article className={cn("relative overflow-hidden rounded-3xl border border-slate-200/90 bg-white p-5 shadow-expatos-xl ring-1 ring-slate-900/[0.04]", movingNlCardMicroLiftClass)}>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className={providerTypeClass}>{provider.storageType}</span>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-[920px] divide-y divide-slate-200 text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-[0.12em] text-slate-500">
            <tr>
              {["Provider", "Cities Served", "Expat Focus", "Languages", "Access", "Storage Type"].map((heading) => (
          ...
          <tbody className="divide-y divide-slate-100 bg-white">
            {page.comparisonTable.map((row) => (
              <tr key={row.provider} className="align-top">
                ...
                <td className="px-4 py-4 text-foreground-muted">{row.access}</td>
                <td className="px-4 py-4 text-foreground-muted">{row.storageType}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export function StorageCompaniesNetherlandsView() {
  const breadcrumbCrumbs = [
    { name: "Home", item: new URL("/", baseUrl).toString() },
    { name: "Netherlands", item: new URL("/netherlands", baseUrl).toString() },
    { name: "Services", item: new URL("/netherlands/services/", baseUrl).toString() },
    { name: "Storage Companies", item: new URL(page.path, baseUrl).toString() },
  ];
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          <nav aria-label="Breadcrumb" className="mb-6 text-sm text-copilot-text-secondary">
            <ol className="flex flex-wrap items-center gap-x-2 gap-y-1">
              ...
              <li aria-hidden className="text-copilot-text-muted">/</li>
              <li className="font-medium text-copilot-text-primary" aria-current="page">Storage Companies</li>
            </ol>
          </nav>
            ...
            <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1fr)_0.78fr] lg:items-center">
              <div>
                ...
                <p className="mt-5 max-w-3xl rounded-2xl border border-amber-200/70 bg-amber-50/70 px-4 py-3 text-sm leading-relaxed text-amber-950 shadow-sm ring-1 ring-amber-200/40">
                  Provider inclusion is informational soft discovery — not a ranking or price guarantee. Always verify unit size, access hours, climate, insurance and fit directly. For domestic vans use Moving companies; for international household goods use Removal companies.
                </p>
              </div>
              <HeroImage />
            </div>
          ...
          <nav
            ...
            aria-label="Storage companies directory sections"
          >
          ...
          <div className={sectionStackClass}>
              ...
              <section id="differentiate" className={sectionClass}>
                <SectionIntro eyebrow="Provider types" title="Storage Companies Are Not the Same As…">
                  <p>Start here so you do not collect the wrong quotes. Storage, domestic movers, international removals, housing platforms and rental agencies solve different problems.</p>
                </SectionIntro>
              ...
              <section id="snapshot" className={sectionClass}>
                <SectionIntro eyebrow="Snapshot" title="Storage Companies at a Glance">
                  <p>Practical orientation points before you shortlist storage providers or request quotes.</p>
                </SectionIntro>
              ...
              <section id="storage-types" className={sectionClass}>
                <SectionIntro eyebrow="Storage models" title="Types of Storage Companies">
                  <p>Self-storage chains, pick-up-and-store services, climate units, garage boxes and budget sites solve different access and volume problems. Match the model before comparing brand names.</p>
                </SectionIntro>
                <div className="mt-6 grid gap-4">
                  {page.storageTypeComparison.map((item) => (
                ...
                <VisualFigure visual={page.visuals.storageTypes} className={sectionVisualMtClass} />
              </section>

              <section id="storage-role" className={sectionClass}>
                <SectionIntro eyebrow="Storage role" title="What Storage Companies Actually Help With">
                  <p>Scope varies by company. The common pattern is unit rental, pick-up options, climate control, security access and short move bridges — not domestic vans or international containers.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
                  {page.storageServices.map((item, idx) => <FeatureCard key={item.title} {...item} iconIndex={idx} />)}
                </div>
                <VisualFigure visual={page.visuals.storageServices} className={sectionVisualMtClass} />
              </section>

              <section id="compare" className={sectionClass}>
                <SectionIntro eyebrow="Comparison criteria" title="How to Compare Storage Companies">
                  <p>Use criteria that reveal storage fit — unit size, access hours, climate, insurance and notice periods — rather than star ratings.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
              ...
              <section id="costs" className={sectionClass}>
                <SectionIntro eyebrow="Costs" title="Cost Drivers for Dutch Storage">
                  <p>Unit size, location, climate options, access hours and contract terms drive Dutch storage prices. Ask what fees are included before you compare totals.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
              ...
              <section id="prep" className={sectionClass}>
                <SectionIntro eyebrow="Prep" title="What to Prepare Before Storing">
                  <p>Access hours and inventory labels often matter as much as unit size. Share volume photos and lease-gap dates early.</p>
                </SectionIntro>
                <div className="mt-6">
                  <InfoTable
              ...
              <section id="challenges" className={sectionClass}>
                <SectionIntro eyebrow="Common challenges" title="Challenges Expats Often Face with Storage">
              ...
              <section id="cities" className={sectionClass}>
                <SectionIntro eyebrow="City coverage" title="Storage Companies Across the Netherlands">
                  <p>Major chains cover many Dutch cities. Local access hours, parking and climate options still vary by site.</p>
                </SectionIntro>
              ...
              <section id="directory" className={sectionClass}>
                <SectionIntro eyebrow="Directory" title="Storage Companies for Expats in the Netherlands">
                  <p>This editorial directory uses real companies and neutral descriptions focused on self-storage and interim furniture storage. It does not rank providers, invent reviews or guarantee prices.</p>
                </SectionIntro>
              ...
              <section id="comparison" className={sectionClass}>
                <SectionIntro eyebrow="Comparison table" title="Compare Domestic Storage Companies">
                  <p>Use this as a starting point for your shortlist. Coverage, climate options and language support can change — verify details directly.</p>
                </SectionIntro>
                <div className={sectionStackClass}>
                  <ComparisonTable />
                </div>
              ...
              {showAffiliateProviders && affiliateProviderData ? (
                <section id="affiliate-providers" className={sectionClass}>
                  <SectionIntro eyebrow="Related support providers" title="Providers That May Help Around Storage">
                    <p>
                      These listings support overlapping needs — housing search, insurance, tax and relocation coordination — not a ranking of storage companies. Confirm services and credentials before you commit.
                    </p>
                  </SectionIntro>
                  <div className="mt-6">
                    <AffiliateBlockView
              ...
              <section id="questions" className={sectionClass}>
                <SectionIntro eyebrow="Storage interview" title="Questions Expats Should Ask">
                  <p>Use these questions before paying a deposit. The answers should reveal access assumptions, insurance and climate scope.</p>
                </SectionIntro>
              ...
              <section id="lead-cta" className={cn(CITIES_FUNNEL_SECTION_SCROLL_MARGIN, "scroll-mt-24 overflow-hidden rounded-[2rem] bg-slate-950 p-6 text-white shadow-expatos-xl sm:p-8 lg:p-10")}>
                ...
                <div className="mt-6 rounded-3xl border border-white/10 bg-white/10 p-5 ring-1 ring-white/10">
                  ...
                  <ul className="mt-4 grid gap-3 text-sm leading-relaxed text-slate-300 sm:grid-cols-3">
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Confirm storage-only scope vs domestic movers or international removals.</li>
                    <li className="flex gap-2"><BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" aria-hidden />Get written access hours, climate and insurance assumptions.</li>
              ...
              <section id="faq" className={sectionClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently Asked Questions">
                  <p>Use these answers to separate storage from domestic movers, international removals and housing platforms — and to spot quote red flags early.</p>
                </SectionIntro>
              ...
              <section id="sources" className={sectionClass}>
                <SectionIntro eyebrow="Trust" title="Official & Trusted Resources">
                  <p>Consumer and housing context should be checked against official sources — not storage marketing alone.</p>
                </SectionIntro>
              ...
              <section id="related-guides" className={sectionClass}>
                <SectionIntro eyebrow="Related guides" title="Continue Your Research">
                  <p>Connect storage shortlists to domestic movers, international removals, housing platforms and relocation support.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <section id="services-links" className={sectionClass}>
                <SectionIntro eyebrow="Services ecosystem" title="Related Services Around Dutch Interim Storage">
                  <p>Storage often overlaps with housing search, domestic moves, relocation packages and — when goods cross borders — international removals.</p>
                </SectionIntro>
                <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <section id="explore-next" className={sectionClass}>
                <SectionIntro eyebrow="Explore next" title="Plan the Next Step">
                  <p>Move from storage discovery into domestic movers, international removals, housing search and city choice.</p>
                </SectionIntro>
              ...
              <footer className="rounded-2xl border border-slate-200/80 bg-white/80 px-5 py-4 text-sm leading-relaxed text-foreground-muted shadow-sm">
                Some links on this page may be affiliate or referral links. If you use them, we may earn a commission at no extra cost to you. Ordering reflects relevance to expat storage discovery and related support — not pay-to-rank placement of storage companies. This is not storage, legal, tax or consumer-rights advice.
              </footer>
          </div>
        </Container>
      </main>
