...
import { getZzpNetherlandsSupportCardGroups } from "@/src/lib/recommended-services/pageRegistryRecommendations";
...
export function ZzpTaxAffiliateSupport() {
  const { taxPayroll } = getZzpNetherlandsSupportCardGroups();
  return (
...
export function ZzpResidenceAffiliateSupport() {
  const { visaPermits } = getZzpNetherlandsSupportCardGroups();
  return (
    <div className="mt-6 space-y-4">
      <ProviderCardSection
        eyebrow="Visa & permits"
        title="Immigration support when permits intersect with ZZP"
        description="Self-employment registration and IND rules are separate topics — use qualified help for permit timing."
        cards={visaPermits}
      />
      <SectionAffiliateDisclosure />
    </div>
...
export function ZzpNetherlandsRecommendedServices() {
  const { visaPermits, taxPayroll, relocation, setup } = getZzpNetherlandsSupportCardGroups();
  ...
  return (
    <div className="mt-8 space-y-8">
      ...
      <ProviderCardSection
        eyebrow="Tax & accounting"
        title="Accountants and tax advisors for ZZP'ers"
      ...
      <ProviderCardSection
        ...
        description="Route changes and self-employment visa questions alongside ZZP registration."
        cards={visaPermits}
      />
      <ProviderCardSection
        eyebrow="Relocation"
        title="Relocation services aligned with freelance timing"
        description="Moves that depend on client pipeline, housing search and arrival planning."
        cards={relocation}
      />
      <ProviderCardSection
        eyebrow="Post-move setup"
        title="Banking, health insurance and housing during transitions"
        description="Many ZZP'ers coordinate accounts and insurance around registration and first client payments."
        cards={setup}
      />
      <SectionAffiliateDisclosure />
    </div>
