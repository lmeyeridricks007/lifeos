...
import {
  ...
  PRESCRIPTIONS_NETHERLANDS_PATH,
  prescriptionsNetherlandsPage as page,
  type MistakeCard as MistakeCardData,
  type PrescriptionLink,
  type TimelineStep,
  type UrgencyRow,
} from "./prescriptionsNetherlandsPageModel";
...
const snapshotIcons = [Stethoscope, Pill, RefreshCw, Store] as const;
const orientationIcons = [Stethoscope, ClipboardList, Pill, RefreshCw] as const;
...
function LinkCard({ item, iconIndex = 0, tone = "default" }: { item: PrescriptionLink; iconIndex?: number; tone?: "default" | "onDark" }) {
  const Icon = iconPool[iconIndex % iconPool.length];
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four building blocks before your first Dutch prescription</h3>
...
export function PrescriptionsNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Prescriptions", item: new URL(PRESCRIPTIONS_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Prescription file checklist" items={page.safetyFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Six building blocks of Dutch prescriptions" fullWidth>
                  <p>
                    These six cards cover almost every prescription question expats ask first — who issues recepten,
                    electronic prescriptions, pharmacy pickup, herhaalrecept, medication lists and foreign prescriptions.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <DecisionFlowStrip labels={page.howItWorks.flowLabels} eyebrow="Prescription pathway" />
              <Timeline steps={page.howItWorks.timeline} eyebrow="Stage" />
              <BulletPanel title="Decision tips" items={page.howItWorks.decisionTips} />
              <HowToStepCards steps={page.howItWorks.howToSteps} title="How to use Dutch prescriptions" />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="who-issues"
              tipsKey="whoIssues"
              visual={page.visuals.whoIssues}
              intro={
                <SectionIntro eyebrow="Who issues" title={page.whoIssues.heading} fullWidth>
                  <p>{page.whoIssues.intro}</p>
                  {page.whoIssues.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.whoIssues.cards.length)}>
                {page.whoIssues.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="Who-issues essentials" items={page.whoIssues.points} />
              <InfoTable
                columns={contactRouteColumns}
                rows={page.whoIssues.contrastRows.map((row) => ({
                  route: row.route,
                  when: row.when,
                  how: row.how,
                  note: row.note,
              ...
              />
              <ChecklistBlock title="Who-issues checklist" items={page.whoIssues.checklist} columns={2} />
              <ScenarioTable title="Who issues in practice" rows={page.whoIssues.scenarios} />
              <BulletPanel title="Tips" items={page.whoIssues.tips} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="electronic"
              tipsKey="electronic"
              visual={page.visuals.electronic}
              intro={
                <SectionIntro eyebrow="E-prescription" title={page.electronic.heading} fullWidth>
                  <p>{page.electronic.intro}</p>
                  {page.electronic.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.electronic.cards.length)}>
                {page.electronic.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="Electronic prescription essentials" items={page.electronic.points} />
              <ChecklistBlock title="E-prescription checklist" items={page.electronic.checklist} columns={2} />
              <ScenarioTable title="Electronic prescriptions in practice" rows={page.electronic.scenarios} />
              <BulletPanel title="Tips" items={page.electronic.tips} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="pharmacy-pickup"
              tipsKey="pharmacyPickup"
              visual={page.visuals.pharmacyPickup}
              intro={
                <SectionIntro eyebrow="Pharmacy pickup" title={page.pharmacyPickup.heading} fullWidth>
                  <p>{page.pharmacyPickup.intro}</p>
                  {page.pharmacyPickup.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.pharmacyPickup.cards.length)}>
                {page.pharmacyPickup.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="Pickup essentials" items={page.pharmacyPickup.points} />
              <ChecklistBlock title="Pharmacy pickup checklist" items={page.pharmacyPickup.checklist} columns={2} />
              <ScenarioTable title="Pharmacy pickup in practice" rows={page.pharmacyPickup.scenarios} />
              <BulletPanel title="Tips" items={page.pharmacyPickup.tips} />
              <GuideCrossLink
                href={page.pharmacyPickup.crossLink.href}
                title={page.pharmacyPickup.crossLink.label}
                description={page.pharmacyPickup.crossLink.description}
            ...
            <PremiumGuideSection
              id="herhaalrecept"
              tipsKey="herhaalrecept"
              visual={page.visuals.herhaalrecept}
              intro={
                <SectionIntro eyebrow="Herhaalrecept" title={page.herhaalrecept.heading} fullWidth>
                  <p>{page.herhaalrecept.intro}</p>
                  {page.herhaalrecept.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <DecisionFlowStrip labels={page.herhaalrecept.timeline.map((step) => step.title)} eyebrow="Herhaalrecept loop" />
              <Timeline steps={page.herhaalrecept.timeline} eyebrow="Step" />
              <div className={guidePremiumCardGridClass(page.herhaalrecept.cards.length)}>
                {page.herhaalrecept.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="Herhaalrecept essentials" items={page.herhaalrecept.points} />
              <ChecklistBlock title="Herhaalrecept checklist" items={page.herhaalrecept.checklist} columns={2} />
              <ScenarioTable title="Herhaalrecept in practice" rows={page.herhaalrecept.scenarios} />
              <BulletPanel title="Tips" items={page.herhaalrecept.tips} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="medication-list"
              tipsKey="medicationList"
              visual={page.visuals.medicationList}
              intro={
                <SectionIntro eyebrow="Medication list" title={page.medicationList.heading} fullWidth>
                  <p>{page.medicationList.intro}</p>
                  {page.medicationList.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.medicationList.cards.length)}>
                {page.medicationList.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="Medication-list essentials" items={page.medicationList.points} />
              <ChecklistBlock title="Medication-list checklist" items={page.medicationList.checklist} columns={2} />
              <ScenarioTable title="Medication lists in practice" rows={page.medicationList.scenarios} />
              <BulletPanel title="Tips" items={page.medicationList.tips} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="foreign"
              tipsKey="foreign"
              visual={page.visuals.foreign}
              intro={
                <SectionIntro eyebrow="Foreign Rx" title={page.foreign.heading} fullWidth>
                  <p>{page.foreign.intro}</p>
                  {page.foreign.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.foreign.cards.length)}>
                {page.foreign.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="Foreign prescription essentials" items={page.foreign.points} />
              <UrgencyBoard rows={page.foreign.urgencyRows} />
              <ChecklistBlock title="Foreign prescriptions checklist" items={page.foreign.checklist} columns={2} />
              <ScenarioTable title="Foreign prescriptions in practice" rows={page.foreign.scenarios} />
              <BulletPanel title="Tips" items={page.foreign.tips} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>
                    Orientation answers only — confirm your own situation with your pharmacist, GP and insurer. Call 112
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related guides for prescriptions" fullWidth>
                  <p>
                    Prescriptions connect to pharmacies, GP care, emergency routes, insurance and family pathways — read
                    them together.
                  </p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Health hub" title="Explore the healthcare cluster" fullWidth>
                  <p>This page is the prescriptions cornerstone — explore related health topics next.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Plan the next step" tone="onDark" fullWidth>
                  <p>
                    Pick the card that matches what is still open — pharmacy setup, GP registration, insurance,
                    emergency routes or children&apos;s care — and verify specifics on the official sources below.
                  </p>
                </SectionIntro>
              }
            >
