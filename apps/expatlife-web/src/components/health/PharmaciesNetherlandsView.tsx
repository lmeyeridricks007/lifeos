...
import {
  ...
  PHARMACIES_NETHERLANDS_PATH,
  pharmaciesNetherlandsPage as page,
  type ComparisonRow,
  type MistakeCard as MistakeCardData,
  type PharmacyLink,
  type TimelineStep,
  type UrgencyRow,
} from "./pharmaciesNetherlandsPageModel";
...
const snapshotIcons = [Store, Pill, ShoppingBag, Siren] as const;
const orientationIcons = [Store, ClipboardList, Pill, Clock] as const;
...
function LinkCard({ item, iconIndex = 0, tone = "default" }: { item: PharmacyLink; iconIndex?: number; tone?: "default" | "onDark" }) {
  const Icon = iconPool[iconIndex % iconPool.length];
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four building blocks before your first apotheek visit</h3>
...
export function PharmaciesNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Pharmacies", item: new URL(PHARMACIES_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Pharmacy file checklist" items={page.safetyFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Six building blocks of Dutch pharmacies" fullWidth>
                  <p>
                    These six cards cover almost every pharmacy question expats ask first — finding an apotheek, hours,
                    OTC versus prescription pickup, the service desk, counseling and emergency pharmacy orientation.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <DecisionFlowStrip labels={page.howItWorks.flowLabels} eyebrow="Pharmacy pathway" />
              <Timeline steps={page.howItWorks.timeline} eyebrow="Stage" />
              <BulletPanel title="Decision tips" items={page.howItWorks.decisionTips} />
              <HowToStepCards steps={page.howItWorks.howToSteps} title="How to use a Dutch pharmacy" />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Finding a pharmacy" title={page.finding.heading} fullWidth>
            ...
            >
              ...
              />
              <ChecklistBlock title="Finding-a-pharmacy checklist" items={page.finding.checklist} columns={2} />
              <ScenarioTable title="Finding a pharmacy in practice" rows={page.finding.scenarios} />
              <BulletPanel title="Tips" items={page.finding.tips} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="opening-hours"
              tipsKey="openingHours"
              visual={page.visuals.openingHours}
              intro={
                <SectionIntro eyebrow="Opening hours" title={page.openingHours.heading} fullWidth>
                  <p>{page.openingHours.intro}</p>
                  {page.openingHours.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.openingHours.cards.length)}>
                {page.openingHours.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="Hours essentials" items={page.openingHours.points} />
              <ChecklistBlock title="Opening-hours checklist" items={page.openingHours.checklist} columns={2} />
              <ScenarioTable title="Opening hours in practice" rows={page.openingHours.scenarios} />
              <BulletPanel title="Tips" items={page.openingHours.tips} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="otc-vs-rx"
              tipsKey="otcVsRx"
              visual={page.visuals.otcVsRx}
              intro={
                <SectionIntro eyebrow="OTC vs Rx" title={page.otcVsRx.heading} fullWidth>
                  <p>{page.otcVsRx.intro}</p>
                  {page.otcVsRx.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={pharmacyCompareColumns}
                rows={page.otcVsRx.rows.map((row: ComparisonRow) => ({
                  type: row.type,
                  focus: row.focus,
              ...
              />
              <div className={guidePremiumCardGridClass(page.otcVsRx.cards.length)}>
                {page.otcVsRx.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="OTC vs prescription essentials" items={page.otcVsRx.points} />
              <ChecklistBlock title="OTC vs Rx checklist" items={page.otcVsRx.checklist} columns={2} />
              <ScenarioTable title="OTC vs prescription in practice" rows={page.otcVsRx.scenarios} />
              <BulletPanel title="Tips" items={page.otcVsRx.tips} />
              <NotePanel>
                Deep herhaalrecept and e-prescription mechanics belong on the Prescriptions guide. This section stays at
                overall pickup orientation.
              </NotePanel>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="service-desk"
              tipsKey="serviceDesk"
              visual={page.visuals.serviceDesk}
              intro={
                <SectionIntro eyebrow="Service desk" title={page.serviceDesk.heading} fullWidth>
                  <p>{page.serviceDesk.intro}</p>
                  {page.serviceDesk.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.serviceDesk.cards.length)}>
                {page.serviceDesk.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="Service desk essentials" items={page.serviceDesk.points} />
              <InfoTable
                columns={contactRouteColumns}
                rows={page.serviceDesk.contrastRows.map((row) => ({
                  route: row.route,
                  when: row.when,
                  how: row.how,
                  note: row.note,
              ...
              />
              <ChecklistBlock title="Service desk checklist" items={page.serviceDesk.checklist} columns={2} />
              <ScenarioTable title="Service desk in practice" rows={page.serviceDesk.scenarios} />
              <BulletPanel title="Tips" items={page.serviceDesk.tips} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="counseling"
              tipsKey="counseling"
              visual={page.visuals.counseling}
              intro={
                <SectionIntro eyebrow="Counseling" title={page.counseling.heading} fullWidth>
                  <p>{page.counseling.intro}</p>
                  {page.counseling.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.counseling.cards.length)}>
                {page.counseling.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="Counseling essentials" items={page.counseling.points} />
              <ChecklistBlock title="Counseling checklist" items={page.counseling.checklist} columns={2} />
              <ScenarioTable title="Counseling in practice" rows={page.counseling.scenarios} />
              <BulletPanel title="Tips" items={page.counseling.tips} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Emergency pharmacy" title={page.emergency.heading} fullWidth>
                  <p>{page.emergency.intro}</p>
            ...
            >
              <BulletPanel title="Emergency pharmacy essentials" items={page.emergency.points} />
              <UrgencyBoard rows={page.emergency.urgencyRows} />
              <ChecklistBlock title="Emergency pharmacy checklist" items={page.emergency.checklist} columns={2} />
              <ScenarioTable title="Emergency pharmacy in practice" rows={page.emergency.scenarios} />
              <WarningPanel title="Important" items={[page.emergency.disclaimer]} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related guides for pharmacies" fullWidth>
                  <p>
                    Pharmacies connect to GP care, emergency routes, insurance and family pathways — read them together.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Health hub" title="Explore the healthcare cluster" fullWidth>
                  <p>This page is the pharmacies cornerstone — explore related health topics next.</p>
                </SectionIntro>
              }
            >
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Plan the next step" tone="onDark" fullWidth>
                  <p>
                    ...
                    children&apos;s care or prescriptions depth — and verify specifics on the official sources below.
                  </p>
                </SectionIntro>
              }
            >
