...
import {
  ...
  dentistsNetherlandsPage as page,
  type DentistLink,
  type MistakeCard as MistakeCardData,
  type TimelineStep,
  type UrgencyRow,
} from "./dentistsNetherlandsPageModel";
...
const snapshotIcons = [Smile, ClipboardList, Hospital, Siren] as const;
const orientationIcons = [WalletCards, Smile, CalendarCheck, Siren] as const;
...
const urgencyGroups = [
  ...
  },
  {
    ...
    title: "Urgent — your dentist or the out-of-hours dental service",
    icon: Clock,
  ...
  },
  {
    ...
    title: "Routine — a normal check-up appointment",
    icon: CalendarCheck,
...
function LinkCard({ item, iconIndex = 0, tone = "default" }: { item: DentistLink; iconIndex?: number; tone?: "default" | "onDark" }) {
  const Icon = iconPool[iconIndex % iconPool.length];
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four building blocks before your first dental visit</h3>
...
export function DentistsNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Dentists", item: new URL(DENTISTS_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Dental file — keep these together" items={page.safetyFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Six building blocks of Dutch dental care" fullWidth>
                  <p>
                    Almost every question expats ask about dentists fits into one of these six blocks. Read them once now,
            ...
            >
              ...
              <NotePanel>{page.howItWorks.keyNote}</NotePanel>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Finding a dentist" title={page.finding.heading} fullWidth>
            ...
            >
              <Timeline steps={page.finding.timeline} eyebrow="Phase" />
              <ChecklistBlock title="Registration checklist" items={page.finding.checklist} columns={2} />
              <ol className="space-y-3">
                {page.finding.howToSteps.map((step, index) => (
              ...
              <BulletPanel title="If nearby practices are full" items={page.finding.waitlistTips} />
            </PremiumGuideSection>
            ...
            >
              <InfoTable
                ...
                rows={page.insurance.comparisonRows.map((row) => ({
                  topic: row.topic,
                  children: row.children,
                  adults: row.adults,
                }))}
              />
              <BulletPanel title="Insurance practicalities" items={page.insurance.tips} />
              <ChecklistBlock title="Dental insurance checklist" items={page.insurance.checklist} columns={2} />
              <GuideCrossLink
                href={HEALTH_INSURANCE_PATH}
                title="Health Insurance in the Netherlands"
                description="Compare basic and supplementary insurance policies and understand the annual deductible (eigen risico)."
            ...
            <PremiumGuideSection
              id="check-ups"
              tipsKey="checkUps"
              visual={page.visuals.checkUps}
              intro={
                <SectionIntro eyebrow="Check-ups" title={page.checkUps.heading} fullWidth>
                  {page.checkUps.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <BulletPanel title="What a check-up covers" items={page.checkUps.points} />
              <ChecklistBlock title="Prepare before you go" items={page.checkUps.prepareChecklist} columns={2} />
              <BulletPanel title="Visit script — make a short appointment useful" items={page.checkUps.visitScript} />
              <ChecklistBlock title="Leave with these clarified" items={page.checkUps.leaveWith} columns={2} />
              <ScenarioTable title="Common check-up situations" rows={page.checkUps.scenarios} />
              <BulletPanel title="Tips for getting the most from a check-up" items={page.checkUps.tips} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="treatments"
              tipsKey="treatments"
              visual={page.visuals.treatments}
              intro={
                <SectionIntro eyebrow="Treatments" title={page.treatments.heading} fullWidth>
                  <p>{page.treatments.intro}</p>
                  {page.treatments.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.treatments.cards.length)}>
                {page.treatments.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <ScenarioTable title="When treatment is discussed" rows={page.treatments.scenarios} />
              <BulletPanel title="Before agreeing to treatment" items={page.treatments.beforeTreatmentTips} />
              <ChecklistBlock title="After treatment" items={page.treatments.afterTreatmentChecklist} columns={2} />
            </PremiumGuideSection>
            ...
            >
              ...
              <BulletPanel title="Family dental practicalities" items={page.children.points} />
              <ChecklistBlock title="Children's dental setup checklist" items={page.children.checklist} columns={2} />
              <GuideCrossLink
            ...
            <PremiumGuideSection
              id="orthodontics"
              tipsKey="orthodontics"
              visual={page.visuals.orthodontics}
              intro={
                <SectionIntro eyebrow="Orthodontics" title={page.orthodontics.heading} fullWidth>
                  {page.orthodontics.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.orthodontics.pathCards.length)}>
                {page.orthodontics.pathCards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <InfoTable
                columns={coverageColumns}
                rows={page.orthodontics.comparisonRows.map((row) => ({
                  topic: row.topic,
                  children: row.children,
                  adults: row.adults,
                }))}
              />
              <BulletPanel title="Orthodontics essentials" items={page.orthodontics.points} />
              <ScenarioTable title="Common orthodontics situations" rows={page.orthodontics.scenarios} />
              <ChecklistBlock title="Before you start orthodontics" items={page.orthodontics.checklist} columns={2} />
              <BulletPanel title="Practical tips" items={page.orthodontics.tips} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Emergencies" title={page.emergency.heading} fullWidth>
            ...
            >
              <div className={guidePremiumCardGridClass(page.emergency.numbers.length)}>
                {page.emergency.numbers.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              ...
              <InfoTable
                ...
                rows={page.emergency.contrastRows.map((row) => ({
                  route: row.route,
                  when: row.when,
                  how: row.how,
                  note: row.note,
              ...
              />
              <BulletPanel title="Decision shortcuts" items={page.emergency.decisionTips} />
              <ChecklistBlock title="What to say when you call" items={page.emergency.whatToSay} columns={2} />
            ...
            >
              ...
              <div className={guidePremiumCardGridClass(page.costs.exampleCosts.length)}>
                {page.costs.exampleCosts.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="What drives the price" items={page.costs.costFactors} />
              <ChecklistBlock title="Begroting and insurer checklist" items={page.costs.begrotingChecklist} columns={2} />
              <BulletPanel title="Cost tips" items={page.costs.tips} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="preventive"
              tipsKey="preventive"
              visual={page.visuals.preventive}
              intro={
                <SectionIntro eyebrow="Preventive care" title={page.preventive.heading} fullWidth>
                  {page.preventive.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.preventive.roleCards.length)}>
                {page.preventive.roleCards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="Prevention essentials" items={page.preventive.points} />
              <ChecklistBlock title="Daily prevention checklist" items={page.preventive.dailyChecklist} columns={2} />
              <BulletPanel title="Practical tips" items={page.preventive.tips} />
            </PremiumGuideSection>
            ...
            >
              <div className={guidePremiumCardGridClass(page.specialists.roles.length)}>
                {page.specialists.roles.map((role, index) => (
                  <FeatureCard key={role.role} title={role.role} body={role.focus} iconIndex={index} />
                ))}
              </div>
              <ScenarioTable title="When a referral is common" rows={page.specialists.scenarios} />
              <BulletPanel title="Referral practicalities" items={page.specialists.points} />
              <ChecklistBlock title="After the specialist referral" items={page.specialists.afterReferralChecklist} columns={2} />
            </PremiumGuideSection>
            ...
            >
              <div className="grid gap-4 sm:grid-cols-2">
                <BulletPanel title="Advantages of the Dutch system" items={page.expatDifferences.advantages} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Moving checklist" title={page.moving.heading} fullWidth>
                  <p>{page.moving.intro}</p>
                </SectionIntro>
              }
            >
              <BulletPanel title="How to use this checklist" items={page.moving.tips} />
              <ChecklistBlock title="Phase 1 — early research" items={page.moving.early} columns={2} />
              <ChecklistBlock title="Phase 2 — registration" items={page.moving.registration} columns={2} />
              <ChecklistBlock title="Phase 3 — readiness" items={page.moving.readiness} columns={2} />
              <ChecklistBlock title="Full dental setup checklist" items={page.moving.full} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Orientation answers only — confirm practice rules, insurer contracting and your own dental situation with professionals.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related guides for healthcare setup" fullWidth>
                  <p>Connect dental care with GP care, insurance, family healthcare, emergencies and living healthcare basics.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Health hub" title="Explore the healthcare cluster" fullWidth>
                  <p>This page is the dentistry cornerstone — explore related health topics next.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Plan the next step" tone="onDark" fullWidth>
                  <p>
                    Pick the card that matches what is still open — GP care, insurance, children&apos;s care or emergencies —
