...
import { MaternityCareNetherlandsRecommendedServices } from "./MaternityCareNetherlandsRecommendedServices";
import {
  HEALTH_HUB_PATH,
...
const snapshotIcons = [Baby, ShieldCheck, HeartPulse, Languages] as const;
const orientationIcons = [Stethoscope, ClipboardList, WalletCards, Baby] as const;
...
const urgencyGroups = [
  ...
  },
  {
    ...
    title: "Routine — midwife practice daytime contact",
    icon: CalendarCheck,
...
function LinkCard({ item, iconIndex = 0, tone = "default" }: { item: MaternityLink; iconIndex?: number; tone?: "default" | "onDark" }) {
  const Icon = iconPool[iconIndex % iconPool.length];
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four building blocks before your first maternity pathway step</h3>
...
export function MaternityCareNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Maternity care", item: new URL(MATERNITY_CARE_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Maternity file checklist" items={page.safetyFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Six building blocks of Dutch maternity care" fullWidth>
                  <p>
                    These six cards cover almost every maternity orientation question expats ask first — midwife
                    first line, obstetric pathways, birth places, registration timing, insurance and kraamzorg
                    awareness.
                  </p>
                </SectionIntro>
              }
            ...
            >
              <DecisionFlowStrip labels={page.howItWorks.flowLabels} eyebrow="Maternity pathway" />
              <Timeline steps={page.howItWorks.timeline} eyebrow="Stage" />
              <BulletPanel title="Decision tips" items={page.howItWorks.decisionTips} />
              <HowToStepCards steps={page.howItWorks.howToSteps} title="How to use Dutch maternity pathways" />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="midwife"
              tipsKey="midwife"
              visual={page.visuals.midwife}
              intro={
                <SectionIntro eyebrow="Midwife" title={page.midwife.heading} fullWidth>
                  <p>{page.midwife.intro}</p>
                  {page.midwife.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.midwife.cards.length)}>
                {page.midwife.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="Midwife essentials" items={page.midwife.points} />
              <InfoTable
                columns={[
                  { key: "type", label: "Door" },
                ...
                rows={page.midwife.contrastRows.map((row) => ({
                  type: row.type,
                  focus: row.focus,
              ...
              />
              <ChecklistBlock title="Midwife checklist" items={page.midwife.checklist} columns={2} />
              <ScenarioTable title="Midwife first line in practice" rows={page.midwife.scenarios} />
              <BulletPanel title="Tips" items={page.midwife.tips} />
              <GuideCrossLink
                href={page.midwife.crossLink.href}
                title={page.midwife.crossLink.label}
                description={page.midwife.crossLink.description}
            ...
            <PremiumGuideSection
              id="obstetric-pathways"
              tipsKey="obstetricPathways"
              visual={page.visuals.obstetricPathways}
              intro={
                <SectionIntro eyebrow="Obstetric pathways" title={page.obstetricPathways.heading} fullWidth>
                  <p>{page.obstetricPathways.intro}</p>
                  {page.obstetricPathways.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.obstetricPathways.cards.length)}>
                {page.obstetricPathways.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="Pathway essentials" items={page.obstetricPathways.points} />
              <InfoTable
                columns={contactRouteColumns}
                rows={page.obstetricPathways.contrastRows.map((row) => ({
                  route: row.route,
                  when: row.when,
                  how: row.how,
                  note: row.note,
              ...
              />
              <ChecklistBlock title="Obstetric pathway checklist" items={page.obstetricPathways.checklist} columns={2} />
              <ScenarioTable title="Obstetric pathways in practice" rows={page.obstetricPathways.scenarios} />
              <BulletPanel title="Tips" items={page.obstetricPathways.tips} />
              <GuideCrossLink
                href={page.obstetricPathways.crossLink.href}
                title={page.obstetricPathways.crossLink.label}
                description={page.obstetricPathways.crossLink.description}
            ...
            <PremiumGuideSection
              id="birth-places"
              tipsKey="birthPlaces"
              visual={page.visuals.birthPlaces}
              intro={
                <SectionIntro eyebrow="Birth places" title={page.birthPlaces.heading} fullWidth>
                  <p>{page.birthPlaces.intro}</p>
                  {page.birthPlaces.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.birthPlaces.cards.length)}>
                {page.birthPlaces.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <DecisionFlowStrip labels={page.birthPlaces.timeline.map((step) => step.title)} eyebrow="Birth-place conversation stages" />
              <Timeline steps={page.birthPlaces.timeline} eyebrow="Stage" />
              <UrgencyBoard rows={page.birthPlaces.urgencyRows} />
              <BulletPanel title="Birth-place essentials" items={page.birthPlaces.points} />
              <ChecklistBlock title="Birth-place checklist" items={page.birthPlaces.checklist} columns={2} />
              <ScenarioTable title="Birth places in practice" rows={page.birthPlaces.scenarios} />
              <BulletPanel title="Tips" items={page.birthPlaces.tips} />
              <GuideCrossLink
                href={page.birthPlaces.crossLink.href}
                title={page.birthPlaces.crossLink.label}
                description={page.birthPlaces.crossLink.description}
                linkLabel="Open emergency healthcare"
                icon={Siren}
              />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Registration" title={page.registration.heading} fullWidth>
            ...
            >
              <div className={guidePremiumCardGridClass(page.registration.cards.length)}>
                {page.registration.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="Registration essentials" items={page.registration.points} />
              <InfoTable
                columns={contactRouteColumns}
                rows={page.registration.contrastRows.map((row) => ({
                  route: row.route,
                  when: row.when,
                  how: row.how,
                  note: row.note,
              ...
              />
              <ChecklistBlock title="Registration checklist" items={page.registration.checklist} columns={2} />
              <ScenarioTable title="Registration in practice" rows={page.registration.scenarios} />
              <BulletPanel title="Tips" items={page.registration.tips} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Costs" title={page.insurance.heading} fullWidth>
            ...
            <section id="recommended" className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <MaternityCareNetherlandsRecommendedServices />
              </div>
            </section>
            ...
            <PremiumGuideSection
              id="kraamzorg"
              tipsKey="kraamzorg"
              visual={page.visuals.kraamzorg}
              intro={
                <SectionIntro eyebrow="Kraamzorg" title={page.kraamzorg.heading} fullWidth>
                  <p>{page.kraamzorg.intro}</p>
                  {page.kraamzorg.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.kraamzorg.cards.length)}>
                {page.kraamzorg.cards.map((card, index) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={index} />
                ))}
              </div>
              <BulletPanel title="Kraamzorg essentials" items={page.kraamzorg.points} />
              <ChecklistBlock title="Kraamzorg awareness checklist" items={page.kraamzorg.checklist} columns={2} />
              <ScenarioTable title="Kraamzorg awareness in practice" rows={page.kraamzorg.scenarios} />
              <BulletPanel title="Tips" items={page.kraamzorg.tips} />
              <GuideCrossLink
                href={page.kraamzorg.crossLink.href}
                title={page.kraamzorg.crossLink.label}
                description={page.kraamzorg.crossLink.description}
                linkLabel="Open healthcare for children"
                icon={Baby}
              />
            </PremiumGuideSection>
            ...
            >
              ...
              <ScenarioTable title="Finding maternity care in practice" rows={page.finding.scenarios} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related guides for maternity care" fullWidth>
                  <p>
                    Maternity care connects to GP care, hospitals, insurance and children&apos;s healthcare —
                    read them together.
                  </p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Health hub" title="Explore the healthcare cluster" fullWidth>
                  <p>This page is the maternity care cornerstone — explore related health topics next.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Plan the next step" tone="onDark" fullWidth>
                  <p>
                    Pick the card that matches what is still open — children&apos;s healthcare after birth, GP
                    registration, insurance, hospital pathways or emergency routes — and verify specifics on the
