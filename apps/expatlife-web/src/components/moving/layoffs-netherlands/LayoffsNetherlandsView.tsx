...
import {
  ...
  moveMisunderstandingCardLinkClass,
...
import type { ChangingJobsNlPracticalLifeSection } from "@/src/components/moving/changing-jobs-netherlands/config/moveChangingJobsNl.types";
import { layoffsNlPageMeta, moveLayoffsNlRoutes, workingInNl } from "./config/layoffsNetherlands.config";
import { LayoffsNetherlandsHeroGraphic } from "./LayoffsNetherlandsHeroGraphic";
import {
  LayoffsFourLensesInfographic,
  LayoffsReassurancePrioritiesGraphic,
  LayoffsThreePhasesRail,
...
const PHASE_HINT_CHIP =
  "inline-flex rounded-full border border-amber-200/80 bg-amber-50/90 px-2 py-0.5 text-[9px] font-bold uppercase tracking-[0.12em] text-amber-950/90";
const PHASE_HINT_CHIP_SECOND =
...
const WORKING_NL = workingInNl;
const ROUTES = moveLayoffsNlRoutes;

const LAYOFFS_SUPPORT_LINKS = [
...
export function LayoffsNetherlandsView() {
  ...
  const reassuranceFooter = meta.reassuranceFooter;
          ...
          <PillarGuideHeroRegion>
            <PageHero
              ...
              heroMediaSlot={
                <div
                  className={cn(
                    "grid gap-4 sm:gap-8 md:grid-cols-[minmax(0,1fr)_minmax(0,min(100%,380px))] md:items-start md:gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,420px)]",
                    sitePillarFramedHeroGutterXClass
                  )}
                >
                  <div className="min-w-0">
                    ...
                    <div className="mt-4 space-y-2 text-xs leading-relaxed text-foreground-muted sm:space-y-2.5 sm:text-sm">
                      <p>
                        ...
                        {" · "}
                        <Link href={ROUTES.changingJobs} className="font-semibold text-link hover:text-link-hover hover:underline">
                          Changing jobs
                        </Link>
                        {" · "}
                      ...
                      <p>
                        ...
                        <Link href={ROUTES.contractScanner} className="font-semibold text-link hover:text-link-hover hover:underline">
                          Contract scanner
                        </Link>
                        {" · "}
                      ...
                      <p>
                        <span className="font-medium text-foreground">Planners & living setup: </span>
                        <Link href={ROUTES.first90Days} className="font-semibold text-link hover:text-link-hover hover:underline">
                          First 90 days
                        </Link>
                        {" · "}
                        <Link href={ROUTES.arrivalPlanner} className="font-semibold text-link hover:text-link-hover hover:underline">
                          Arrival planner
                        </Link>
                        {" · "}
                        <Link href={ROUTES.movingChecklist} className="font-semibold text-link hover:text-link-hover hover:underline">
                          Moving checklist
                        </Link>
                        {" · "}
                        <Link href={ROUTES.documentReadiness} className="font-semibold text-link hover:text-link-hover hover:underline">
                          Document readiness
                        </Link>
                        {" · "}
                        <Link href={ROUTES.afterArriving} className="font-semibold text-link hover:text-link-hover hover:underline">
                          After arriving
                        </Link>
                        {" · "}
                        <Link href={ROUTES.municipalityRegistration} className="font-semibold text-link hover:text-link-hover hover:underline">
                          Municipality registration
                        </Link>
                        {" · "}
                        <Link href={ROUTES.healthcareBasics} className="font-semibold text-link hover:text-link-hover hover:underline">
                          Healthcare basics
                        </Link>
                      </p>
                    </div>
                  </div>
                  <LayoffsNetherlandsHeroGraphic className="w-full min-w-0 shrink-0 max-md:mx-auto max-md:max-w-md md:justify-self-end md:pt-1" />
                </div>
              }
              shareUrl={shareUrl}
              pageId={CANONICAL}
          ...
          <PillarGuideAtGlanceRegion>
            ...
            >
              ...
              <p className={cn(movingNlSectionSubtitleClass, "mt-2 text-copilot-text-secondary")}>
                <BoldInline text={meta.atAGlance.subtitle} className="[&_strong]:font-semibold [&_strong]:text-copilot-text-primary" />
              </p>
        ...
        keySections={
          <PillarJourneyStack variant="guide" density="compact" className="gap-1 sm:gap-2 md:gap-3">
            ...
            <section
              ...
              aria-labelledby="laynl-reassurance-heading"
            >
              <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,260px)] lg:items-start lg:gap-6">
                <div className="flex gap-3 sm:gap-4">
                  ...
                  <div className="min-w-0">
                    <p id="laynl-reassurance-heading" className="text-sm font-semibold text-foreground">
                      <BoldInline text={reassurance.title} className="[&_strong]:font-semibold [&_strong]:text-foreground" />
                    </p>
                    <BoldParagraph
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <span className={PHASE_HINT_CHIP}>Clarify early</span>
                      <span className={PHASE_HINT_CHIP_SECOND}>Confirm in writing</span>
                      <span className={PHASE_HINT_CHIP_SECOND}>One lane per week</span>
                    </div>
                    {reassuranceFooter ? (
                      <BoldParagraph
                        text={reassuranceFooter}
                        className="mt-3 text-xs font-medium leading-relaxed text-foreground-muted/90 [&_strong]:font-semibold [&_strong]:text-foreground"
                      />
                    ) : null}
                ...
                <LayoffsReassurancePrioritiesGraphic className="lg:justify-self-end" />
              </div>
              {meta.practicalTipCallouts && meta.practicalTipCallouts.length > 0 ? (
                <div className="mt-5 space-y-3 border-t border-brand/10 pt-4">
                  <p className={cn(SECTION_CARD_KICKER, "text-copilot-primary")}>Practical tips</p>
                  {meta.practicalTipCallouts.map((tip) => (
                    <div
                      key={tip.id}
                      className="rounded-xl border border-copilot-primary/[0.08] bg-white/80 p-3 shadow-sm ring-1 ring-border/10 sm:p-4"
                    >
                      <p className="text-sm font-semibold text-foreground">
                        <BoldInline text={tip.title} className="[&_strong]:font-semibold [&_strong]:text-foreground" />
                      </p>
                      <BoldParagraph
                        text={tip.intro}
                        className="mt-1.5 text-[13px] leading-snug text-foreground-muted sm:text-sm [&_strong]:font-semibold [&_strong]:text-foreground"
                      />
                      {tip.bestFor ? (
                        <BoldParagraph
                      ...
                      {tip.keyPoints?.length ? (
                        <ul className="mt-2 space-y-1 text-[12px] leading-snug text-foreground-muted" role="list">
                          {tip.keyPoints.map((pt) => (
                            <li key={pt} className="flex gap-2">
                              <span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-brand/70" aria-hidden />
                              <BoldParagraph text={pt} className="min-w-0 [&_strong]:font-semibold [&_strong]:text-foreground" />
                            </li>
                          ))}
                        </ul>
                      ) : null}
                      <MovePillarInternalLinkList links={tip.internalLinks} className="mt-2" />
                    </div>
                  ))}
                </div>
              ) : null}
            ...
            <div
              className="flex flex-col gap-4 rounded-xl border border-dashed border-brand/20 bg-surface-raised/70 px-4 py-4 shadow-sm ring-1 ring-border/10 sm:px-5 md:flex-row md:items-center md:gap-6"
              role="note"
              aria-label="How to use the three start-here phases"
            >
              <LayoffsThreePhasesRail className="mx-auto md:mx-0" />
              <div className="min-w-0 flex-1 text-center md:text-left">
                <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-foreground-muted">How to read the phases</p>
                <p className="mt-2 text-sm leading-snug text-foreground-muted">
                  <span className="font-semibold text-foreground">Early signal</span> — gather questions and save copies.{" "}
                  <span className="font-semibold text-foreground">Role at risk</span> — pin down dates and who owns each topic.{" "}
                  <span className="font-semibold text-foreground">Confirmed</span> — work through tasks with real deadlines.
                </p>
              </div>
            </div>
            ...
            <JourneySectionFour section={meta.whatAffectsSection} leadVisual={<LayoffsFourLensesInfographic className="mb-1 sm:mb-2" />} />
            ...
            {meta.employeeRightsSection ? <CardGridSection section={meta.employeeRightsSection} /> : null}

            <SectionDivider />

            {meta.expatBenefitsSection ? <CardGridSection section={meta.expatBenefitsSection} /> : null}

            <SectionDivider />

            {meta.expatWatchOutsSection ? <CardGridSection section={meta.expatWatchOutsSection} /> : null}

            <SectionDivider />

            {meta.expatActionsSection ? <CardGridSection section={meta.expatActionsSection} /> : null}

            <SectionDivider />

            <section
              id="layoffs-support-providers"
              className={SECTION_SCROLL_MARGIN}
              aria-label="Professional support and provider comparisons"
            >
              <MoveGuideAffiliateSupportBlock
                placementId="nl-moving-layoffs-support-providers"
                categoryLinks={[...LAYOFFS_SUPPORT_LINKS]}
        ...
        tools={
          ...
          >
            <div className="space-y-6 sm:space-y-7">
              <div className="grid gap-4 rounded-xl bg-copilot-bg-soft/50 px-4 py-4 ring-1 ring-copilot-primary/[0.08] sm:grid-cols-[1fr_minmax(0,200px)] sm:items-center sm:gap-5 sm:px-5">
                <BoldParagraph
                  text={meta.relatedTools.journeyIntro}
                  className="min-w-0 text-sm leading-relaxed text-copilot-text-secondary [&_strong]:font-semibold [&_strong]:text-copilot-text-primary"
                />
                <LayoffsToolTracksInfographic className="justify-self-center sm:justify-self-end" />
              </div>
              <SectionBlock
                compact
                className="!pt-0"
...
function SectionDivider() {
  return (
    <div
      className="mx-auto my-3 h-px w-full max-w-3xl bg-border/45 sm:my-4 sm:bg-gradient-to-r sm:from-transparent sm:via-border/75 sm:to-transparent"
      aria-hidden
    />
  );
}

function JourneySectionFour({
  section,
  leadVisual,
}: {
  section: MoveWorkingNlJourneySection & { stages: MoveWorkingNlJourneyBlock[] };
  ...
  return (
    <SectionBlock id={section.id} className={TIGHT_SECTION_SPACING} eyebrow={section.eyebrow} title={section.title} subtitle={section.subtitle} subtitleMarkdown>
      ...
      <div className="mt-4 rounded-2xl border border-brand/12 bg-gradient-to-br from-brand-muted/20 via-surface-raised to-copilot-bg-soft/35 p-4 shadow-expatos-sm ring-1 ring-border/10 sm:p-5">
        ...
        <BoldParagraph
          ...
          className="mt-2 text-sm leading-snug text-foreground-muted sm:leading-relaxed [&_strong]:font-semibold [&_strong]:text-foreground"
        />
      </div>
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {section.stages.map((stage) => (
          <MovePillarLifecycleCard key={stage.id} className="h-full border-l-[3px] border-l-brand/25 bg-surface-raised/60 shadow-sm ring-1 ring-border/[0.06]">
            ...
            <ul className="mt-3 space-y-1.5 text-[13px] leading-snug text-foreground-muted" role="list">
              {stage.keyPoints.map((point) => (
                <li key={point} className="flex gap-2">
                  ...
                  <BoldInline text={point} className="min-w-0 [&_strong]:font-semibold [&_strong]:text-foreground" />
                </li>
              ))}
            </ul>
    ...
    <SectionBlock id={section.id} className={TIGHT_SECTION_SPACING} eyebrow={section.eyebrow} title={section.title} subtitle={section.subtitle} subtitleMarkdown>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        {section.cards.map((card) => (
          <article
            key={card.id}
            className="relative overflow-hidden rounded-2xl border border-copilot-primary/[0.08] bg-copilot-surface p-4 shadow-expatos-md ring-1 ring-copilot-primary/[0.06] transition-shadow duration-200 hover:shadow-expatos-lg sm:p-5"
          >
    ...
    <SectionBlock id={region.id} className={TIGHT_SECTION_SPACING} eyebrow={region.eyebrow} title={region.title} subtitle={region.subtitle} subtitleMarkdown>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {region.cards.map((card) => (
          <MovePillarLifecycleCard
            key={card.id}
            className="h-full border-l-[3px] border-l-teal-600/25 bg-surface-raised/50 shadow-sm ring-1 ring-border/[0.06]"
          >
            <div className="flex flex-wrap items-center gap-2">
        ...
        rows={meta.misunderstandings.map((row) => ({
          ...
          body: (
            <div className="space-y-3">
              ...
              {row.internalLinks?.length ? (
                <MovePillarInternalLinkList links={row.internalLinks} linkClassName={moveMisunderstandingCardLinkClass} />
              ) : null}
            </div>
          ),
        }))}
