...
import {
  changingJobsNl,
  resigningJobNlPageMeta,
  moveResigningJobNlRoutes,
  workingInNl,
} from "./config/resigningJobsNetherlands.config";
import { ResigningJobNetherlandsHeroGraphic } from "./ResigningJobNetherlandsHeroGraphic";
...
const RESIGNING_JOB_SUPPORT_LINKS = [
...
export function ResigningJobNetherlandsView() {
  ...
  const crumbs = [
    ...
    { name: "Resigning a Job in the Netherlands", item: new URL(CANONICAL, baseUrl).toString() },
  ];
            ...
            <PageHero
              ...
              heroMediaSlot={
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="mt-4 space-y-2.5 text-sm leading-relaxed text-foreground-muted">
                      <p>
                        ...
                        <Link href={workingInNl} className="font-semibold text-link hover:text-link-hover hover:underline">
                          Working in the Netherlands
                        </Link>
                        {" · "}
                        <Link href={changingJobsNl} className="font-semibold text-link hover:text-link-hover hover:underline">
                          Changing jobs
                        </Link>
                        {" · "}
                      ...
                      <p>
                        <span className="font-medium text-foreground">Contracts & numbers: </span>
                  ...
                  <ResigningJobNetherlandsHeroGraphic className="w-full min-w-0 shrink-0 md:justify-self-end" />
                </div>
              }
              shareUrl={shareUrl}
              pageId={CANONICAL}
        ...
        keySections={
          <PillarJourneyStack variant="guide" density="compact" className="gap-2 sm:gap-3 md:gap-4">
            ...
            <section
              ...
              aria-labelledby="rjnl-reassurance-heading"
            >
              <div className="flex gap-3 sm:gap-4">
                ...
                <div className="min-w-0">
                  <p id="rjnl-reassurance-heading" className="text-sm font-semibold text-foreground">
                  ...
                  <MovePillarInternalLinkList links={reassurance.internalLinks} className="mt-2" />
                  {reassuranceFooter ? (
                    <BoldParagraph
            ...
            <section
              id="resignation-support-providers"
              className={SECTION_SCROLL_MARGIN}
              aria-label="Professional support and provider comparisons"
            >
              <MoveGuideAffiliateSupportBlock
                placementId="nl-moving-resigning-job-support-providers"
                categoryLinks={[...RESIGNING_JOB_SUPPORT_LINKS]}
          ...
          >
            <div className="space-y-6 sm:space-y-7">
              ...
              <MovePillarExploreGrid
                cards={meta.explorePillarCards}
                title="Explore the Move pillar"
                ...
                {meta.continueMove.cards.map((card) => (
                  ...
                  >
                    <p className="text-sm font-semibold text-foreground group-hover:text-brand-strong">
                      <BoldInline text={card.title} className="[&_strong]:font-semibold [&_strong]:text-foreground" />
                    </p>
                    <p className="mt-2 text-sm text-foreground-muted">
                      <BoldInline text={card.description} className="[&_strong]:font-semibold [&_strong]:text-foreground-muted" />
                    </p>
...
/** Re-anchor the three checkpoints + the “full map” section for mobile/desktop scanning. */
function ResigningReadingOrderRibbon() {
  const phases = [
    { href: "#checkpoint-before-notice", short: "1 · Notice", hint: "Before you send" },
    { href: "#checkpoint-before-last-day", short: "2 · Last day", hint: "Before date is fixed" },
    { href: "#checkpoint-final-weeks", short: "3 · Final weeks", hint: "Before access ends" },
  ] as const;
  const pillClass =
    "inline-flex min-h-[40px] flex-1 flex-col items-center justify-center gap-0.5 rounded-xl border border-brand/15 bg-surface-raised/90 px-3 py-2 text-center shadow-sm ring-1 ring-border/10 transition-colors hover:border-brand/30 hover:bg-surface-muted sm:min-h-0 sm:flex-none sm:flex-row sm:gap-2 sm:px-3.5 sm:py-2";
  return (
    <section
      className={cn(
        SECTION_SCROLL_MARGIN,
        "rounded-2xl border border-copilot-primary/[0.08] bg-gradient-to-br from-copilot-bg-soft/80 via-surface-raised to-brand-muted/15 p-4 shadow-expatos-sm ring-1 ring-copilot-primary/[0.05] sm:p-5"
      )}
      aria-label="Reading order: three checkpoints"
    >
      <div className="flex flex-wrap items-end justify-between gap-2">
        <p className={cn(SECTION_CARD_KICKER, "text-copilot-primary")}>Your three clocks</p>
        <Link
          href="#what-resignation-affects"
          className="text-xs font-semibold text-link hover:text-link-hover hover:underline sm:text-sm"
        >
          Full map: what can move →
        </Link>
      </div>
      <p className="mt-1 text-xs leading-relaxed text-foreground-muted sm:text-sm">
        The deep sections below follow the same rhythm: <strong className="font-semibold text-foreground">before notice</strong>,{" "}
        <strong className="font-semibold text-foreground">before the last day is fixed</strong>, then{" "}
        <strong className="font-semibold text-foreground">after payroll stops</strong>.
      </p>
      <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:gap-2.5">
        {phases.map((p) => (
          <Link key={p.href} href={p.href} className={cn(pillClass, transitionInteractive, activeBrightnessPress)}>
            <span className="text-xs font-bold text-foreground sm:text-sm">{p.short}</span>
            <span className="text-[10px] font-medium text-foreground-muted sm:text-xs">{p.hint}</span>
          </Link>
        ))}
      </div>
    </section>
...
function JourneySectionFour({ section }: { section: MoveWorkingNlJourneySection & { stages: MoveWorkingNlJourneyBlock[] } }) {
  return (
    <SectionBlock
      ...
      subtitleMarkdown
      contentClassName="mt-5 sm:mt-6"
    >
      <MovePillarLifecycleCard className="max-w-none shadow-expatos-sm ring-1 ring-border/10">
        <BoldParagraph
          text={section.intro}
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {section.stages.map((stage) => (
          <MovePillarLifecycleCard
            key={stage.id}
            className="h-full border-l-[3px] border-l-brand/25 bg-surface-raised/60 shadow-expatos-sm ring-1 ring-border/[0.06]"
          >
    ...
    >
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        {section.cards.map((card) => (
          <article
            key={card.id}
            className="relative overflow-hidden rounded-2xl border border-copilot-primary/[0.09] bg-copilot-surface p-4 shadow-expatos-md ring-1 ring-copilot-primary/[0.07] sm:p-5"
          >
            ...
            <div className={cn(SECTION_SCAN_BOX, "mt-3")}>
              ...
              <BoldParagraph
                ...
                className="mt-1 text-[13px] leading-snug text-copilot-text-primary sm:text-sm [&_strong]:font-semibold [&_strong]:text-copilot-text-primary"
              />
            </div>
    ...
    >
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        {region.cards.map((card) => (
          <MovePillarLifecycleCard
            key={card.id}
            className="h-full border-l-[3px] border-l-teal-600/25 bg-surface-raised/50 shadow-expatos-sm ring-1 ring-border/[0.06]"
          >
            <div className="flex flex-wrap items-center gap-2">
