...
import type { ExtensionsChangesPageMeta } from "./config";
...
const meta = moveExtensionsChangesPageMeta;
...
export function ExtensionsChangesView() {
  ...
  const crumbs = [
    ...
    { name: "Extensions & Changes", item: new URL(CANONICAL, baseUrl).toString() },
  ];
  ...
  return (
    <>
      ...
      <ArticleJsonLd
        ...
        urlPath={CANONICAL}
      />
      <FaqPageJsonLd items={meta.faq.map((i) => ({ q: i.q, a: i.a.replace(/\*\*/g, "") }))} />
            ...
            <PageHero
              ...
              heroTopBandSlot={
                <nav aria-label="Breadcrumbs" className="flex flex-wrap gap-2 text-xs text-foreground-muted">
                  ...
                  <span className="text-foreground">Extensions &amp; Changes</span>
                </nav>
              }
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-4 text-sm text-foreground-muted">
                      Need the wider route map first? Open{" "}
                    ...
                    <p className="mt-3 text-sm text-foreground-muted">
                      For the <strong className="font-semibold text-foreground">full Move pillar</strong> (stages, scenarios, and the same helpful-tools rhythm), start from{" "}
                      <Link href={HUB} className="font-semibold text-link hover:text-link-hover hover:underline">
                        Moving to the Netherlands
                      </Link>
                      — this page is a sibling guide under that hub.
                    </p>
                    <p className="mt-3 text-sm text-foreground-muted">
                      If you suspect the issue is not only an extension, but a{" "}
                      <strong className="font-semibold text-foreground">change in the basis of your stay</strong>, open{" "}
                      <Link
                      ...
                      for the route-shift guide across work, study, family, and self-employment.
                    </p>
                  </div>
                  <ExtensionsChangesHeroGraphic className="w-full min-w-0 shrink-0 md:justify-self-end" />
                </div>
              }
              shareUrl={shareUrl}
              pageId={CANONICAL}
        ...
        keySections={
          <PillarJourneyStack variant="guide" density="compact" className="gap-1 sm:gap-2 md:gap-3">
            ...
            <section
              ...
              aria-labelledby="ec-reassurance-heading"
            >
              <div className="flex gap-3 sm:gap-4">
                ...
                <div className="min-w-0">
                  <p id="ec-reassurance-heading" className="text-sm font-semibold text-foreground">
                    {reassurance.title}
                  </p>
                  <BoldParagraph
            ...
            <ExtensionsChangesStartHereGrid
            ...
            <ExtensionsChangesSituationGrid region={meta.commonSituationsRegion} cards={meta.situationCards} />

            <SectionDivider />

            <WorkChangesSection section={meta.workSection} />

            <SectionDivider />

            <OtherContextsSection
              id={meta.otherContextsRegion.id}
              eyebrow={meta.otherContextsRegion.eyebrow}
              title={meta.otherContextsRegion.title}
              subtitle={meta.otherContextsRegion.subtitle}
              blocks={meta.otherContextsRegion.blocks}
            />
            ...
            <div className="space-y-6 sm:space-y-7">
              ...
              >
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  {meta.toolsJourneySnapshot.steps.map((s) => (
                    <CardLink key={s.href} href={s.href} title={s.label} description={s.description} meta={s.meta} />
                  ))}
                </div>
              </SectionBlock>
...
function WorkChangesSection({
  section,
}: {
  section: typeof moveExtensionsChangesPageMeta.workSection;
}) {
  return (
    <SectionBlock
      id={section.id}
    ...
    >
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        {section.keyPoints.map((line, i) => (
          <MovePillarLifecycleCard key={line} className="border-l-[3px] border-l-brand/30">
...
function OtherContextsSection({
  ...
  title: string;
  subtitle: string;
  blocks: ExtensionsChangesPageMeta["otherContextsRegion"]["blocks"];
}) {
  return (
    <SectionBlock
      id={id}
      ...
      <div className="grid gap-3 sm:grid-cols-2 sm:gap-4">
        {blocks.map((b) => (
          ...
          >
            ...
            {b.intro ? (
              <p className="mt-2 text-[13px] leading-snug text-foreground-muted sm:text-sm">{b.intro}</p>
            ) : null}
            {b.keyPoints?.length ? (
              <ul className="mt-2 space-y-1.5 text-[13px] leading-snug text-foreground-muted sm:text-sm" role="list">
                {b.keyPoints.map((pt) => (
            ...
            <div className="mt-3 space-y-2.5">
              ...
              <div className="rounded-lg border-l-[3px] border-l-brand/25 bg-white/80 px-3 py-2 ring-1 ring-border/60">
                <p className="text-[9px] font-bold uppercase tracking-[0.14em] text-foreground-muted">Next focus</p>
                <BoldParagraph
...
const TIMING_RHYTHM_CHIPS = ["−6 mo", "−3 mo", "Check-in", "Due date"] as const;

function TimingSection({ region }: { region: typeof moveExtensionsChangesPageMeta.timingRegion }) {
  return (
    <SectionBlock
      id={region.id}
      className={TIGHT_SECTION_SPACING}
    ...
    >
      <ul
        ...
        aria-label="Typical planning checkpoints from months before a deadline through the due date"
      >
        {TIMING_RHYTHM_CHIPS.map((label) => (
...
function LifeImpactSection({ region }: { region: typeof moveExtensionsChangesPageMeta.lifeImpactRegion }) {
  return (
    <SectionBlock
      id={region.id}
      className={TIGHT_SECTION_SPACING}
...
function SectionBlockWhatNext() {
  ...
  return (
    <SectionBlock id="what-next" className={TIGHT_SECTION_SPACING} eyebrow={r.eyebrow} title={r.title} subtitle={r.subtitle} subtitleMarkdown>
      <NextSteps
        variant="progression"
        compact
        movingHubPremium
