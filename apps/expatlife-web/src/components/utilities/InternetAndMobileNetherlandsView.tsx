...
import {
  ...
  internetAndMobileNetherlandsPage as page,
...
const iconPool = [Wifi, Router, Smartphone, Signal, Tv, Laptop, GraduationCap, ClipboardList, MapPin] as const;
          ...
          <div>
            <nav aria-label="Breadcrumb" className="mb-6 text-sm text-foreground-muted">
              <ol className="flex flex-wrap items-center gap-x-2 gap-y-1">
                ...
                <li aria-hidden>/</li>
                <li className="font-semibold text-foreground" aria-current="page">Internet and Mobile</li>
              </ol>
            </nav>
...
function SectionNav() {
  return (
    <div className="sticky top-0 z-20 border-y border-slate-200/80 bg-white/90 shadow-sm backdrop-blur-xl">
      <Container>
        <nav aria-label="Internet and mobile page sections" className="flex gap-2 overflow-x-auto py-3">
...
function TipCardGrid({ items, icon: Icon = Wifi }: { items: Array<{ title: string; body: string }>; icon?: typeof Wifi }) {
  return (
...
function TechnologyComparisonTable() {
  return (
      ...
      <div className="overflow-x-auto">
        <table className="min-w-[860px] w-full text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-[0.12em] text-foreground-muted">
            <tr>
              <th scope="col" className="px-4 py-4 font-bold">Technology</th>
              <th scope="col" className="px-4 py-4 font-bold">Speed potential</th>
              <th scope="col" className="px-4 py-4 font-bold">Availability</th>
              <th scope="col" className="px-4 py-4 font-bold">Best suited for</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {page.technologyComparison.map((row) => (
              <tr key={row.technology} className="hover:bg-slate-50/70">
                <th scope="row" className="px-4 py-4 font-bold text-foreground">{row.technology}</th>
                <td className="px-4 py-4 text-foreground-muted">{row.speedPotential}</td>
                <td className="px-4 py-4 text-foreground-muted">{row.availability}</td>
                <td className="px-4 py-4 text-foreground-muted">{row.suitability}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function CostExampleGrid({ items }: { items: typeof page.internetCostExamples }) {
  return (
...
function NumberedSteps({ items }: { items: readonly string[] }) {
  return (
    <div className="grid gap-3 md:grid-cols-2">
      {items.map((item, index) => (
        <div key={item} className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm ring-1 ring-slate-900/[0.03]">
          ...
          <span className="text-sm leading-relaxed text-foreground-muted">{item}</span>
        </div>
      ))}
    </div>
  );
...
function DetailedProviderCard({ provider }: { provider: ConnectivityProvider }) {
  const isInternetProvider = provider.serviceType !== "Mobile";
  const isMobileProvider = provider.serviceType !== "Internet";
    ...
    <article key={`${provider.serviceType}-${provider.name}`} className={cardClass}>
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex shrink-0 flex-wrap justify-end gap-2">
          ...
          {provider.esimSupport ? (
            <span className="rounded-full border border-copilot-primary/20 bg-copilot-bg-soft px-2.5 py-1 text-xs font-semibold text-brand-strong">eSIM support</span>
          ) : null}
          {provider.businessServices ? (
            <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-semibold text-foreground-muted">Business services</span>
          ) : null}
        </div>
      </div>
      <p className="mt-4 text-sm leading-relaxed text-foreground-muted">{provider.summary}</p>

      {isInternetProvider && provider.technologies?.length ? (
        <div className="mt-4 flex flex-wrap gap-2">
          {provider.technologies.map((type) => (
      ...
      {isInternetProvider ? (
        <div className="mt-4 grid gap-2 text-xs leading-relaxed text-foreground-muted md:grid-cols-2">
          {provider.fibreAvailable ? (
            <p><span className="font-bold text-foreground">Fibre: </span>{provider.fibreAvailable}</p>
          ) : null}
          {provider.residentialServices ? (
            <p><span className="font-bold text-foreground">Residential: </span>{provider.residentialServices}</p>
          ) : null}
          {provider.businessServicesSummary ? (
            <p className="md:col-span-2"><span className="font-bold text-foreground">Business: </span>{provider.businessServicesSummary}</p>
          ) : null}
        </div>
      ) : null}
      ...
      {isMobileProvider ? (
        <div className="mt-4 grid gap-2 text-xs leading-relaxed text-foreground-muted md:grid-cols-2">
          {provider.network ? (
            <p><span className="font-bold text-foreground">Network: </span>{provider.network}</p>
          ) : null}
          <p><span className="font-bold text-foreground">Prepaid: </span>{provider.prepaid ? "Yes" : "No"}</p>
          <p><span className="font-bold text-foreground">SIM-only: </span>{provider.simOnly ? "Yes" : "No"}</p>
          {provider.internationalCalling ? (
            <p className="md:col-span-2"><span className="font-bold text-foreground">International: </span>{provider.internationalCalling}</p>
          ) : null}
        </div>
      ) : null}
...
function ProviderCardGrid({ providers }: { providers: readonly ConnectivityProvider[] }) {
  return (
    <div className="grid gap-5 xl:grid-cols-2">
      {providers.map((provider) => (
        <DetailedProviderCard key={`${provider.serviceType}-${provider.name}`} provider={provider} />
      ))}
    </div>
  );
}

function InternetComparisonTable() {
  return (
    <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="overflow-x-auto">
        <table className="min-w-[900px] w-full text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-[0.12em] text-foreground-muted">
            <tr>
              <th scope="col" className="px-4 py-4 font-bold">Provider</th>
              <th scope="col" className="px-4 py-4 font-bold">Fibre</th>
              <th scope="col" className="px-4 py-4 font-bold">Cable</th>
              <th scope="col" className="px-4 py-4 font-bold">DSL</th>
              <th scope="col" className="px-4 py-4 font-bold">TV Bundles</th>
              <th scope="col" className="px-4 py-4 font-bold">Business</th>
              <th scope="col" className="px-4 py-4 font-bold">eSIM/Mobile</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {page.internetComparisonRows.map((row) => (
              <tr key={row.provider} className="hover:bg-slate-50/70">
                ...
                <td className="px-4 py-4 text-foreground-muted">{row.fibre}</td>
                <td className="px-4 py-4 text-foreground-muted">{row.cable}</td>
                <td className="px-4 py-4 text-foreground-muted">{row.dsl}</td>
                <td className="px-4 py-4 text-foreground-muted">{row.tvBundles}</td>
                <td className="px-4 py-4 text-foreground-muted">{row.businessServices}</td>
                <td className="px-4 py-4 text-foreground-muted">{row.esimMobile}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <table className="min-w-[760px] w-full text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-[0.12em] text-foreground-muted">
            <tr>
              ...
              <th scope="col" className="px-4 py-4 font-bold">SIM-only</th>
              <th scope="col" className="px-4 py-4 font-bold">Phone contract</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {page.planComparison.map((row) => (
              <tr key={row.aspect} className="hover:bg-slate-50/70">
                <th scope="row" className="px-4 py-4 font-bold text-foreground">{row.aspect}</th>
                <td className="px-4 py-4 text-foreground-muted">{row.simOnly}</td>
                <td className="px-4 py-4 text-foreground-muted">{row.phoneContract}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function ProviderDirectory() {
  return <ProviderCardGrid providers={page.providerDirectory} />;
}

export function InternetAndMobileNetherlandsView() {
    ...
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Internet and Mobile in the Netherlands", item: new URL(page.path, baseUrl).toString() },
        ]}
      />
        ...
        <Container className="py-12 sm:py-16">
          <main className="space-y-8">
            <PremiumGuideSection
              id="intro"
              intro={
                <SectionIntro eyebrow="Connectivity guide" title={page.intro.heading}>
            ...
            >
              ...
              <Link href={page.parentGuidePath} className={cn(cardClass, "block", transitionInteractive, activeBrightnessPress)}>
                ...
                <h3 className="mt-3 text-lg font-black tracking-tight text-foreground">Utilities in the Netherlands</h3>
            ...
            <PremiumGuideSection
              id="internet-types"
              intro={<SectionIntro title="Internet Types in the Netherlands"><p>Home internet technology depends on your address — compare fibre, cable, DSL and wireless options before ordering.</p></SectionIntro>}
              visual={page.visuals.internetTypes}
            >
              <VisualTextDetails details={page.visualTextDetails.internetTypes} />
              <FeatureGrid items={page.internetTypes} />
              <TechnologyComparisonTable />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="internet-providers"
              intro={<SectionIntro title="Major Internet Providers"><p>These are real provider examples for orientation only. Inclusion does not rank or recommend any supplier.</p></SectionIntro>}
              visual={page.visuals.internetProviders}
            >
              <VisualTextDetails details={page.visualTextDetails.internetProviders} />
              <ProviderCardGrid providers={page.internetProviders} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="internet-comparison"
              intro={<SectionIntro title="Internet Provider Comparison"><p>Compare major providers by technology, TV bundles, business services and mobile options. Verify current products at your address.</p></SectionIntro>}
              visual={page.visuals.internetComparison}
            >
              <VisualTextDetails details={page.visualTextDetails.internetComparison} />
              <InternetComparisonTable />
              <TipCardGrid items={page.providerSelectionTips} icon={Router} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="fibre"
              intro={<SectionIntro title={page.fibre.heading}>{page.fibre.paragraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>}
              visual={page.visuals.fibre}
            >
              <VisualTextDetails details={page.visualTextDetails.fibre} />
              <BulletPanel title="Fibre installation checklist" items={page.fibreInstallSteps} />
              <TipCardGrid items={page.fibreCityExamples} icon={MapPin} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="internet-costs"
              intro={<SectionIntro title="Typical Internet and Mobile Costs"><p>These are realistic example ranges only. They are not quotes, guarantees or provider recommendations.</p></SectionIntro>}
              visual={page.visuals.internetCosts}
            >
              <VisualTextDetails details={page.visualTextDetails.internetCosts} />
              <div className="space-y-4">
                <h3 className="text-lg font-black tracking-tight text-foreground">Home internet</h3>
                <CostExampleGrid items={page.internetCostExamples} />
              </div>
              <div className="space-y-4">
                <h3 className="text-lg font-black tracking-tight text-foreground">Mobile plans</h3>
                <CostExampleGrid items={page.mobileCostExamples} />
              </div>
            </PremiumGuideSection>

            <PremiumGuideSection
              id="mobile-providers"
              intro={<SectionIntro title="Major Mobile Providers"><p>Compare national networks, prepaid, SIM-only and eSIM options before signing a mobile contract.</p></SectionIntro>}
              visual={page.visuals.mobileProviders}
            >
              <VisualTextDetails details={page.visualTextDetails.mobileProviders} />
              <ProviderCardGrid providers={page.mobileProviders} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="sim-contracts"
              intro={<SectionIntro title="SIM-Only vs Phone Contracts"><p>SIM-only suits many expats who already own a phone. Phone contracts bundle device costs across the contract term.</p></SectionIntro>}
              visual={page.visuals.simContracts}
            >
              <VisualTextDetails details={page.visualTextDetails.simContracts} />
              <PlanComparisonTable />
              <div className="grid gap-4 lg:grid-cols-3">
                {page.planPicks.map((pick) => (
            ...
            <PremiumGuideSection
              id="prepaid"
              intro={<SectionIntro title={page.prepaid.heading}>{page.prepaid.paragraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>}
              visual={page.visuals.prepaid}
            >
              <VisualTextDetails details={page.visualTextDetails.prepaid} />
              <TipCardGrid items={page.prepaidExamples} icon={Smartphone} />
              <BulletPanel title="Prepaid setup tips" items={page.prepaidTips} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="esim"
              intro={<SectionIntro title={page.esim.heading}>{page.esim.paragraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>}
              visual={page.visuals.esim}
            >
              <VisualTextDetails details={page.visualTextDetails.esim} />
              <FeatureGrid items={page.esimTips} />
              <NumberedSteps items={page.esimSteps} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="coverage"
              intro={<SectionIntro title="Mobile Coverage by City"><p>National coverage is generally strong. Verify indoor signal in your specific home if needed.</p></SectionIntro>}
              visual={page.visuals.coverage}
            >
            ...
            <PremiumGuideSection
              id="remote-work"
              intro={<SectionIntro title={page.remoteWork.heading}>{page.remoteWork.paragraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>}
              visual={page.visuals.remoteWork}
            >
              <VisualTextDetails details={page.visualTextDetails.remoteWork} />
              <TipCardGrid items={page.remoteWorkTips} icon={Laptop} />
              <div className="grid gap-4 md:grid-cols-2">
                <Link href={FREELANCING_NETHERLANDS_PATH} className={cn(cardClass, "block", transitionInteractive, activeBrightnessPress)}>
                  <div className={cn("absolute inset-x-0 top-0 h-1", movingNlSignatureGradientClass)} aria-hidden />
                  <Laptop className="h-5 w-5 text-brand-strong" aria-hidden />
                  <h3 className="mt-3 text-base font-black tracking-tight text-foreground">Freelancing in the Netherlands</h3>
                  <p className="mt-2 text-sm leading-relaxed text-foreground-muted">Connect home internet setup with freelancing, client work and business admin.</p>
                  <span className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-brand-strong">Open guide <ArrowRight className="h-3.5 w-3.5" aria-hidden /></span>
                </Link>
                <Link href={ZZP_NETHERLANDS_PATH} className={cn(cardClass, "block", transitionInteractive, activeBrightnessPress)}>
                  <div className={cn("absolute inset-x-0 top-0 h-1", movingNlSignatureGradientClass)} aria-hidden />
                  <Router className="h-5 w-5 text-brand-strong" aria-hidden />
                  <h3 className="mt-3 text-base font-black tracking-tight text-foreground">ZZP Netherlands</h3>
                  <p className="mt-2 text-sm leading-relaxed text-foreground-muted">Business connectivity and admin for self-employed professionals.</p>
                  <span className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-brand-strong">Open guide <ArrowRight className="h-3.5 w-3.5" aria-hidden /></span>
                </Link>
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="students"
              intro={<SectionIntro title={page.students.heading}>{page.students.paragraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>}
              visual={page.visuals.students}
            >
              <VisualTextDetails details={page.visualTextDetails.students} />
              <TipCardGrid items={page.studentTips} icon={GraduationCap} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="checklist"
              intro={<SectionIntro title="Internet and Mobile Setup Checklist"><p>Use this checklist after confirming your move-in date and address availability.</p></SectionIntro>}
            ...
            <PremiumGuideSection
              id="mistakes"
              intro={<SectionIntro title="Common Connectivity Mistakes"><p>These are the setup errors expats most often make with internet ordering, mobile plans and contract terms.</p></SectionIntro>}
            ...
            <PremiumGuideSection
              id="providers"
              intro={<SectionIntro title="Internet and Mobile Provider Directory"><p>Example prices are orientation ranges only. Verify current tariffs, contract terms and address availability directly with providers.</p></SectionIntro>}
            ...
            <section id="future-guides" className={sectionClass}>
              <SectionIntro eyebrow="Planned expansion" title="Deeper Connectivity Guides">
                <p>These focused child guides are planned as the connectivity cluster expands. Use this page for the full overview until they ship.</p>
              </SectionIntro>
            ...
            <PremiumGuideSection
              id="faq"
              intro={<SectionIntro title="Internet and Mobile FAQ"><p>Use these quick answers for orientation before checking provider terms and postcode availability.</p></SectionIntro>}
            ...
            <PremiumGuideSection
              id="sources"
              intro={<SectionIntro title="Official Resources"><p>Telecom markets, tariffs and regulations change over time. Always verify current information through providers and official sources.</p></SectionIntro>}
            ...
            <PremiumGuideSection
              id="related-guides"
              intro={<SectionIntro title="Related Guides"><p>Continue from internet and mobile into the wider utilities, housing and work guides.</p></SectionIntro>}
