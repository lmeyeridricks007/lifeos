...
import {
  ...
  motApkNetherlandsPage as page,
  type DrivingLink,
  type MistakeCard as MistakeCardData,
} from "./motApkNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for APK</h3>
...
export function MotApkNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "MOT / APK", item: new URL(MOT_APK_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Inspection file checklist" items={page.inspectionFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="MOT / APK at a glance" fullWidth>
                  <p>
                    Six cards cover what APK is, when it is due, how to book, pass/fail, cost bands, and timing after purchase — deeper
                    sections expand each theme without legal advice.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="when-due"
              tipsKey="whenDue"
              visual={page.visuals.whenDue}
              intro={
                <SectionIntro eyebrow="Timing" title={page.whenDue.heading} fullWidth>
                  <p>{page.whenDue.intro}</p>
                  {page.whenDue.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.whenDue.steps.length)}>
                {page.whenDue.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
              ...
              <div className={guidePremiumCardGridClass(page.whenDue.cards.length)}>
                {page.whenDue.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.whenDue.crossLinks.length)}>
                {page.whenDue.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="book"
              tipsKey="book"
              visual={page.visuals.book}
              intro={
                <SectionIntro eyebrow="Book" title={page.book.heading} fullWidth>
                  <p>{page.book.intro}</p>
                  {page.book.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.book.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.book.cards.length)}>
                {page.book.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.book.crossLinks.length)}>
                {page.book.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="pass-fail"
              tipsKey="passFail"
              visual={page.visuals.passFail}
              intro={
                <SectionIntro eyebrow="Results" title={page.passFail.heading} fullWidth>
                  <p>{page.passFail.intro}</p>
                  {page.passFail.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.passFail.steps.length)}>
                {page.passFail.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
                    body={`${step.timing} — ${step.detail}`}
              ...
              <div className={guidePremiumCardGridClass(page.passFail.cards.length)}>
                {page.passFail.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="after-purchase"
              tipsKey="afterPurchase"
              visual={page.visuals.afterPurchase}
              intro={
                <SectionIntro eyebrow="Timing" title={page.afterPurchase.heading} fullWidth>
                  <p>{page.afterPurchase.intro}</p>
                  {page.afterPurchase.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.afterPurchase.steps.length)}>
                {page.afterPurchase.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
                    body={`${step.timing} — ${step.detail}`}
              ...
              <div className={guidePremiumCardGridClass(page.afterPurchase.cards.length)}>
                {page.afterPurchase.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.afterPurchase.crossLinks.length)}>
                {page.afterPurchase.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common APK mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — driving with an expired APK, booking a non-approved garage,
                    or mixing APK with insurance and road tax. Official RDW checks beat forum folklore.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="MOT / APK readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats handling Dutch APK after buying or keeping a car.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about MOT / APK in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and practical guides" fullWidth>
                  <p>
                    Buying a car covers purchase and RDW transfer. Car insurance covers liability. Road tax covers MRB.
                    Licence exchange covers drive rights. Speed cameras, getting around and parking deepen the cluster.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Driving cluster hub" fullWidth>
                  <p>
                    Stay inside the driving cluster — MOT / APK, buying a car, car insurance, road tax, licence exchange,
                    speed cameras, getting around and parking.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>Continue to buying a car, car insurance, road tax, speed cameras, parking permits or getting around.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm due dates and station rules with RDW — these sources are orientation only.</p>
                </SectionIntro>
