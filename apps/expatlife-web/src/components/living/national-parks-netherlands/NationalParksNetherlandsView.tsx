...
import {
  ...
  nationalParksNetherlandsPage as page,
  type LifestyleLink,
  type MistakeCard as MistakeCardData,
} from "./nationalParksNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for National parks</h3>
...
export function NationalParksNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "National parks", item: new URL(NATIONAL_PARKS_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="National parks at a glance" fullWidth>
                  <p>
                    Six cards cover system orientation, landscape types, how to choose, visitor basics, access tips and
                    etiquette — deeper sections expand each theme without ranking parks.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="system"
              tipsKey="system"
              visual={page.visuals.system}
              intro={
                <SectionIntro eyebrow="System" title={page.system.heading} fullWidth>
                  <p>{page.system.intro}</p>
                  {page.system.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                ...
                rows={page.system.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.system.cards.length)}>
                {page.system.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.system.crossLinks.length)}>
                {page.system.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="landscapes"
              tipsKey="landscapes"
              visual={page.visuals.landscapes}
              intro={
                <SectionIntro eyebrow="Landscapes" title={page.landscapes.heading} fullWidth>
                  <p>{page.landscapes.intro}</p>
                  {page.landscapes.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.landscapes.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.landscapes.cards.length)}>
                {page.landscapes.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.landscapes.crossLinks.length)}>
                {page.landscapes.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Choose" title={page.choose.heading} fullWidth>
                  <p>{page.choose.intro}</p>
                  {page.choose.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.choose.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.choose.cards.length)}>
                {page.choose.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.choose.crossLinks.length)}>
                {page.choose.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Visitor" title={page.visitor.heading} fullWidth>
                  <p>{page.visitor.intro}</p>
                  {page.visitor.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.visitor.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.visitor.cards.length)}>
                {page.visitor.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.visitor.crossLinks.length)}>
                {page.visitor.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="access"
              tipsKey="access"
              visual={page.visuals.access}
              intro={
                <SectionIntro eyebrow="Access" title={page.access.heading} fullWidth>
                  <p>{page.access.intro}</p>
                  {page.access.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.access.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.access.cards.length)}>
                {page.access.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.access.crossLinks.length)}>
                {page.access.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={NATIONAL_PARKS_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common national-park mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — expecting wilderness, ignoring etiquette, skipping the access
                    plan, or treating this page as a park ranking. Calm landscape-first planning beats FOMO lists.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="National parks readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats planning a calm Dutch national-park day.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about national parks in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and lifestyle guides" fullWidth>
                  <p>
                    Weekend trips covers destination ideas. Hiking deepens trail culture. Weekend travel covers OV how-to.
                    Cycling and Bike sharing help last mile. Family activities, Weather and Survival Guide support kids,
                    seasons and wider living orientation.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Weekend & lifestyle hub" fullWidth>
                  <p>
                    Stay inside the cluster — Weekend trips, National parks and Hiking — and cross-link to Weekend travel
                    for transport, plus Weather, Family activities and Getting around when useful.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>
                    Continue to Hiking for trail culture, Weekend travel for OV, Weekend trips for destination ideas,
                    Cycling for bike access, Family activities for kids, or Weather for seasons.
                  </p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>
                    Confirm park, tourism and transport details on official tools — these sources are orientation only.
                  </p>
                </SectionIntro>
