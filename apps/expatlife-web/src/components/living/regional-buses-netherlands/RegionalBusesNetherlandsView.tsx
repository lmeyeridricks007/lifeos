...
import {
  ...
  regionalBusesNetherlandsPage as page,
  type TransportLink,
  type MistakeCard as MistakeCardData,
} from "./regionalBusesNetherlandsPageModel";
...
const iconPool = [IdCard, FileCheck2, CreditCard, ClipboardList, Scale, Wallet, MapPin, Timer, ShieldCheck, Bus] as const;
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Regional buses</h3>
...
export function RegionalBusesNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Regional buses", item: new URL(REGIONAL_BUSES_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Regional buses at a glance" fullWidth>
                  <p>
                    Six cards cover networks, operators, tickets, 9292 planning, night buses, and when buses win —
                    deeper sections expand each theme without product advice.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="networks"
              tipsKey="networks"
              visual={page.visuals.networks}
              intro={
                <SectionIntro eyebrow="Networks" title={page.networks.heading} fullWidth>
                  <p>{page.networks.intro}</p>
                  {page.networks.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                ...
                rows={page.networks.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.networks.cards.length)}>
                {page.networks.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.networks.crossLinks.length)}>
                {page.networks.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="operators"
              tipsKey="operators"
              visual={page.visuals.operators}
              intro={
                <SectionIntro eyebrow="Operators" title={page.operators.heading} fullWidth>
                  <p>{page.operators.intro}</p>
                  {page.operators.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.operators.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.operators.cards.length)}>
                {page.operators.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.operators.crossLinks.length)}>
                {page.operators.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="tickets"
              tipsKey="tickets"
              visual={page.visuals.tickets}
              intro={
                <SectionIntro eyebrow="Tickets" title={page.tickets.heading} fullWidth>
                  <p>{page.tickets.intro}</p>
                  {page.tickets.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.tickets.steps.length)}>
                {page.tickets.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
              ...
              <InfoTable
                columns={compareColumns}
                rows={page.tickets.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.tickets.cards.length)}>
                {page.tickets.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.tickets.crossLinks.length)}>
                {page.tickets.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="planning"
              tipsKey="planning"
              visual={page.visuals.planning}
              intro={
                <SectionIntro eyebrow="Planning" title={page.planning.heading} fullWidth>
                  <p>{page.planning.intro}</p>
                  {page.planning.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.planning.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.planning.cards.length)}>
                {page.planning.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.planning.crossLinks.length)}>
                {page.planning.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="night-buses"
              tipsKey="nightBuses"
              visual={page.visuals.nightBuses}
              intro={
                <SectionIntro eyebrow="Night" title={page.nightBuses.heading} fullWidth>
                  <p>{page.nightBuses.intro}</p>
                  {page.nightBuses.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.nightBuses.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.nightBuses.cards.length)}>
                {page.nightBuses.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.nightBuses.crossLinks.length)}>
                {page.nightBuses.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="when-buses-only"
              tipsKey="whenBusesOnly"
              visual={page.visuals.whenBusesOnly}
              intro={
                <SectionIntro eyebrow="Mode choice" title={page.whenBusesOnly.heading} fullWidth>
                  <p>{page.whenBusesOnly.intro}</p>
                  {page.whenBusesOnly.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.whenBusesOnly.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.whenBusesOnly.cards.length)}>
                {page.whenBusesOnly.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.whenBusesOnly.crossLinks.length)}>
                {page.whenBusesOnly.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={REGIONAL_BUSES_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common bus mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — missed checkouts, mixing payment methods, treating buses like
                    tram or metro folklore, or ignoring 9292. Official operator and OVpay terms beat forum screenshots.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Regional buses readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats starting regional and city bus travel calmly in the Netherlands.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about regional buses in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and transport guides" fullWidth>
                  <p>
                    ...
                    travel cards. Trams, Metro and NS trains are mode siblings — this page stays on regional and city buses.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Public transport cluster hub" fullWidth>
                  <p>
                    Stay inside the public transport cluster — Regional buses, Trams, Metro, NS trains, OVpay, OV-chipkaart
                    and Getting around.
                  </p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>Continue to Trams, Metro, OVpay, OV-chipkaart, NS trains, or Getting around when you need them.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm operators, tap paths and live journeys on official tools — these sources are orientation only.</p>
                </SectionIntro>
