...
import {
  ...
  cyclingNetherlandsPage as page,
  type TransportLink,
  type MistakeCard as MistakeCardData,
} from "./cyclingNetherlandsPageModel";
...
const iconPool = [IdCard, FileCheck2, Bike, ClipboardList, Scale, Wallet, MapPin, Timer, ShieldCheck, Lock, CloudRain] as const;
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Cycling</h3>
...
export function CyclingNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Cycling", item: new URL(CYCLING_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Cycling at a glance" fullWidth>
                  <p>
                    Six cards cover buy or borrow, rules, lights and locks, rain and winter, parking and theft, and bike vs OV —
            ...
            <PremiumGuideSection
              id="buying"
              tipsKey="buying"
              visual={page.visuals.buying}
              intro={
                <SectionIntro eyebrow="Access" title={page.buying.heading} fullWidth>
                  <p>{page.buying.intro}</p>
                  {page.buying.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                ...
                rows={page.buying.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.buying.cards.length)}>
                {page.buying.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.buying.crossLinks.length)}>
                {page.buying.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="rules"
              tipsKey="rules"
              visual={page.visuals.rules}
              intro={
                <SectionIntro eyebrow="Etiquette" title={page.rules.heading} fullWidth>
                  <p>{page.rules.intro}</p>
                  {page.rules.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.rules.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.rules.cards.length)}>
                {page.rules.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.rules.crossLinks.length)}>
                {page.rules.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            >
              <div className={guidePremiumCardGridClass(page.gear.steps.length)}>
                {page.gear.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
            ...
            <PremiumGuideSection
              id="weather"
              tipsKey="weather"
              visual={page.visuals.weather}
              intro={
                <SectionIntro eyebrow="Weather" title={page.weather.heading} fullWidth>
                  <p>{page.weather.intro}</p>
                  {page.weather.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.weather.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.weather.cards.length)}>
                {page.weather.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.weather.crossLinks.length)}>
                {page.weather.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Security" title={page.parking.heading} fullWidth>
            ...
            <PremiumGuideSection
              id="bike-vs-ov"
              tipsKey="bikeVsOv"
              visual={page.visuals.bikeVsOv}
              intro={
                <SectionIntro eyebrow="Mode choice" title={page.bikeVsOv.heading} fullWidth>
                  <p>{page.bikeVsOv.intro}</p>
                  {page.bikeVsOv.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.bikeVsOv.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.bikeVsOv.cards.length)}>
                {page.bikeVsOv.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.bikeVsOv.crossLinks.length)}>
                {page.bikeVsOv.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={CYCLING_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common cycling mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — missing lights, weak locks, path etiquette gaps, or forcing the bike
                    in storms. Official traffic rules and calm parking habits beat forum screenshots.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Cycling readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats starting everyday cycling calmly in the Netherlands.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.snapshot}
              intro={
                <SectionIntro eyebrow="Hub" title="Active mobility and public transport hub" fullWidth>
                  <p>
                    Stay inside the cluster — Cycling, Bike sharing, Getting around, OVpay, OV-chipkaart, NS trains,
                    Regional buses, Trams and Metro.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>
                    Continue to Bike sharing, Getting around, OVpay, OV-chipkaart, NS trains, or Regional buses when you
                    need them.
                  </p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>
                    Confirm traffic rules, parking notices and live journeys on official tools — these sources are
                    orientation only.
                  </p>
                </SectionIntro>
