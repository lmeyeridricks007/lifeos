...
import {
  ...
  dutchSupermarketsPage as page,
  type GuideLink,
  type MistakeCard as MistakeCardData,
} from "./dutchSupermarketsPageModel";
...
const chainColumns = [
  ...
  { key: "examples", label: "Examples (orientation)" },
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Dutch supermarket week</h3>
...
function CompareSection({
  ...
  section: {
    ...
    rows: ReadonlyArray<{ topic: string; whatToCheck: string; tip: string }>;
...
function TimelineSection({
  ...
  section: {
    ...
    intro: string;
    paragraphs: readonly string[];
    steps: ReadonlyArray<{ phase: string; timing: string; detail: string }>;
    cards: ReadonlyArray<{ title: string; body: string }>;
    crossLinks: readonly GuideLink[];
  };
}) {
  return (
    ...
    >
      <div className={guidePremiumCardGridClass(section.steps.length)}>
        {section.steps.map((step, idx) => (
          <FeatureCard key={step.phase} title={step.phase} body={`${step.timing} — ${step.detail}`} iconIndex={idx} />
        ))}
      </div>
      <div className={guidePremiumCardGridClass(section.cards.length)}>
        {section.cards.map((card, idx) => (
...
export function DutchSupermarketsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Dutch supermarkets", item: new URL(DUTCH_SUPERMARKETS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Dutch supermarkets at a glance" fullWidth>
                  <p>
                    Six cards cover formats, the chains landscape, opening hours, bags and carts, self-scan and loyalty apps,
                    and produce codes — deeper sections expand each theme without ranking stores.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <CompareSection id="system" tipsKey="system" visual={page.visuals.system} eyebrow="System" section={page.system} />
            <CompareSection id="formats" tipsKey="formats" visual={page.visuals.formats} eyebrow="Formats" section={page.formats} />

            <PremiumGuideSection
              id="chains"
              tipsKey="chains"
              visual={page.visuals.chains}
              intro={
                <SectionIntro eyebrow="Landscape" title={page.chains.heading} fullWidth>
                  <p>{page.chains.intro}</p>
                  {page.chains.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              ...
              />
              <div className={guidePremiumCardGridClass(page.chains.cards.length)}>
                {page.chains.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.chains.crossLinks.length)}>
                {page.chains.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>

            <TimelineSection id="hours" tipsKey="hours" visual={page.visuals.hours} eyebrow="Hours" section={page.hours} />
            <CompareSection id="bags" tipsKey="bags" visual={page.visuals.bags} eyebrow="Bags" section={page.bags} />
            <CompareSection
              id="self-scan"
              tipsKey="selfScan"
              visual={page.visuals.selfScan}
              eyebrow="Self-scan"
              section={page.selfScan}
            />
            <CompareSection id="loyalty" tipsKey="loyalty" visual={page.visuals.loyalty} eyebrow="Apps" section={page.loyalty} />
            <TimelineSection
              id="weekly-rhythm"
              tipsKey="weeklyRhythm"
              visual={page.visuals.weeklyRhythm}
              eyebrow="Rhythm"
              section={page.weeklyRhythm}
            />
            <CompareSection
              id="produce-codes"
              tipsKey="produceCodes"
              visual={page.visuals.produceCodes}
              eyebrow="Produce"
              section={page.produceCodes}
            />

            <PremiumGuideSection
              id="expectations"
              tipsKey="expectations"
              visual={page.visuals.expectations}
              intro={
                <SectionIntro eyebrow="Expectations" title={page.expectations.heading} fullWidth>
                  <p>{page.expectations.intro}</p>
                  {page.expectations.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.expectations.cards.length)}>
                {page.expectations.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.expectations.crossLinks.length)}>
                {page.expectations.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={DUTCH_SUPERMARKETS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common Dutch supermarket mistakes" fullWidth>
                  <p>
                    These patterns create avoidable first-month friction — missing bags, wrong formats, Sunday surprises, or
                    treating this system guide like a ranking.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Supermarket readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats settling a calm supermarket routine in the Netherlands.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about Dutch supermarket chains.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and money guides" fullWidth>
                  <p>
                    Shopping & groceries covers errands and self-checkout how-to. This page stays on the supermarket system.
                    Money guides place food spend in the wider budget picture.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>Continue to Shopping & groceries, apps, payments, or money guides when you need them.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official and retailer sources" tone="onDark" fullWidth>
                  <p>Confirm branch hours, apps and coverage on retailer and consumer sources — orientation only.</p>
                </SectionIntro>
