...
import {
  ...
  trainDiscountsNetherlandsPage as page,
  type TransportLink,
  type MistakeCard as MistakeCardData,
} from "./trainDiscountsNetherlandsPageModel";
...
const iconPool = [IdCard, FileCheck2, TrainFront, ClipboardList, Scale, Wallet, MapPin, Timer, ShieldCheck, Lock, CloudRain] as const;
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Train discounts</h3>
...
export function TrainDiscountsNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Train discounts", item: new URL(TRAIN_DISCOUNTS_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Train discounts at a glance" fullWidth>
                  <p>
                    Six cards cover the product map, off-peak vs weekend, ticket method, break-even, student and age
                    orientation, and mistakes to avoid — deeper sections expand each theme without product rankings.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="chipkaart"
              tipsKey="chipkaart"
              visual={page.visuals.chipkaart}
              intro={
                <SectionIntro eyebrow="Ticket method" title={page.chipkaart.heading} fullWidth>
                  <p>{page.chipkaart.intro}</p>
                  {page.chipkaart.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.chipkaart.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.chipkaart.cards.length)}>
                {page.chipkaart.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.chipkaart.crossLinks.length)}>
                {page.chipkaart.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            >
              ...
              <InfoTable
                columns={costColumns}
                rows={page.decide.costRows.map((row) => ({
            ...
            <PremiumGuideSection
              id="student"
              tipsKey="student"
              visual={page.visuals.student}
              intro={
                <SectionIntro eyebrow="Eligibility" title={page.student.heading} fullWidth>
                  <p>{page.student.intro}</p>
                  {page.student.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.student.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.student.cards.length)}>
                {page.student.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.student.crossLinks.length)}>
                {page.student.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={TRAIN_DISCOUNTS_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common train-discount mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — buying too soon, ignoring peak windows, assuming OVpay carries
                    Dal, or treating indicative prices as quotes. Calm trip history and ticket-method checks beat forum
                    screenshots.
                  </p>
                </SectionIntro>
              }
            ...
            >
              <ChecklistBlock title="Train discounts readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats choosing an NS train discount calmly in the Netherlands.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about train discounts in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and transport guides" fullWidth>
                  <p>
                    NS trains covers how to ride and ticket types. Weekend travel covers leisure days out. OV-chipkaart and
                    OVpay deepen ticket methods. Getting around covers multimodal mobility — this page stays on NS discount
                    product fit.
                  </p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Active mobility and public transport hub" fullWidth>
                  <p>
                    Stay inside the cluster — Train discounts, NS trains, Weekend travel, Getting around, OVpay,
                    OV-chipkaart, Trams, Metro, Regional buses, Cycling and Bike sharing.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>
                    Continue to NS trains, Weekend travel, OV-chipkaart, OVpay, Getting around, or Bike sharing when you
                    need them.
                  </p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>
                    Confirm NS product sheets, peak windows and OV rules on official tools — these sources are orientation
                    only.
                  </p>
                </SectionIntro>
