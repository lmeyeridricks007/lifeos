...
type Props = {
  items: LivingSectionNavItem[];
  /** Optional CTA block below in-page anchors (e.g. deep link into cluster). */
  deepLink?: LivingSectionNavDeepLink;
  /** Multiple cluster CTAs (e.g. Survival Guide + sibling guide)—rendered as stacked cards. */
  deepLinks?: LivingSectionNavDeepLink[];
};
...
export function LivingSectionNav({ items, deepLink, deepLinks }: Props) {
  ...
  return (
    <nav className="space-y-6" aria-labelledby="living-sg-toc-heading">
      <div>
        <p id="living-sg-toc-heading" className="text-[11px] font-bold uppercase tracking-[0.12em] text-foreground-muted">
          On this page
        </p>
      ...
      {clusterLinks.length > 0 ? (
        <div className="space-y-3">
          <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-foreground-muted">Inside the Living cluster</p>
          {clusterLinks.map((dl) => (
            <div
              key={dl.href}
