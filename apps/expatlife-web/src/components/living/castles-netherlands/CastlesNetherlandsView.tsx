...
import {
  ...
  castlesNetherlandsPage as page,
  type LifestyleLink,
  type MistakeCard as MistakeCardData,
} from "./castlesNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Castles</h3>
...
export function CastlesNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Castles", item: new URL(CASTLES_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Castles at a glance" fullWidth>
                  <p>
                    Six cards cover visit culture, booking and seasons, regional patterns, gardens vs interiors, visit
                    styles, and getting there — deeper sections expand each theme without ranking castles or hotels.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="regions"
              tipsKey="regions"
              visual={page.visuals.regions}
              intro={
                <SectionIntro eyebrow="Regions" title={page.regions.heading} fullWidth>
                  <p>{page.regions.intro}</p>
                  {page.regions.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.regions.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.regions.cards.length)}>
                {page.regions.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.regions.crossLinks.length)}>
                {page.regions.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="grounds"
              tipsKey="grounds"
              visual={page.visuals.grounds}
              intro={
                <SectionIntro eyebrow="Gardens" title={page.grounds.heading} fullWidth>
                  <p>{page.grounds.intro}</p>
                  {page.grounds.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.grounds.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.grounds.cards.length)}>
                {page.grounds.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.grounds.crossLinks.length)}>
                {page.grounds.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="visit-styles"
              tipsKey="visitStyles"
              visual={page.visuals.visitStyles}
              intro={
                <SectionIntro eyebrow="Visit styles" title={page.visitStyles.heading} fullWidth>
                  <p>{page.visitStyles.intro}</p>
                  {page.visitStyles.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.visitStyles.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.visitStyles.cards.length)}>
                {page.visitStyles.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.visitStyles.crossLinks.length)}>
                {page.visitStyles.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={CASTLES_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common castle-day mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — ranked FOMO lists, skipped tickets, late peak arrivals, or
                    ignoring return trains. Calm region-and-visit-style planning beats influencer castle checklists.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Castle day readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats planning calm Dutch castle days.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about castles in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and lifestyle guides" fullWidth>
                  <p>
                    Weekend trips covers destination weekends. Weekend travel covers OV how-to. Museums covers galleries
                    and Museumkaart. Hidden gems covers lesser-known non-castle spots. Beach towns, Hiking and National
                    parks are cluster peers. Cycling and Bike sharing help last mile without duplicating Castles in the
                    Cycling menu.
                  </p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>
                    Continue to Weekend trips for destination ideas, Weekend travel for OV, Museums for galleries,
                    Hidden gems for lesser-known places, Beach towns for coast energy, or Hiking for trail weekends.
                  </p>
                </SectionIntro>
              }
            >
