...
import {
  ...
  dayTripsNetherlandsPage as page,
  type LifestyleLink,
  type MistakeCard as MistakeCardData,
} from "./dayTripsNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Day trips</h3>
...
export function DayTripsNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Day trips", item: new URL(DAY_TRIPS_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Day trips at a glance" fullWidth>
                  <p>
                    Six cards cover when a day trip beats overnight, classic day-out patterns, timing and return buffers,
                    packing light, getting there, and Plan B indoor — deeper sections expand each theme without ranking
                    destinations, routes or tour operators.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="when-day-trip"
              tipsKey="whenDayTrip"
              visual={page.visuals.whenDayTrip}
              intro={
                <SectionIntro eyebrow="Decision" title={page.whenDayTrip.heading} fullWidth>
                  <p>{page.whenDayTrip.intro}</p>
                  {page.whenDayTrip.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                ...
                rows={page.whenDayTrip.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.whenDayTrip.cards.length)}>
                {page.whenDayTrip.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.whenDayTrip.crossLinks.length)}>
                {page.whenDayTrip.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="patterns"
              tipsKey="patterns"
              visual={page.visuals.patterns}
              intro={
                <SectionIntro eyebrow="Patterns" title={page.patterns.heading} fullWidth>
                  <p>{page.patterns.intro}</p>
                  {page.patterns.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.patterns.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.patterns.cards.length)}>
                {page.patterns.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.patterns.crossLinks.length)}>
                {page.patterns.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Timing" title={page.timing.heading} fullWidth>
                  <p>{page.timing.intro}</p>
                  {page.timing.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.timing.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.timing.cards.length)}>
                {page.timing.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.timing.crossLinks.length)}>
                {page.timing.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="modes"
              tipsKey="modes"
              visual={page.visuals.modes}
              intro={
                <SectionIntro eyebrow="Modes" title={page.modes.heading} fullWidth>
                  <p>{page.modes.intro}</p>
                  {page.modes.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.modes.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.modes.cards.length)}>
                {page.modes.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.modes.crossLinks.length)}>
                {page.modes.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={DAY_TRIPS_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common day-trip mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — overnight FOMO for nearby hubs, too many stops, missing return
                    windows, heavy bags, ignoring mode reality, or treating ranked destination lists as orientation. Calm
                    same-day planning beats influencer itineraries.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Day trip readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats planning calm Dutch day trips.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about day trips in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and lifestyle guides" fullWidth>
                  <p>
                    Weekend trips covers overnight destination vibes. Road trips covers car multi-stop leisure. Weekend
                    travel covers OV how-to. Beach towns, Castles, National parks, Hiking, Museums and Hidden gems are
                    place-type peers for same-day patterns.
                  </p>
                </SectionIntro>
              }
            >
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Weekend & lifestyle hub" fullWidth>
                  <p>
                    ...
                    Castles, Road trips and Day trips — and cross-link to Weekend travel for transport and Cycling for
                    nearby loops.
                  </p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>
                    Continue to Weekend trips for overnight vibes, Road trips for multi-stop car days, Weekend travel for
                    OV, or Museums, Beach towns, Castles, National parks and Hidden gems for place-type depth.
                  </p>
                </SectionIntro>
              }
            >
