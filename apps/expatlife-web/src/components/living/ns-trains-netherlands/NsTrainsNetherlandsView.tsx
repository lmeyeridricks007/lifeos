...
import {
  ...
  nsTrainsNetherlandsPage as page,
  type TransportLink,
  type MistakeCard as MistakeCardData,
} from "./nsTrainsNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for NS trains</h3>
...
export function NsTrainsNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "NS trains", item: new URL(NS_TRAINS_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="NS trains at a glance" fullWidth>
                  <p>
                    Six cards cover what NS is, ticket types, how to buy, peak hours, first-week tips, and the NS
                    International boundary — deeper sections expand each theme without product advice.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="ticket-types"
              tipsKey="ticketTypes"
              visual={page.visuals.ticketTypes}
              intro={
                <SectionIntro eyebrow="Tickets" title={page.ticketTypes.heading} fullWidth>
                  <p>{page.ticketTypes.intro}</p>
                  {page.ticketTypes.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.ticketTypes.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.ticketTypes.cards.length)}>
                {page.ticketTypes.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.ticketTypes.crossLinks.length)}>
                {page.ticketTypes.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="how-to-buy"
              tipsKey="howToBuy"
              visual={page.visuals.howToBuy}
              intro={
                <SectionIntro eyebrow="Buy & travel" title={page.howToBuy.heading} fullWidth>
                  <p>{page.howToBuy.intro}</p>
                  {page.howToBuy.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.howToBuy.steps.length)}>
                {page.howToBuy.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
              ...
              <InfoTable
                columns={compareColumns}
                rows={page.howToBuy.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.howToBuy.cards.length)}>
                {page.howToBuy.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.howToBuy.crossLinks.length)}>
                {page.howToBuy.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="peak-hours"
              tipsKey="peakHours"
              visual={page.visuals.peakHours}
              intro={
                <SectionIntro eyebrow="Peak" title={page.peakHours.heading} fullWidth>
                  <p>{page.peakHours.intro}</p>
                  {page.peakHours.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.peakHours.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.peakHours.cards.length)}>
                {page.peakHours.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.peakHours.crossLinks.length)}>
                {page.peakHours.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="first-week"
              tipsKey="firstWeek"
              visual={page.visuals.firstWeek}
              intro={
                <SectionIntro eyebrow="Commute" title={page.firstWeek.heading} fullWidth>
                  <p>{page.firstWeek.intro}</p>
                  {page.firstWeek.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.firstWeek.steps.length)}>
                {page.firstWeek.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
                    body={`${step.timing} — ${step.detail}`}
              ...
              <div className={guidePremiumCardGridClass(page.firstWeek.cards.length)}>
                {page.firstWeek.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.firstWeek.crossLinks.length)}>
                {page.firstWeek.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="ns-international"
              tipsKey="nsInternational"
              visual={page.visuals.nsInternational}
              intro={
                <SectionIntro eyebrow="Boundary" title={page.nsInternational.heading} fullWidth>
                  <p>{page.nsInternational.intro}</p>
                  {page.nsInternational.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.nsInternational.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.nsInternational.cards.length)}>
                {page.nsInternational.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.nsInternational.crossLinks.length)}>
                {page.nsInternational.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={NS_TRAINS_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common NS trains mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — buying season tickets too early, missed checkouts, confusing
                    metro with NS, or trusting outdated peak folklore. Official NS terms beat forum screenshots.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="NS trains readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats starting NS train travel calmly in the Netherlands.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about NS trains in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and transport guides" fullWidth>
                  <p>
                    Getting around covers multimodal mobility. OVpay covers contactless taps. OV-chipkaart covers personal
                    travel cards — this page stays on NS train travel for expats.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Public transport cluster hub" fullWidth>
                  <p>
                    Stay inside the public transport cluster — NS trains, OVpay, OV-chipkaart and Getting around — with light
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>Continue to OVpay, OV-chipkaart, Getting around, or light driving options when you need them.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm tickets, peak rules and tap paths on official tools — these sources are orientation only.</p>
                </SectionIntro>
