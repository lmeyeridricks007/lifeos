...
import {
  ...
  metroNetherlandsPage as page,
  type TransportLink,
  type MistakeCard as MistakeCardData,
} from "./metroNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Metro</h3>
...
export function MetroNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Metro", item: new URL(METRO_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Metro at a glance" fullWidth>
                  <p>
                    Six cards cover networks, tickets, peak tips, NS interchange, when metro wins, and sibling
                    modes — deeper sections expand each theme without product advice.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="peak-tips"
              tipsKey="peakTips"
              visual={page.visuals.peakTips}
              intro={
                <SectionIntro eyebrow="Peak" title={page.peakTips.heading} fullWidth>
                  <p>{page.peakTips.intro}</p>
                  {page.peakTips.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.peakTips.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.peakTips.cards.length)}>
                {page.peakTips.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.peakTips.crossLinks.length)}>
                {page.peakTips.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="ns-interchange"
              tipsKey="nsInterchange"
              visual={page.visuals.nsInterchange}
              intro={
                <SectionIntro eyebrow="Transfers" title={page.nsInterchange.heading} fullWidth>
                  <p>{page.nsInterchange.intro}</p>
                  {page.nsInterchange.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.nsInterchange.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.nsInterchange.cards.length)}>
                {page.nsInterchange.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.nsInterchange.crossLinks.length)}>
                {page.nsInterchange.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="when-metro-wins"
              tipsKey="whenMetroWins"
              visual={page.visuals.whenMetroWins}
              intro={
                <SectionIntro eyebrow="Mode choice" title={page.whenMetroWins.heading} fullWidth>
                  <p>{page.whenMetroWins.intro}</p>
                  {page.whenMetroWins.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.whenMetroWins.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.whenMetroWins.cards.length)}>
                {page.whenMetroWins.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.whenMetroWins.crossLinks.length)}>
                {page.whenMetroWins.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={METRO_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common metro mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — missed checkouts, mixing payment methods, treating metro as
                    NS trains, or rushing peak doors. Official operator and OVpay terms beat forum screenshots.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Metro readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats starting metro travel calmly in Amsterdam or Rotterdam.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about Metro in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and transport guides" fullWidth>
                  <p>
                    ...
                    travel cards. Trams, Regional buses and NS trains are mode siblings — this page stays on metro / rapid
                    transit.
                  </p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Public transport cluster hub" fullWidth>
                  <p>
                    Stay inside the public transport cluster — Metro, Trams, Regional buses, NS trains, OVpay, OV-chipkaart
                    and Getting around.
                  </p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>Continue to Trams, Regional buses, OVpay, OV-chipkaart, NS trains, or Getting around when you need them.</p>
                </SectionIntro>
              }
            >
