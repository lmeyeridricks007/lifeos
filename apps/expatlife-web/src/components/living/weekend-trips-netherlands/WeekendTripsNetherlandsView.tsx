...
import {
  ...
  weekendTripsNetherlandsPage as page,
  type LifestyleLink,
  type MistakeCard as MistakeCardData,
} from "./weekendTripsNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Weekend trips</h3>
...
export function WeekendTripsNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Weekend trips", item: new URL(WEEKEND_TRIPS_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Weekend trips at a glance" fullWidth>
                  <p>
                    Six cards cover vibe-first planning, destination patterns, when to go, the planning flow, nature
                    short-orientation, and packing culture — deeper sections expand each theme without destination rankings.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="destinations"
              tipsKey="destinations"
              visual={page.visuals.destinations}
              intro={
                <SectionIntro eyebrow="Destinations" title={page.destinations.heading} fullWidth>
                  <p>{page.destinations.intro}</p>
                  {page.destinations.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.destinations.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.destinations.cards.length)}>
                {page.destinations.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.destinations.crossLinks.length)}>
                {page.destinations.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="When to go" title={page.seasons.heading} fullWidth>
            ...
            <PremiumGuideSection
              id="nature"
              tipsKey="nature"
              visual={page.visuals.nature}
              intro={
                <SectionIntro eyebrow="Nature" title={page.nature.heading} fullWidth>
                  <p>{page.nature.intro}</p>
                  {page.nature.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.nature.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.nature.cards.length)}>
                {page.nature.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.nature.crossLinks.length)}>
                {page.nature.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={WEEKEND_TRIPS_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common weekend-trip mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — marathon itineraries, ignoring weather, skipping the transport
                    sibling, or treating this page as a hotel ranking. Calm vibe-first planning beats FOMO lists.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Weekend trips readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats planning a calm Dutch weekend trip.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about weekend trips in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and lifestyle guides" fullWidth>
                  <p>
                    Weekend travel covers OV how-to. National parks and Hiking deepen nature weekends. Family activities
                    helps with kids. Weather and Survival Guide support seasons and wider living orientation.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>
                    Continue to Weekend travel for OV, National parks or Hiking for nature depth, Family activities for
                    kids, Weather for seasons, or Cycling for local bike days.
                  </p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>
                    Confirm tourism, park and transport details on official tools — these sources are orientation only.
                  </p>
                </SectionIntro>
