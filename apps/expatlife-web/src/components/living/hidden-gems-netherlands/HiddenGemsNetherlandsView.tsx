...
import {
  ...
  hiddenGemsNetherlandsPage as page,
  type LifestyleLink,
  type MistakeCard as MistakeCardData,
} from "./hiddenGemsNetherlandsPageModel";
...
const iconPool = [IdCard, FileCheck2, Trees, ClipboardList, Scale, Wallet, MapPin, Timer, ShieldCheck, Lock, CloudRain] as const;
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Hidden gems</h3>
...
export function HiddenGemsNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Hidden gems", item: new URL(HIDDEN_GEMS_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Travel file checklist" items={page.travelFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Hidden gems at a glance" fullWidth>
                  <p>
                    Six cards cover meaning, neighbourhoods, small towns, nature edges, museums-lane cues and etiquette —
                    deeper sections expand each theme without ranking secret spots or attractions.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="meaning"
              tipsKey="meaning"
              visual={page.visuals.meaning}
              intro={
                <SectionIntro eyebrow="Meaning" title={page.meaning.heading} fullWidth>
                  <p>{page.meaning.intro}</p>
                  {page.meaning.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                ...
                rows={page.meaning.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.meaning.cards.length)}>
                {page.meaning.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.meaning.crossLinks.length)}>
                {page.meaning.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="neighbourhood"
              tipsKey="neighbourhood"
              visual={page.visuals.neighbourhood}
              intro={
                <SectionIntro eyebrow="Neighbourhoods" title={page.neighbourhood.heading} fullWidth>
                  <p>{page.neighbourhood.intro}</p>
                  {page.neighbourhood.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.neighbourhood.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.neighbourhood.cards.length)}>
                {page.neighbourhood.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.neighbourhood.crossLinks.length)}>
                {page.neighbourhood.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="small-towns"
              tipsKey="smallTowns"
              visual={page.visuals.smallTowns}
              intro={
                <SectionIntro eyebrow="Small towns" title={page.smallTowns.heading} fullWidth>
                  <p>{page.smallTowns.intro}</p>
                  {page.smallTowns.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.smallTowns.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.smallTowns.cards.length)}>
                {page.smallTowns.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.smallTowns.crossLinks.length)}>
                {page.smallTowns.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="nature-edges"
              tipsKey="natureEdges"
              visual={page.visuals.natureEdges}
              intro={
                <SectionIntro eyebrow="Nature edges" title={page.natureEdges.heading} fullWidth>
                  <p>{page.natureEdges.intro}</p>
                  {page.natureEdges.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.natureEdges.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.natureEdges.cards.length)}>
                {page.natureEdges.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.natureEdges.crossLinks.length)}>
                {page.natureEdges.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="museums-lane"
              tipsKey="museumsLane"
              visual={page.visuals.museumsLane}
              intro={
                <SectionIntro eyebrow="Museums lane" title={page.museumsLane.heading} fullWidth>
                  <p>{page.museumsLane.intro}</p>
                  {page.museumsLane.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.museumsLane.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.museumsLane.cards.length)}>
                {page.museumsLane.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.museumsLane.crossLinks.length)}>
                {page.museumsLane.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="getting-there"
              tipsKey="gettingThere"
              visual={page.visuals.gettingThere}
              intro={
                <SectionIntro eyebrow="Getting there" title={page.gettingThere.heading} fullWidth>
                  <p>{page.gettingThere.intro}</p>
                  {page.gettingThere.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.gettingThere.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.gettingThere.cards.length)}>
                {page.gettingThere.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.gettingThere.crossLinks.length)}>
                {page.gettingThere.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="etiquette"
              tipsKey="etiquette"
              visual={page.visuals.etiquette}
              intro={
                <SectionIntro eyebrow="Etiquette" title={page.etiquette.heading} fullWidth>
                  <p>{page.etiquette.intro}</p>
                  {page.etiquette.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.etiquette.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.etiquette.cards.length)}>
                {page.etiquette.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.etiquette.crossLinks.length)}>
                {page.etiquette.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={HIDDEN_GEMS_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common hidden-gems mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — ranked secret lists, overcrowding viral tips, ignoring
                    residents, or opening the wrong sibling lane. Calm patterns beat influencer checklists.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Quiet-day readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats exploring lesser-known places calmly in the Netherlands.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about lesser-known places in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.intro}
              intro={
                <SectionIntro eyebrow="Related" title="Related living and lifestyle guides" fullWidth>
                  <p>
                    Weekend trips covers destination weekends. Museums covers museum visits. Weekend travel covers OV
                    how-to. Family activities supports kid leisure. Hiking and National parks are nature peers. Cycling
                    and Bike sharing help last mile without duplicating Hidden gems in the Cycling menu.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.snapshot}
              intro={
                <SectionIntro eyebrow="Hub" title="Weekend & lifestyle hub" fullWidth>
                  <p>
                    Stay inside the cluster — Weekend trips, National parks, Hiking, Museums and Hidden gems — and
                    cross-link to Weekend travel for transport, plus Family activities when useful.
                  </p>
                </SectionIntro>
              }
            >
              <BulletPanel title="Hub tips" items={page.lifestyleHubTips} />
              <div className={guidePremiumCardGridClass(page.lifestyleHubCards.length)}>
                {page.lifestyleHubCards.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>
                    Continue to Weekend trips for destination ideas, Museums for museum visits, Weekend travel for OV,
                    National parks or Hiking for nature, or Cycling for bike last mile.
                  </p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>
                    Confirm tourism, park, municipal and transport details on official tools — these sources are
                    orientation only.
                  </p>
                </SectionIntro>
