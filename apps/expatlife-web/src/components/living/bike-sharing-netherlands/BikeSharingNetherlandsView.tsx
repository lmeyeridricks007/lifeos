...
import {
  ...
  bikeSharingNetherlandsPage as page,
  type TransportLink,
  type MistakeCard as MistakeCardData,
} from "./bikeSharingNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Bike sharing</h3>
...
export function BikeSharingNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Bike sharing", item: new URL(BIKE_SHARING_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Bike sharing at a glance" fullWidth>
                  <p>
                    Six cards cover share vs own, models, costs, how to start, OV-fiets plus train, and mistakes to avoid —
                    deeper sections expand each theme without product rankings.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Models" title={page.models.heading} fullWidth>
                  <p>{page.models.intro}</p>
                  {page.models.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.models.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.models.cards.length)}>
                {page.models.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.models.crossLinks.length)}>
                {page.models.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="start"
              tipsKey="start"
              visual={page.visuals.start}
              intro={
                <SectionIntro eyebrow="Start" title={page.start.heading} fullWidth>
                  <p>{page.start.intro}</p>
                  {page.start.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.start.steps.length)}>
                {page.start.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
              ...
              <InfoTable
                columns={compareColumns}
                rows={page.start.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.start.cards.length)}>
                {page.start.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.start.crossLinks.length)}>
                {page.start.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="ovfiets"
              tipsKey="ovfiets"
              visual={page.visuals.ovfiets}
              intro={
                <SectionIntro eyebrow="Multimodal" title={page.ovfiets.heading} fullWidth>
                  <p>{page.ovfiets.intro}</p>
                  {page.ovfiets.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.ovfiets.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.ovfiets.cards.length)}>
                {page.ovfiets.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.ovfiets.crossLinks.length)}>
                {page.ovfiets.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={BIKE_SHARING_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common bike-sharing mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — buying too soon, using OV-fiets as a sole commute bike, late
                    returns, or skipping path habits. Calm model choice and return timers beat forum screenshots.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Bike sharing readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats starting bike sharing calmly in the Netherlands.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about bike sharing in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and transport guides" fullWidth>
                  <p>
                    Cycling covers everyday ownership, rules and locks. Getting around covers multimodal mobility. NS trains
                    pairs with OV-fiets. OVpay and OV-chipkaart deepen tickets — this page stays on shared and subscription
                    bikes.
                  </p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Active mobility and public transport hub" fullWidth>
                  <p>
                    Stay inside the cluster — Bike sharing, Cycling, Getting around, OVpay, OV-chipkaart, NS trains,
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>
                    Continue to Cycling, Getting around, NS trains, OVpay, OV-chipkaart, or Car sharing when you need
                    them.
                  </p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>
                    Confirm OV-fiets rules, provider terms and live journeys on official tools — these sources are
                    orientation only.
                  </p>
                </SectionIntro>
