...
import {
  ...
  roadTaxNetherlandsPage as page,
  type DrivingLink,
  type MistakeCard as MistakeCardData,
} from "./roadTaxNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for road tax</h3>
...
export function RoadTaxNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Road tax", item: new URL(ROAD_TAX_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Tax file checklist" items={page.taxFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Road tax at a glance" fullWidth>
                  <p>
                    Six cards cover what MRB is, who pays, amount factors, how to check, how to pay, and timing after RDW
                    registration — deeper sections expand each theme without tax advice.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="who-pays"
              tipsKey="whoPays"
              visual={page.visuals.whoPays}
              intro={
                <SectionIntro eyebrow="Responsibility" title={page.whoPays.heading} fullWidth>
                  <p>{page.whoPays.intro}</p>
                  {page.whoPays.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.whoPays.steps.length)}>
                {page.whoPays.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
              ...
              <div className={guidePremiumCardGridClass(page.whoPays.cards.length)}>
                {page.whoPays.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.whoPays.crossLinks.length)}>
                {page.whoPays.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Amount" title={page.factors.heading} fullWidth>
                  <p>{page.factors.intro}</p>
                  {page.factors.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.factors.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.factors.cards.length)}>
                {page.factors.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.factors.crossLinks.length)}>
                {page.factors.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="check"
              tipsKey="check"
              visual={page.visuals.check}
              intro={
                <SectionIntro eyebrow="Check" title={page.check.heading} fullWidth>
                  <p>{page.check.intro}</p>
                  {page.check.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.check.steps.length)}>
                {page.check.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
                    body={`${step.timing} — ${step.detail}`}
              ...
              <div className={guidePremiumCardGridClass(page.check.cards.length)}>
                {page.check.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="pay"
              tipsKey="pay"
              visual={page.visuals.pay}
              intro={
                <SectionIntro eyebrow="Pay" title={page.pay.heading} fullWidth>
                  <p>{page.pay.intro}</p>
                  {page.pay.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.pay.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.pay.cards.length)}>
                {page.pay.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.pay.crossLinks.length)}>
                {page.pay.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            >
              <div className={guidePremiumCardGridClass(page.timing.steps.length)}>
                {page.timing.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
                    body={`${step.timing} — ${step.detail}`}
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common road-tax mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — budgeting only the purchase price, copying a colleague’s
                    figure, or mixing BPM with MRB. Official checks beat forum folklore.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Road tax readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats handling Dutch road tax after registering a car.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about road tax in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and practical guides" fullWidth>
                  <p>
                    Buying a car covers purchase and RDW transfer. Car insurance covers liability. Driving licence
                    exchange covers drive rights. Getting around and parking deepen mobility and curb-side admin.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Driving cluster hub" fullWidth>
                  <p>
                    Stay inside the driving cluster — road tax, buying a car, car insurance, licence exchange, getting
                    around and parking.
                  </p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>Continue to buying a car, car insurance, parking permits or getting around.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm amounts and payment rules with Belastingdienst and RDW — these sources are orientation only.</p>
                </SectionIntro>
