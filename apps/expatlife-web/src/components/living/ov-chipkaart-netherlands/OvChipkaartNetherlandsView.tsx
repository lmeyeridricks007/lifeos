...
} from "./ovChipkaartNetherlandsPageModel";
...
const iconPool = [IdCard, FileCheck2, CreditCard, ClipboardList, Scale, Wallet, MapPin, Timer, ShieldCheck, TrainFront] as const;
...
const costColumns = [
  ...
  { key: "range", label: "Planning range" },
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for OV-chipkaart</h3>
...
export function OvChipkaartNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "OV-chipkaart", item: new URL(OV_CHIPKAART_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Card file checklist" items={page.cardFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="OV-chipkaart at a glance" fullWidth>
                  <p>
                    Six cards cover what the card is, anonymous versus personal, get and top up, check-in/out, missed
                    checkout, and when OVpay is enough — deeper sections expand each theme without product advice.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="what-it-is"
              tipsKey="whatItIs"
              visual={page.visuals.whatItIs}
              intro={
                <SectionIntro eyebrow="Basics" title={page.whatItIs.heading} fullWidth>
                  <p>{page.whatItIs.intro}</p>
                  {page.whatItIs.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                ...
                rows={page.whatItIs.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.whatItIs.cards.length)}>
                {page.whatItIs.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.whatItIs.crossLinks.length)}>
                {page.whatItIs.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="anonymous-vs-personal"
              tipsKey="anonymousVsPersonal"
              visual={page.visuals.anonymousVsPersonal}
              intro={
                <SectionIntro eyebrow="Card types" title={page.anonymousVsPersonal.heading} fullWidth>
                  <p>{page.anonymousVsPersonal.intro}</p>
                  {page.anonymousVsPersonal.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.anonymousVsPersonal.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.anonymousVsPersonal.cards.length)}>
                {page.anonymousVsPersonal.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.anonymousVsPersonal.crossLinks.length)}>
                {page.anonymousVsPersonal.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="get-card"
              tipsKey="getCard"
              visual={page.visuals.getCard}
              intro={
                <SectionIntro eyebrow="Acquire" title={page.getCard.heading} fullWidth>
                  <p>{page.getCard.intro}</p>
                  {page.getCard.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.getCard.steps.length)}>
                {page.getCard.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
              ...
              <div className={guidePremiumCardGridClass(page.getCard.cards.length)}>
                {page.getCard.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.getCard.crossLinks.length)}>
                {page.getCard.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="top-up"
              tipsKey="topUp"
              visual={page.visuals.topUp}
              intro={
                <SectionIntro eyebrow="Balance" title={page.topUp.heading} fullWidth>
                  <p>{page.topUp.intro}</p>
                  {page.topUp.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={costColumns}
                rows={page.topUp.rows.map((row) => ({
                  category: row.category,
                  range: row.range,
                  notes: row.notes,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.topUp.cards.length)}>
                {page.topUp.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.topUp.crossLinks.length)}>
                {page.topUp.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="check-in-out"
              tipsKey="checkInOut"
              visual={page.visuals.checkInOut}
              intro={
                <SectionIntro eyebrow="Taps" title={page.checkInOut.heading} fullWidth>
                  <p>{page.checkInOut.intro}</p>
                  {page.checkInOut.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.checkInOut.steps.length)}>
                {page.checkInOut.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
                    body={`${step.timing} — ${step.detail}`}
              ...
              <div className={guidePremiumCardGridClass(page.checkInOut.cards.length)}>
                {page.checkInOut.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.checkInOut.crossLinks.length)}>
                {page.checkInOut.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="missed-checkout"
              tipsKey="missedCheckout"
              visual={page.visuals.missedCheckout}
              intro={
                <SectionIntro eyebrow="Recovery" title={page.missedCheckout.heading} fullWidth>
                  <p>{page.missedCheckout.intro}</p>
                  {page.missedCheckout.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.missedCheckout.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.missedCheckout.cards.length)}>
                {page.missedCheckout.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.missedCheckout.crossLinks.length)}>
                {page.missedCheckout.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="products"
              tipsKey="products"
              visual={page.visuals.products}
              intro={
                <SectionIntro eyebrow="Products" title={page.products.heading} fullWidth>
                  <p>{page.products.intro}</p>
                  {page.products.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.products.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.products.cards.length)}>
                {page.products.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.products.crossLinks.length)}>
                {page.products.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="vs-ovpay"
              tipsKey="vsOvpay"
              visual={page.visuals.vsOvpay}
              intro={
                <SectionIntro eyebrow="Compare" title={page.vsOvpay.heading} fullWidth>
                  <p>{page.vsOvpay.intro}</p>
                  {page.vsOvpay.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.vsOvpay.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.vsOvpay.cards.length)}>
                {page.vsOvpay.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.vsOvpay.crossLinks.length)}>
                {page.vsOvpay.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={OV_CHIPKAART_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common OV-chipkaart mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — missed checkouts, empty balances, buying personal too early, or
                    ignoring OVpay. Official operator terms beat forum folklore.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="OV-chipkaart readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats setting up OV-chipkaart calmly in the Netherlands.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about OV-chipkaart in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.intro}
              intro={
                <SectionIntro eyebrow="Related" title="Related living and transport guides" fullWidth>
                  <p>
                    Getting around covers multimodal mobility. OVpay covers contactless taps. NS trains covers rail products —
                    this page stays on the personal OV-chipkaart journey.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.snapshot}
              intro={
                <SectionIntro eyebrow="Hub" title="Public transport cluster hub" fullWidth>
                  <p>
                    Stay inside the public transport cluster — OV-chipkaart, OVpay, NS trains and Getting around — with light
                    links to car sharing when OV is not enough.
                  </p>
                </SectionIntro>
              }
            >
              <BulletPanel title="Hub tips" items={page.transportHubTips} />
              <div className={guidePremiumCardGridClass(page.transportHubCards.length)}>
                {page.transportHubCards.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>Continue to OVpay, NS trains, Getting around, or light driving options when you need them.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm card and tap rules on official tools — these sources are orientation only.</p>
                </SectionIntro>
