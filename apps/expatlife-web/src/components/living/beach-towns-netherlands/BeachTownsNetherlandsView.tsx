...
import {
  ...
  beachTownsNetherlandsPage as page,
  type LifestyleLink,
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Beach towns</h3>
...
export function BeachTownsNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Beach towns", item: new URL(BEACH_TOWNS_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Beach towns at a glance" fullWidth>
                  <p>
                    Six cards cover beach culture, seasons, coastal stretches, vibe filters, packing and weather, and
                    crowds plus getting there — deeper sections expand each theme without ranking beach towns or hotels.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Culture" title={page.culture.heading} fullWidth>
                  <p>{page.culture.intro}</p>
                  {page.culture.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                ...
                rows={page.culture.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.culture.cards.length)}>
                {page.culture.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.culture.crossLinks.length)}>
                {page.culture.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="seasons"
              tipsKey="seasons"
              visual={page.visuals.seasons}
              intro={
                <SectionIntro eyebrow="Seasons" title={page.seasons.heading} fullWidth>
                  <p>{page.seasons.intro}</p>
                  {page.seasons.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.seasons.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.seasons.cards.length)}>
                {page.seasons.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.seasons.crossLinks.length)}>
                {page.seasons.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="stretches"
              tipsKey="stretches"
              visual={page.visuals.stretches}
              intro={
                <SectionIntro eyebrow="Stretches" title={page.stretches.heading} fullWidth>
                  <p>{page.stretches.intro}</p>
                  {page.stretches.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.stretches.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.stretches.cards.length)}>
                {page.stretches.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.stretches.crossLinks.length)}>
                {page.stretches.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="vibes"
              tipsKey="vibes"
              visual={page.visuals.vibes}
              intro={
                <SectionIntro eyebrow="Vibes" title={page.vibes.heading} fullWidth>
                  <p>{page.vibes.intro}</p>
                  {page.vibes.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.vibes.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.vibes.cards.length)}>
                {page.vibes.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.vibes.crossLinks.length)}>
                {page.vibes.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="packing"
              tipsKey="packing"
              visual={page.visuals.packing}
              intro={
                <SectionIntro eyebrow="Packing" title={page.packing.heading} fullWidth>
                  <p>{page.packing.intro}</p>
                  {page.packing.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.packing.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.packing.cards.length)}>
                {page.packing.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.packing.crossLinks.length)}>
                {page.packing.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="crowds"
              tipsKey="crowds"
              visual={page.visuals.crowds}
              intro={
                <SectionIntro eyebrow="Crowds" title={page.crowds.heading} fullWidth>
                  <p>{page.crowds.intro}</p>
                  {page.crowds.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.crowds.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.crowds.cards.length)}>
                {page.crowds.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.crowds.crossLinks.length)}>
                {page.crowds.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={BEACH_TOWNS_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common beach-day mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — ranked FOMO lists, swimsuit-only packing, late peak arrivals,
                    or ignoring return trains. Calm stretch-and-vibe planning beats influencer beach checklists.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Beach day readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats planning calm Dutch beach days.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about beach towns in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and lifestyle guides" fullWidth>
                  <p>
                    Weekend trips covers destination weekends. Weekend travel covers OV how-to. Hidden gems covers
                    lesser-known non-beach spots. Hiking, National parks and Museums are cluster peers. Cycling and Bike
                    sharing help last mile without duplicating Beach towns in the Cycling menu.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Weekend & lifestyle hub" fullWidth>
                  <p>
                    Stay inside the cluster — Weekend trips, National parks, Hiking, Museums, Hidden gems, Beach towns
                    and Castles — and cross-link to Weekend travel for transport.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>
                    Continue to Weekend trips for destination ideas, Weekend travel for OV, Hidden gems for lesser-known
                    places, Castles for historic-house weekends, Hiking or National parks for nature, or Museums for an
                    indoor Plan B.
                  </p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>
                    Confirm tourism, weather and transport details on official tools — these sources are orientation
                    only.
                  </p>
                </SectionIntro>
