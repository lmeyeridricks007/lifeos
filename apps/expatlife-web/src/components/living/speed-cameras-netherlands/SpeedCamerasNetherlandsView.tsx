...
import {
  ...
  speedCamerasNetherlandsPage as page,
  type DrivingLink,
  type MistakeCard as MistakeCardData,
} from "./speedCamerasNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for speed enforcement</h3>
...
export function SpeedCamerasNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Speed cameras", item: new URL(SPEED_CAMERAS_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Fine file checklist" items={page.fineFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Speed cameras at a glance" fullWidth>
                  <p>
                    Six cards cover camera types, enforcement flow, trajectcontrole, how fines arrive, pay/object orientation, and
                    practical driver habits — deeper sections expand each theme without legal advice.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="what"
              tipsKey="what"
              visual={page.visuals.what}
              intro={
                <SectionIntro eyebrow="Definition" title={page.what.heading} fullWidth>
                  <p>{page.what.intro}</p>
            ...
            >
              <InfoTable
                ...
                rows={page.what.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.what.cards.length)}>
                {page.what.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.what.crossLinks.length)}>
                {page.what.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="enforcement"
              tipsKey="enforcement"
              visual={page.visuals.enforcement}
              intro={
                <SectionIntro eyebrow="Process" title={page.enforcement.heading} fullWidth>
                  <p>{page.enforcement.intro}</p>
                  {page.enforcement.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.enforcement.steps.length)}>
                {page.enforcement.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
              ...
              <div className={guidePremiumCardGridClass(page.enforcement.cards.length)}>
                {page.enforcement.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.enforcement.crossLinks.length)}>
                {page.enforcement.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="trajectcontrole"
              tipsKey="trajectcontrole"
              visual={page.visuals.trajectcontrole}
              intro={
                <SectionIntro eyebrow="Corridors" title={page.trajectcontrole.heading} fullWidth>
                  <p>{page.trajectcontrole.intro}</p>
                  {page.trajectcontrole.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.trajectcontrole.steps.length)}>
                {page.trajectcontrole.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
                    body={`${step.timing} — ${step.detail}`}
              ...
              <div className={guidePremiumCardGridClass(page.trajectcontrole.cards.length)}>
                {page.trajectcontrole.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.trajectcontrole.crossLinks.length)}>
                {page.trajectcontrole.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="fines"
              tipsKey="fines"
              visual={page.visuals.fines}
              intro={
                <SectionIntro eyebrow="Notices" title={page.fines.heading} fullWidth>
                  <p>{page.fines.intro}</p>
                  {page.fines.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.fines.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.fines.cards.length)}>
                {page.fines.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.fines.crossLinks.length)}>
                {page.fines.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="payment"
              tipsKey="payment"
              visual={page.visuals.payment}
              intro={
                <SectionIntro eyebrow="Admin" title={page.payment.heading} fullWidth>
                  <p>{page.payment.intro}</p>
                  {page.payment.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.payment.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.payment.cards.length)}>
                {page.payment.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.payment.crossLinks.length)}>
                {page.payment.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="practical"
              tipsKey="practical"
              visual={page.visuals.practical}
              intro={
                <SectionIntro eyebrow="Habits" title={page.practical.heading} fullWidth>
                  <p>{page.practical.intro}</p>
                  {page.practical.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.practical.steps.length)}>
                {page.practical.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
                    body={`${step.timing} — ${step.detail}`}
              ...
              <div className={guidePremiumCardGridClass(page.practical.cards.length)}>
                {page.practical.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.practical.crossLinks.length)}>
                {page.practical.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common speed-camera mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — ignoring letters, paying fake links, braking only at flitsers,
                    or mixing speed fines with parking tickets. Official notices beat forum folklore.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Speed cameras readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats handling a Dutch speed-enforcement notice calmly.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about Speed cameras in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and practical guides" fullWidth>
                  <p>
                    Getting around covers multimodal mobility. Licence exchange covers drive rights. Buying a car, insurance,
                    road tax and MOT / APK cover ownership. Parking covers curb-side tickets — this page stays on speed enforcement.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Driving cluster hub" fullWidth>
                  <p>
                    Stay inside the driving cluster — speed cameras, getting around, licence exchange, buying a car, car
                    insurance, road tax, MOT / APK and parking.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>Continue to getting around, MOT / APK, car insurance, buying a car, parking permits or licence exchange.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm fine amounts and procedures with CJIB and the notice itself — these sources are orientation only.</p>
                </SectionIntro>
