  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Trams</h3>
...
export function TramsNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Trams", item: new URL(TRAMS_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Trams at a glance" fullWidth>
                  <p>
                    Six cards cover city networks, how to ride, etiquette, when tram wins, city examples, and sibling
            ...
            <PremiumGuideSection
              id="how-to-ride"
              tipsKey="howToRide"
              visual={page.visuals.howToRide}
              intro={
                <SectionIntro eyebrow="Ride" title={page.howToRide.heading} fullWidth>
                  <p>{page.howToRide.intro}</p>
                  {page.howToRide.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.howToRide.steps.length)}>
                {page.howToRide.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
              ...
              <InfoTable
                columns={compareColumns}
                rows={page.howToRide.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.howToRide.cards.length)}>
                {page.howToRide.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.howToRide.crossLinks.length)}>
                {page.howToRide.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="when-tram-wins"
              tipsKey="whenTramWins"
              visual={page.visuals.whenTramWins}
              intro={
                <SectionIntro eyebrow="Mode choice" title={page.whenTramWins.heading} fullWidth>
                  <p>{page.whenTramWins.intro}</p>
                  {page.whenTramWins.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.whenTramWins.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.whenTramWins.cards.length)}>
                {page.whenTramWins.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.whenTramWins.crossLinks.length)}>
                {page.whenTramWins.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="city-examples"
              tipsKey="cityExamples"
              visual={page.visuals.cityExamples}
              intro={
                <SectionIntro eyebrow="Cities" title={page.cityExamples.heading} fullWidth>
                  <p>{page.cityExamples.intro}</p>
                  {page.cityExamples.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.cityExamples.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.cityExamples.cards.length)}>
                {page.cityExamples.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.cityExamples.crossLinks.length)}>
                {page.cityExamples.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={TRAMS_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common tram mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — missed checkouts, mixing payment methods, treating tram as
                    NS trains, or blocking doors while validating. Official operator and OVpay terms beat forum screenshots.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Trams readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats starting city tram travel calmly in the Netherlands.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about Trams in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and transport guides" fullWidth>
                  <p>
                    ...
                    travel cards. Metro, Regional buses and NS trains are mode siblings — this page stays on city trams.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Public transport cluster hub" fullWidth>
                  <p>
                    Stay inside the public transport cluster — Trams, Metro, Regional buses, NS trains, OVpay, OV-chipkaart
                    and Getting around.
                  </p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>Continue to Metro, Regional buses, OVpay, OV-chipkaart, NS trains, or Getting around when you need them.</p>
                </SectionIntro>
              }
            >
