...
import {
  ...
  roadTripsNetherlandsPage as page,
  type LifestyleLink,
  type MistakeCard as MistakeCardData,
} from "./roadTripsNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Road trips</h3>
...
export function RoadTripsNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Road trips", item: new URL(ROAD_TRIPS_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Road trips at a glance" fullWidth>
                  <p>
                    Six cards cover when a car helps, route-planning culture, parking and ferries, multi-stop shapes,
                    sharing vs owning, and light cross-border caution — deeper sections expand each theme without ranking
                    routes, destinations or rental cars.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="when-car"
              tipsKey="whenCar"
              visual={page.visuals.whenCar}
              intro={
                <SectionIntro eyebrow="Mode" title={page.whenCar.heading} fullWidth>
                  <p>{page.whenCar.intro}</p>
                  {page.whenCar.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                ...
                rows={page.whenCar.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.whenCar.cards.length)}>
                {page.whenCar.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.whenCar.crossLinks.length)}>
                {page.whenCar.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Practical" title={page.parking.heading} fullWidth>
                  <p>{page.parking.intro}</p>
            ...
            >
              <InfoTable
                columns={compareColumns}
                rows={page.parking.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.parking.cards.length)}>
                {page.parking.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.parking.crossLinks.length)}>
                {page.parking.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="routes"
              tipsKey="routes"
              visual={page.visuals.routes}
              intro={
                <SectionIntro eyebrow="Shapes" title={page.routes.heading} fullWidth>
                  <p>{page.routes.intro}</p>
                  {page.routes.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.routes.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.routes.cards.length)}>
                {page.routes.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.routes.crossLinks.length)}>
                {page.routes.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="border"
              tipsKey="border"
              visual={page.visuals.border}
              intro={
                <SectionIntro eyebrow="Border" title={page.border.heading} fullWidth>
                  <p>{page.border.intro}</p>
                  {page.border.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.border.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.border.cards.length)}>
                {page.border.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.border.crossLinks.length)}>
                {page.border.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={ROAD_TRIPS_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common road-trip mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — too many stops, ignored parking, ranked route FOMO, buying a
                    car too early, marathon Sundays, or border document gaps. Calm mode-and-shape planning beats
                    influencer road-trip checklists.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Road trip readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats planning calm Dutch road trips.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about road trips in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and lifestyle guides" fullWidth>
                  <p>
                    Weekend trips covers destination vibes. Day trips covers same-day outings. Weekend travel covers OV
                    how-to. Car sharing and Buying a car deepen vehicle access. Beach towns, Castles, National parks,
                    Hiking, Museums and Hidden gems are place-type peers.
                  </p>
                </SectionIntro>
              }
            >
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Weekend & lifestyle hub" fullWidth>
                  <p>
                    ...
                    Castles, Road trips and Day trips — and cross-link to Weekend travel for transport and Car sharing
                    for fleets.
                  </p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>
                    Continue to Weekend trips for destination vibes, Day trips for same-day outings, Car sharing or
                    Buying a car for access, Weekend travel for OV, or Beach towns, Castles and National parks for
                    place-type depth.
                  </p>
                </SectionIntro>
              }
