...
import {
  ...
  leaseCarsNetherlandsPage as page,
  type DrivingLink,
  type MistakeCard as MistakeCardData,
} from "./leaseCarsNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for lease cars</h3>
...
export function LeaseCarsNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Lease cars", item: new URL(LEASE_CARS_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Lease file checklist" items={page.leaseFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Lease cars at a glance" fullWidth>
                  <p>
                    Six cards cover lease types, the lease-vs-buy decision, private lease, company cars and bijtelling
                    orientation, costs versus buying, and contract watch-outs — deeper sections expand each theme without
                    tax or legal advice.
                  </p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Types" title={page.types.heading} fullWidth>
                  <p>{page.types.intro}</p>
                  {page.types.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                ...
                rows={page.types.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.types.cards.length)}>
                {page.types.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.types.crossLinks.length)}>
                {page.types.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="private"
              tipsKey="private"
              visual={page.visuals.private}
              intro={
                <SectionIntro eyebrow="Private lease" title={page.private.heading} fullWidth>
                  <p>{page.private.intro}</p>
                  {page.private.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.private.steps.length)}>
                {page.private.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
              ...
              <div className={guidePremiumCardGridClass(page.private.cards.length)}>
                {page.private.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.private.crossLinks.length)}>
                {page.private.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="company"
              tipsKey="company"
              visual={page.visuals.company}
              intro={
                <SectionIntro eyebrow="Company" title={page.company.heading} fullWidth>
                  <p>{page.company.intro}</p>
                  {page.company.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.company.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.company.cards.length)}>
                {page.company.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.company.crossLinks.length)}>
                {page.company.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Contracts" title={page.contracts.heading} fullWidth>
                  <p>{page.contracts.intro}</p>
                  {page.contracts.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.contracts.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.contracts.cards.length)}>
                {page.contracts.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.contracts.crossLinks.length)}>
                {page.contracts.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Practical" title={page.practical.heading} fullWidth>
            ...
            >
              <InfoTable
                columns={compareColumns}
                rows={page.practical.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={LEASE_CARS_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common lease mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — signing without kilometre honesty, ignoring bijtelling
                    orientation, treating lease as buy advice, or skipping early-exit clauses. Written contracts beat
                    forum folklore.
                  </p>
                </SectionIntro>
              }
            ...
            >
              <ChecklistBlock title="Lease readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats approaching lease cars calmly in the Netherlands.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about Lease cars in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and practical guides" fullWidth>
                  <p>
                    Buying a car covers purchase paths. Car sharing covers occasional access. Electric vehicles deepen EV
                    ownership. Getting around covers OV and bikes — this page stays on private lease, operational lease and
                    company-car orientation.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Driving cluster hub" fullWidth>
                  <p>
                    Stay inside the driving cluster — lease cars, getting around, licence exchange, buying a car, electric
                    vehicles, car insurance, road tax, MOT / APK, speed cameras and parking.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>Continue to buying a car, car sharing, electric vehicles, getting around, parking or licence exchange.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm tax, licence and transport rules on official tools — these sources are orientation only.</p>
                </SectionIntro>
