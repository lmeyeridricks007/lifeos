...
import {
  ...
  buyingACarNetherlandsPage as page,
  type DrivingLink,
  type MistakeCard as MistakeCardData,
} from "./buyingACarNetherlandsPageModel";
...
const snapshotIcons = [IdCard, FileCheck2, ClipboardList, TrainFront] as const;
const orientationIcons = [IdCard, Scale, ClipboardList, TrainFront] as const;
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for car purchase</h3>
...
export function BuyingACarNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Buying a car", item: new URL(BUYING_A_CAR_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Purchase file checklist" items={page.purchaseFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Buying a car at a glance" fullWidth>
                  <p>
                    Six cards cover the ownership decision, new vs used, dealer vs private, BPM and tax orientation,
                    RDW transfer, plus insurance and APK — deeper sections expand each theme without legal or tax advice.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Decide" title={page.decide.heading} fullWidth>
            ...
            <PremiumGuideSection
              id="new-vs-used"
              tipsKey="newVsUsed"
              visual={page.visuals.newVsUsed}
              intro={
                <SectionIntro eyebrow="New vs used" title={page.newVsUsed.heading} fullWidth>
                  <p>{page.newVsUsed.intro}</p>
                  {page.newVsUsed.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.newVsUsed.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.newVsUsed.cards.length)}>
                {page.newVsUsed.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="dealer-private"
              tipsKey="dealerPrivate"
              visual={page.visuals.dealerPrivate}
              intro={
                <SectionIntro eyebrow="Purchase path" title={page.dealerPrivate.heading} fullWidth>
                  <p>{page.dealerPrivate.intro}</p>
                  {page.dealerPrivate.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.dealerPrivate.steps.length)}>
                {page.dealerPrivate.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
              ...
              <div className={guidePremiumCardGridClass(page.dealerPrivate.cards.length)}>
                {page.dealerPrivate.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.dealerPrivate.crossLinks.length)}>
                {page.dealerPrivate.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="bpm-tax"
              tipsKey="bpmTax"
              visual={page.visuals.bpmTax}
              intro={
                <SectionIntro eyebrow="Tax orientation" title={page.bpmTax.heading} fullWidth>
                  <p>{page.bpmTax.intro}</p>
                  {page.bpmTax.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.bpmTax.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.bpmTax.cards.length)}>
                {page.bpmTax.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="rdw"
              tipsKey="rdw"
              visual={page.visuals.rdw}
              intro={
                <SectionIntro eyebrow="RDW" title={page.rdw.heading} fullWidth>
                  <p>{page.rdw.intro}</p>
                  {page.rdw.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.rdw.steps.length)}>
                {page.rdw.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
                    body={`${step.timing} — ${step.detail}`}
              ...
              <div className={guidePremiumCardGridClass(page.rdw.cards.length)}>
                {page.rdw.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.rdw.crossLinks.length)}>
                {page.rdw.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            >
              <InfoTable
                columns={compareColumns}
                rows={page.insurance.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
            ...
            <PremiumGuideSection
              id="apk"
              tipsKey="apk"
              visual={page.visuals.apk}
              intro={
                <SectionIntro eyebrow="APK" title={page.apk.heading} fullWidth>
                  <p>{page.apk.intro}</p>
                  {page.apk.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.apk.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.apk.cards.length)}>
                {page.apk.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={BUYING_A_CAR_NETHERLANDS_PATH}
                  surfaceTone="muted"
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common car-purchase mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — licence clarity, parking reality and a same-day RDW plus
                    insurance plan beat a rushed classifieds buy.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Car purchase readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats buying a car in the Netherlands.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about buying a car in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and practical guides" fullWidth>
                  <p>
                    Driving licence exchange covers drive rights. Getting around covers OV and bikes. Parking and
                    municipality services deepen the admin around cars and appointments.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Driving cluster hub" fullWidth>
                  <p>
                    Stay inside the driving cluster — buying a car, licence exchange, getting around and parking — plus
                    municipality services for appointments.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>Continue to licence exchange, parking permits, getting around or municipality services.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm registration and tax rules with RDW and Belastingdienst — these sources are orientation only.</p>
                </SectionIntro>
