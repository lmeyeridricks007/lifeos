...
import {
  ...
  museumsNetherlandsPage as page,
  type LifestyleLink,
  type MistakeCard as MistakeCardData,
} from "./museumsNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Museums</h3>
...
export function MuseumsNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Museums", item: new URL(MUSEUMS_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Museums at a glance" fullWidth>
                  <p>
                    Six cards cover museum culture, Museumkaart, free days and booking, city patterns, etiquette, and
                    families — deeper sections expand each theme without ranking museums or tickets.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="museumkaart"
              tipsKey="museumkaart"
              visual={page.visuals.museumkaart}
              intro={
                <SectionIntro eyebrow="Museumkaart" title={page.museumkaart.heading} fullWidth>
                  <p>{page.museumkaart.intro}</p>
                  {page.museumkaart.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.museumkaart.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.museumkaart.cards.length)}>
                {page.museumkaart.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.museumkaart.crossLinks.length)}>
                {page.museumkaart.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            >
              <InfoTable
                columns={compareColumns}
                rows={page.booking.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.booking.cards.length)}>
                {page.booking.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.booking.crossLinks.length)}>
                {page.booking.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Cities" title={page.cities.heading} fullWidth>
                  <p>{page.cities.intro}</p>
                  {page.cities.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.cities.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.cities.cards.length)}>
                {page.cities.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.cities.crossLinks.length)}>
                {page.cities.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Families" title={page.families.heading} fullWidth>
                  <p>{page.families.intro}</p>
                  {page.families.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.families.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.families.cards.length)}>
                {page.families.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.families.crossLinks.length)}>
                {page.families.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={MUSEUMS_NETHERLANDS_PATH}
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common museum mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — ranked FOMO lists, skipping timed tickets, stale Museumkaart
                    info, or marathon itineraries. Calm booking habits beat influencer checklists.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Museum day readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats planning calm Dutch museum visits.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about museums in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and lifestyle guides" fullWidth>
                  <p>
                    Weekend trips covers destination weekends. Hidden gems covers lesser-known places. Weekend travel
                    covers OV how-to. Family activities supports kid leisure. Hiking and National parks are nature peers.
                    Cycling and Bike sharing help last mile without duplicating Museums in the Cycling menu.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>
                    Continue to Weekend trips for destination ideas, Hidden gems for lesser-known places, Weekend travel
                    for OV, Family activities for kid leisure, Hiking for nature, or Cycling for bike last mile.
                  </p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>
                    Confirm Museumkaart, venue, tourism and transport details on official tools — these sources are
                    orientation only.
                  </p>
                </SectionIntro>
