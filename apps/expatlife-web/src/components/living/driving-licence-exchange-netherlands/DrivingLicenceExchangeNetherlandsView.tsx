...
import {
  ...
  drivingLicenceExchangeNetherlandsPage as page,
  type DrivingLink,
  type MistakeCard as MistakeCardData,
} from "./drivingLicenceExchangeNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for licence exchange</h3>
...
export function DrivingLicenceExchangeNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Licence file checklist" items={page.licenceFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Licence exchange at a glance" fullWidth>
                  <p>
                    Six cards cover validity windows, RDW rules, municipality desks, documents, EU vs non-EU paths and
                    test triggers — deeper sections expand each theme without legal advice.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="validity"
              tipsKey="validity"
              visual={page.visuals.validity}
              intro={
                <SectionIntro eyebrow="Validity" title={page.validity.heading} fullWidth>
                  <p>{page.validity.intro}</p>
                  {page.validity.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                ...
                rows={page.validity.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.validity.cards.length)}>
                {page.validity.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.validity.crossLinks.length)}>
                {page.validity.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="process"
              tipsKey="process"
              visual={page.visuals.process}
              intro={
                <SectionIntro eyebrow="Process" title={page.process.heading} fullWidth>
            ...
            >
              <div className={guidePremiumCardGridClass(page.process.steps.length)}>
              ...
              <div className={guidePremiumCardGridClass(page.process.cards.length)}>
                {page.process.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Documents" title={page.documents.heading} fullWidth>
                  <p>{page.documents.intro}</p>
                  {page.documents.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Bring / prepare" items={page.documents.items} columns={2} />
              <InfoTable
                columns={compareColumns}
                rows={page.documents.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.documents.crossLinks.length)}>
                {page.documents.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="timelines"
              tipsKey="timelines"
              visual={page.visuals.timelines}
              intro={
                <SectionIntro eyebrow="Timelines" title={page.timelines.heading} fullWidth>
                  <p>{page.timelines.intro}</p>
                  {page.timelines.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.timelines.steps.length)}>
                {page.timelines.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
                    body={`${step.timing} — ${step.detail}`}
              ...
              <div className={guidePremiumCardGridClass(page.timelines.cards.length)}>
                {page.timelines.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="origin-rules"
              tipsKey="originRules"
              visual={page.visuals.originRules}
              intro={
                <SectionIntro eyebrow="Origin country" title={page.originRules.heading} fullWidth>
                  <p>{page.originRules.intro}</p>
                  {page.originRules.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.originRules.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.originRules.cards.length)}>
                {page.originRules.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="tests"
              tipsKey="tests"
              visual={page.visuals.tests}
              intro={
                <SectionIntro eyebrow="Tests" title={page.tests.heading} fullWidth>
                  <p>{page.tests.intro}</p>
                  {page.tests.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.tests.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.tests.cards.length)}>
                {page.tests.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={DRIVING_LICENCE_EXCHANGE_NETHERLANDS_PATH}
                  surfaceTone="muted"
                />
              </div>
            </section>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common licence-exchange mistakes" fullWidth>
                  <p>These patterns create avoidable stress — early country checks and complete document packs beat last-minute appointments.</p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Licence exchange readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats starting a foreign driving licence exchange in the Netherlands.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about exchanging a foreign driving licence in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and practical guides" fullWidth>
                  <p>
                    Getting around covers OV and bikes. Buying a car is the driving cluster sibling. Parking and
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Driving cluster hub" fullWidth>
                  <p>Stay inside the driving cluster — licence exchange, buying a car, getting around and parking — plus municipality services for appointments.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>Continue to buying a car, getting around while you wait, parking permits or municipality services.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm exchange rules with RDW and your municipality — these sources are orientation only.</p>
                </SectionIntro>
