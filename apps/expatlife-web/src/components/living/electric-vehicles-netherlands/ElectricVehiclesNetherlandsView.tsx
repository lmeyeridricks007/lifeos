...
import {
  ...
  electricVehiclesNetherlandsPage as page,
  type DrivingLink,
  type MistakeCard as MistakeCardData,
} from "./electricVehiclesNetherlandsPageModel";
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for EV ownership</h3>
...
export function ElectricVehiclesNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Electric vehicles", item: new URL(ELECTRIC_VEHICLES_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="EV file checklist" items={page.evFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Electric vehicles at a glance" fullWidth>
                  <p>
                    Six cards cover the EV decision, buying, home and public charging, tax orientation, and range plus ownership
                    ties — deeper sections expand each theme without tax or insurance advice.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Decision" title={page.decide.heading} fullWidth>
                  <p>{page.decide.intro}</p>
                  {page.decide.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                ...
                rows={page.decide.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.decide.cards.length)}>
                {page.decide.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.decide.crossLinks.length)}>
                {page.decide.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="buy"
              tipsKey="buy"
              visual={page.visuals.buy}
              intro={
                <SectionIntro eyebrow="Purchase" title={page.buy.heading} fullWidth>
                  <p>{page.buy.intro}</p>
                  {page.buy.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.buy.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.buy.cards.length)}>
                {page.buy.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.buy.crossLinks.length)}>
                {page.buy.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="home-charging"
              tipsKey="homeCharging"
              visual={page.visuals.homeCharging}
              intro={
                <SectionIntro eyebrow="Home" title={page.homeCharging.heading} fullWidth>
                  <p>{page.homeCharging.intro}</p>
                  {page.homeCharging.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.homeCharging.steps.length)}>
                {page.homeCharging.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
              ...
              <div className={guidePremiumCardGridClass(page.homeCharging.cards.length)}>
                {page.homeCharging.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.homeCharging.crossLinks.length)}>
                {page.homeCharging.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="public-charging"
              tipsKey="publicCharging"
              visual={page.visuals.publicCharging}
              intro={
                <SectionIntro eyebrow="Public" title={page.publicCharging.heading} fullWidth>
                  <p>{page.publicCharging.intro}</p>
                  {page.publicCharging.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.publicCharging.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.publicCharging.cards.length)}>
                {page.publicCharging.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.publicCharging.crossLinks.length)}>
                {page.publicCharging.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="incentives"
              tipsKey="incentives"
              visual={page.visuals.incentives}
              intro={
                <SectionIntro eyebrow="Tax" title={page.incentives.heading} fullWidth>
                  <p>{page.incentives.intro}</p>
                  {page.incentives.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.incentives.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.incentives.cards.length)}>
                {page.incentives.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.incentives.crossLinks.length)}>
                {page.incentives.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="range"
              tipsKey="range"
              visual={page.visuals.range}
              intro={
                <SectionIntro eyebrow="Range" title={page.range.heading} fullWidth>
                  <p>{page.range.intro}</p>
                  {page.range.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.range.steps.length)}>
                {page.range.steps.map((step, idx) => (
                  <FeatureCard
                    key={step.phase}
                    title={step.phase}
                    body={`${step.timing} — ${step.detail}`}
              ...
              <div className={guidePremiumCardGridClass(page.range.cards.length)}>
                {page.range.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.range.crossLinks.length)}>
                {page.range.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="ownership-stack"
              tipsKey="ownershipStack"
              visual={page.visuals.ownershipStack}
              intro={
                <SectionIntro eyebrow="Ownership" title={page.ownershipStack.heading} fullWidth>
                  <p>{page.ownershipStack.intro}</p>
                  {page.ownershipStack.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.ownershipStack.rows.map((row) => ({
                  topic: row.topic,
                  whatToCheck: row.whatToCheck,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.ownershipStack.cards.length)}>
                {page.ownershipStack.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.ownershipStack.crossLinks.length)}>
                {page.ownershipStack.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common EV ownership mistakes" fullWidth>
                  <p>
                    These patterns create avoidable stress — buying before a charging plan, ignoring VvE rules, trusting
                    brochure-only range, or budgeting from stale tax blogs. Official tools beat forum folklore.
                  </p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Electric vehicles readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats starting EV ownership calmly in the Netherlands.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about Electric vehicles in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related living and practical guides" fullWidth>
                  <p>
                    Buying a car covers general purchase and RDW transfer. Licence exchange covers drive rights. Insurance,
                    road tax and MOT / APK deepen ownership admin. Getting around and parking cover mobility and curb reality —
                    this page stays on EV charging and ownership.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Driving cluster hub" fullWidth>
                  <p>
                    Stay inside the driving cluster — electric vehicles, getting around, licence exchange, buying a car, car
                    insurance, road tax, MOT / APK, speed cameras, parking, car sharing and lease cars.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next practical step" tone="onDark" fullWidth>
                  <p>Continue to buying a car, car insurance, road tax, MOT / APK, getting around or parking permits.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm BPM, road tax and registration rules on official tools — these sources are orientation only.</p>
                </SectionIntro>
