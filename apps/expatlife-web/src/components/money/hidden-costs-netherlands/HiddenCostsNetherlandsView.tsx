...
import { MONEY_HUB_PATH } from "@/src/components/money/cost-of-living-netherlands/costOfLivingNetherlandsPageModel";
import {
  HIDDEN_COSTS_NETHERLANDS_PATH,
  hiddenCostsNetherlandsPage as page,
...
const snapshotIcons = [Home, Train, Wallet, MapPin] as const;
const orientationIcons = [Wallet, Scale, Train, MapPin] as const;
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for spotting hidden costs</h3>
...
export function HiddenCostsNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Hidden-costs file — keep these together" items={page.safetyFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Hidden costs at a glance" fullWidth>
                  <p>Six cards cover cross-category surprises beyond headline rent. Deeper sections expand each theme without ranking providers.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="what-are"
              tipsKey="whatAre"
              visual={page.visuals.whatAre}
              intro={
                <SectionIntro eyebrow="Definition" title={page.whatAre.heading} fullWidth>
                  <p>{page.whatAre.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="What this page means by hidden costs" items={page.whatAre.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.whatAre.cards.length)}>
                {page.whatAre.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.whatAre.crossLinks.length)}>
                {page.whatAre.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="move-in"
              tipsKey="moveIn"
              visual={page.visuals.moveIn}
              intro={
                <SectionIntro eyebrow="Move-in cash" title={page.moveIn.heading} fullWidth>
                  <p>{page.moveIn.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Move-in cash habits" items={page.moveIn.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.moveIn.cards.length)}>
                {page.moveIn.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.moveIn.crossLinks.length)}>
                {page.moveIn.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            >
              <ChecklistBlock title="Insurance surprise habits" items={page.insurance.bullets} columns={2} />
            ...
            <PremiumGuideSection
              id="municipal"
              tipsKey="municipal"
              visual={page.visuals.municipal}
              intro={
                <SectionIntro eyebrow="Local taxes" title={page.municipal.heading} fullWidth>
                  <p>{page.municipal.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Municipal tax habits" items={page.municipal.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.municipal.cards.length)}>
                {page.municipal.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.municipal.crossLinks.length)}>
                {page.municipal.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="banking-fx"
              tipsKey="bankingFx"
              visual={page.visuals.bankingFx}
              intro={
                <SectionIntro eyebrow="Banking & FX" title={page.bankingFx.heading} fullWidth>
                  <p>{page.bankingFx.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="FX and banking habits" items={page.bankingFx.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.bankingFx.cards.length)}>
                {page.bankingFx.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.bankingFx.crossLinks.length)}>
                {page.bankingFx.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="family-extras"
              tipsKey="familyExtras"
              visual={page.visuals.familyExtras}
              intro={
                <SectionIntro eyebrow="Family" title={page.familyExtras.heading} fullWidth>
                  <p>{page.familyExtras.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Family extra habits" items={page.familyExtras.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.familyExtras.cards.length)}>
                {page.familyExtras.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.familyExtras.crossLinks.length)}>
                {page.familyExtras.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="bands"
              tipsKey="bands"
              visual={page.visuals.bands}
              intro={
                <SectionIntro eyebrow="2026 bands" title={page.bands.heading} fullWidth>
                  <p>{page.bands.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="How to read bands" items={page.bands.bullets} columns={2} />
              <InfoTable
                columns={costColumns}
                rows={page.bands.rows.map((row) => ({
                  setting: row.setting,
                  band: row.band,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.bands.crossLinks.length)}>
                {page.bands.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            >
              <ScenarioTable title="Hidden-cost money stories" rows={page.scenarios.rows} />
            </PremiumGuideSection>
            ...
            >
              <ChecklistBlock title="Hidden-costs orientation checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Follow these orientation steps before month one — then verify live tariffs with providers.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Tools" title={page.tools.heading} fullWidth>
                  <p>Use tools to model the full month — then verify live deposits, premiums and tariffs.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about surprise costs beyond headline rent.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.intro}
              intro={
                <SectionIntro eyebrow="Related" title="Related money and housing guides" fullWidth>
                  <p>Cluster siblings, cost-of-living peers, and housing/moving boundary guides stay linkable without comingSoon labels.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Money hub and cluster cards" fullWidth>
                  <p>Return to the Money hub or jump to Cheap cities, Financial checklist, Cost of living, and Housing costs.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next money step" tone="onDark" fullWidth>
                  <p>Order your money setup, compare place stacks, or deepen housing deposit mechanics next.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm deposits, premiums, municipal tariffs and FX products with current official and market sources — bands here are orientation only.</p>
                </SectionIntro>
