...
import {
  DEBIT_CARDS_PATH,
  debitCardsPage as page,
  type DebitCardsLink,
  type MistakeCard as MistakeCardData,
} from "./debitCardsPageModel";
...
function LinkCard({ item, iconIndex = 0, tone = "default" }: { item: DebitCardsLink; iconIndex?: number; tone?: "default" | "onDark" }) {
  const Icon = iconPool[iconIndex % iconPool.length];
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Dutch debit</h3>
...
export function DebitCardsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Debit cards", item: new URL(DEBIT_CARDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Debit readiness file — keep these together" items={page.safetyFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Debit cards at a glance" fullWidth>
                  <p>Six cards cover Dutch debit and PIN culture. Deeper sections below expand each theme without ranking issuers.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="pinpas"
              tipsKey="pinpas"
              visual={page.visuals.pinpas}
              intro={
                <SectionIntro eyebrow="Pinpas" title={page.pinpas.heading} fullWidth>
                  <p>{page.pinpas.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Pinpas basics" items={page.pinpas.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.pinpas.cards.length)}>
                {page.pinpas.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.pinpas.crossLinks.length)}>
                {page.pinpas.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Networks" title={page.networks.heading} fullWidth>
                  <p>{page.networks.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="What network labels mean in practice" items={page.networks.bullets} columns={2} />
            ...
            <PremiumGuideSection
              id="shops"
              tipsKey="shops"
              visual={page.visuals.shops}
              intro={
                <SectionIntro eyebrow="In shops" title={page.shops.heading} fullWidth>
                  <p>{page.shops.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="What to expect at the till" items={page.shops.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.shops.cards.length)}>
                {page.shops.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <WarningPanel
                title="Verify tap limits with your bank"
                items={[
                  "Do not assume a universal euro contactless ceiling",
                  "PIN re-checks after tap limits are routine",
                  "If tap fails, switch to chip + PIN without panic",
                ]}
              />
              <div className={guidePremiumCardGridClass(page.shops.crossLinks.length)}>
                {page.shops.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="atms"
              tipsKey="atms"
              visual={page.visuals.atms}
              intro={
                <SectionIntro eyebrow="ATMs" title={page.atms.heading} fullWidth>
                  <p>{page.atms.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="ATM habits" items={page.atms.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.atms.cards.length)}>
                {page.atms.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Online" title={page.online.heading} fullWidth>
                  <p>{page.online.lead}</p>
                </SectionIntro>
              }
            >
              <BulletPanel title="Online debit habits" items={page.online.bullets} />
              <Timeline steps={page.online.timeline} eyebrow="Online" />
              <div className={guidePremiumCardGridClass(page.online.crossLinks.length)}>
                {page.online.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="habits"
              tipsKey="habits"
              visual={page.visuals.habits}
              intro={
                <SectionIntro eyebrow="Habit shift" title={page.habits.heading} fullWidth>
                  <p>{page.habits.lead}</p>
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.habits.rows.map((row) => ({
                  setting: row.setting,
                  homeHabit: row.homeHabit,
                  dutchNorm: row.dutchNorm,
                  tip: row.tip,
              ...
              />
              <div className={guidePremiumCardGridClass(page.habits.crossLinks.length)}>
                {page.habits.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            >
              <ScenarioTable title="Tourist and expat debit stories" rows={page.scenarios.rows} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title={page.mistakes.heading} fullWidth>
                  <p>These patterns create avoidable debit friction — calm backups beat awkward declines.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Ready for debit" title={page.checklist.heading} fullWidth>
                  <p>Work through this list before your first busy debit-heavy shopping week.</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Debit readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about Dutch debit cards and PIN culture.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related banking guides" fullWidth>
                  <p>Live cluster siblings stay linkable — Cash vs card, Credit cards and Wise vs Revolut.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Banking hub and cluster cards" fullWidth>
                  <p>Return to the Banking hub or jump to Cash vs card, payments rails, and live cluster peers.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next money step" tone="onDark" fullWidth>
                  <p>Learn till culture, payment rails, open an account, shortlist banks, or model costs.</p>
                </SectionIntro>
              }
            >
