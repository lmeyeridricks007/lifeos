...
import {
  ...
  openBankAccountNetherlandsPage as page,
...
const snapshotIcons = [Landmark, FileText, Clock, Banknote] as const;
const orientationIcons = [ClipboardList, FileText, Building2, CreditCard] as const;
...
const documentColumns = [
  ...
  { key: "why", label: "Why banks ask" },
  { key: "tip", label: "Practical tip" },
] as const;

const costColumns = [
  { key: "type", label: "Type" },
  { key: "typical", label: "Typical cost (indicative 2026)" },
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four building blocks before you apply</h3>
...
export function OpenBankAccountNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Open a bank account", item: new URL(OPEN_BANK_ACCOUNT_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Setup file — keep these together" items={page.safetyFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Dutch bank setup at a glance" fullWidth>
                  <p>Six cards cover the journey. Deeper sections below expand each theme without ranking banks.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="why-need"
              tipsKey="whyNeed"
              visual={page.visuals.whyNeed}
              intro={
                <SectionIntro eyebrow="Everyday money" title={page.whyNeed.heading} fullWidth>
                  <p>{page.whyNeed.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Common uses for a Dutch account" items={page.whyNeed.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.whyNeed.crossLinks.length)}>
                {page.whyNeed.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="requirements"
              tipsKey="requirements"
              visual={page.visuals.requirements}
              intro={
                <SectionIntro eyebrow="Documents" title={page.requirements.heading} fullWidth>
                  <p>{page.requirements.lead}</p>
                </SectionIntro>
              }
            >
              <GuideCrossLink
                href={page.requirements.callout.href}
                title={page.requirements.callout.title}
                description={page.requirements.callout.text}
                linkLabel={page.requirements.callout.linkLabel}
              />
              <InfoTable
                columns={documentColumns}
                rows={page.requirements.documents.map((row) => ({
                  document: row.document,
                  why: row.why,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.requirements.relatedMoveLinks.length)}>
                {page.requirements.relatedMoveLinks.map((item, idx) => (
                  <LinkCard key={item.href} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Timing" title={page.bsn.heading} fullWidth>
                  <p>{page.bsn.lead}</p>
                </SectionIntro>
              }
            >
              <BulletPanel title="BSN timing habits" items={page.bsn.bullets} />
              <Timeline steps={page.bsn.timeline} eyebrow="BSN" />
              <WarningPanel
                title="Verify “BSN later” on the bank site"
                items={[
                  "Do not rely on outdated blog posts for BSN exceptions",
                  "Diary any submit-later deadline the bank gives you",
                  "If review stalls on BSN, use the Account rejection guide",
                ]}
              />
              <div className={guidePremiumCardGridClass(page.bsn.crossLinks.length)}>
                {page.bsn.crossLinks.map((item, idx) => (
                  <LinkCard key={item.href} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="steps"
              tipsKey="steps"
              visual={page.visuals.steps}
              intro={
                <SectionIntro eyebrow="How-to" title={page.steps.heading} fullWidth>
                  <p>{page.steps.lead}</p>
                </SectionIntro>
              }
            >
              <ol className="space-y-4">
                {page.steps.howToSteps.map((step, index) => (
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.steps}
              intro={
                <SectionIntro eyebrow="Timing bands" title={page.timeline.heading} fullWidth>
                  <p>{page.timeline.lead}</p>
                </SectionIntro>
              }
            >
              <Timeline steps={page.timeline.steps} eyebrow="Band" />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="delays"
              tipsKey="delays"
              visual={page.visuals.delays}
              intro={
                <SectionIntro eyebrow="When it stalls" title={page.delays.heading} fullWidth>
                  <p>{page.delays.lead}</p>
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.delays.cards.length)}>
                {page.delays.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <GuideCrossLink
                href={page.delays.crossLink.href}
                title={page.delays.crossLink.title}
                description={page.delays.crossLink.description}
                linkLabel={page.delays.crossLink.linkLabel}
              />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.snapshot}
              intro={
                <SectionIntro eyebrow="Product labels" title={page.accountTypes.heading} fullWidth>
                  <p>{page.accountTypes.lead}</p>
                </SectionIntro>
              }
            >
              <BulletPanel title="Start with the right label" items={page.accountTypes.bullets} />
              <div className={guidePremiumCardGridClass(page.accountTypes.links.length)}>
                {page.accountTypes.links.map((item, idx) => (
                  <LinkCard key={item.href} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Fees" title={page.costs.heading} fullWidth>
                  <p>{page.costs.lead}</p>
                </SectionIntro>
              }
            >
              ...
              />
              <div className={guidePremiumCardGridClass(page.costs.links.length)}>
                {page.costs.links.map((item, idx) => (
                  <LinkCard key={item.href} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            >
              <ScenarioTable title="Example arrival stories" rows={page.scenarios.rows} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title={page.mistakes.heading} fullWidth>
                  <p>These patterns create avoidable delays — calm fixes beat rapid re-applies.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.checklist}
              intro={
                <SectionIntro eyebrow="Ready to apply" title={page.checklist.heading} fullWidth>
                  <p>Work through this list the day before you upload documents.</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Pre-apply checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Tools" title={page.tools.heading} fullWidth>
                  <p>Use tools for planning — then verify products and fees on each bank’s official pages.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about opening a Dutch account.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.intro}
              intro={
                <SectionIntro eyebrow="Related" title="Related banking and move guides" fullWidth>
                  <p>Cluster siblings stay live — plus fees, payments, rejection recovery and BSN setup guides.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Banking hub and cluster cards" fullWidth>
                  <p>Return to the Banking hub or jump to the two live cluster siblings from this page.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next money step" tone="onDark" fullWidth>
                  <p>Shortlist banks, plan transfers, model costs, or recover a stalled application.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm requirements and fees on official and bank sites — rules change.</p>
                </SectionIntro>
