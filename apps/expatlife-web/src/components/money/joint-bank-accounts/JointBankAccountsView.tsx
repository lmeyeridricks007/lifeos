...
import {
  JOINT_BANK_ACCOUNTS_PATH,
  jointBankAccountsPage as page,
  type JointBankAccountsLink,
  type MistakeCard as MistakeCardData,
} from "./jointBankAccountsPageModel";
...
const compareColumns = [
  ...
  { key: "dutchNorm", label: "Dutch joint norm" },
...
function LinkCard({ item, iconIndex = 0, tone = "default" }: { item: JointBankAccountsLink; iconIndex?: number; tone?: "default" | "onDark" }) {
  const Icon = iconPool[iconIndex % iconPool.length];
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Dutch joint banking</h3>
...
export function JointBankAccountsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Joint bank accounts", item: new URL(JOINT_BANK_ACCOUNTS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Joint readiness file — keep these together" items={page.safetyFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Joint bank accounts at a glance" fullWidth>
                  <p>Six cards cover gezamenlijke rekening norms for couples and housemates. Deeper sections expand each theme without ranking banks.</p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="What a gezamenlijke rekening is" items={page.whatIs.bullets} columns={2} />
            ...
            <PremiumGuideSection
              id="who-can-open"
              tipsKey="whoCanOpen"
              visual={page.visuals.whoCanOpen}
              intro={
                <SectionIntro eyebrow="Eligibility" title={page.whoCanOpen.heading} fullWidth>
                  <p>{page.whoCanOpen.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Who usually opens" items={page.whoCanOpen.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.whoCanOpen.cards.length)}>
                {page.whoCanOpen.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <InfoTable
                columns={compareColumns}
                rows={page.whoCanOpen.rows.map((row) => ({
                  setting: row.setting,
              ...
              />
              <div className={guidePremiumCardGridClass(page.whoCanOpen.crossLinks.length)}>
                {page.whoCanOpen.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="shared-access"
              tipsKey="sharedAccess"
              visual={page.visuals.sharedAccess}
              intro={
                <SectionIntro eyebrow="Access & cards" title={page.sharedAccess.heading} fullWidth>
                  <p>{page.sharedAccess.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Shared access habits" items={page.sharedAccess.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.sharedAccess.cards.length)}>
                {page.sharedAccess.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <WarningPanel
                title="Agree spending norms before rent day"
                items={[
                  "Shared history makes surprise purchases visible",
                  "Dual cards mean both adults can spend — agree a euro threshold",
                  "Liability and limits follow the bank contract — verify them",
                ]}
              />
              <div className={guidePremiumCardGridClass(page.sharedAccess.crossLinks.length)}>
                {page.sharedAccess.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Household habits" title={page.household.heading} fullWidth>
                  <p>{page.household.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Household money rules" items={page.household.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.household.cards.length)}>
                {page.household.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <InfoTable
                columns={compareColumns}
                rows={page.household.rows.map((row) => ({
                  setting: row.setting,
                  homeHabit: row.homeHabit,
                  dutchNorm: row.dutchNorm,
                  tip: row.tip,
              ...
              />
              <div className={guidePremiumCardGridClass(page.household.crossLinks.length)}>
                {page.household.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="opening"
              tipsKey="opening"
              visual={page.visuals.opening}
              intro={
                <SectionIntro eyebrow="Opening" title={page.opening.heading} fullWidth>
                  <p>{page.opening.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Opening habits" items={page.opening.bullets} columns={2} />
              <Timeline steps={page.opening.timeline} eyebrow="Opening" />
              <div className={guidePremiumCardGridClass(page.opening.crossLinks.length)}>
                {page.opening.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="exit"
              tipsKey="exit"
              visual={page.visuals.exit}
              intro={
                <SectionIntro eyebrow="Exit" title={page.exit.heading} fullWidth>
                  <p>{page.exit.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Exit habits" items={page.exit.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.exit.cards.length)}>
                {page.exit.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <ChecklistBlock title="Breakup / exit checklist" items={page.exit.checklistItems} columns={2} />
              <WarningPanel
                title="Not legal or relationship advice"
                items={[
                  "Banks, landlords and courts set their own rules",
                  "Move rent and utilities before you close the IBAN",
                  "Keep statements for deposits and admin — seek qualified advice when needed",
                ]}
              />
            </PremiumGuideSection>
            ...
            >
              <ScenarioTable title="Joint account stories" rows={page.scenarios.rows} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title={page.mistakes.heading} fullWidth>
                  <p>These patterns create avoidable joint-account friction — calm exit planning beats rushed closures.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Ready for joint" title={page.checklist.heading} fullWidth>
                  <p>Work through this list before your first busy shared rent cycle.</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Joint readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Tools" title={page.tools.heading} fullWidth>
                  <p>Use tools for account and cost planning — then verify products and fees on each bank’s official pages.</p>
                </SectionIntro>
              }
            >
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={JOINT_BANK_ACCOUNTS_PATH}
                />
              </div>
            </section>
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about Dutch gezamenlijke rekeningen and shared household banking.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related banking guides" fullWidth>
                  <p>Live cluster siblings stay linkable — Types of accounts, Open a bank account, Debit cards and Best banks.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Banking hub and cluster cards" fullWidth>
                  <p>Return to the Banking hub or jump to Types of accounts, Open a bank account, and live cluster peers.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next money step" tone="onDark" fullWidth>
                  <p>Learn account types, open an account, shortlist banks, understand dual debit cards, or model costs.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm joint product rules and fees with banks — tariffs and eligibility change.</p>
                </SectionIntro>
