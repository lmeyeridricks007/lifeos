...
import {
  STUDENT_BANK_ACCOUNTS_PATH,
  studentBankAccountsPage as page,
  type StudentBankAccountsLink,
  type MistakeCard as MistakeCardData,
} from "./studentBankAccountsPageModel";
...
const snapshotIcons = [CreditCard, Nfc, WalletCards, Smartphone] as const;
const orientationIcons = [WalletCards, Nfc, CreditCard, Landmark] as const;
...
const compareColumns = [
  ...
  { key: "homeHabit", label: "Common habit elsewhere" },
  { key: "dutchNorm", label: "Dutch student norm" },
...
function LinkCard({ item, iconIndex = 0, tone = "default" }: { item: StudentBankAccountsLink; iconIndex?: number; tone?: "default" | "onDark" }) {
  const Icon = iconPool[iconIndex % iconPool.length];
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Dutch student banking</h3>
...
export function StudentBankAccountsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Student bank accounts", item: new URL(STUDENT_BANK_ACCOUNTS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Student readiness file — keep these together" items={page.safetyFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Student bank accounts at a glance" fullWidth>
                  <p>Six cards summarise studentrekening norms for enrolled students. Deeper sections expand each theme without ranking banks.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="what-is"
              tipsKey="whatIs"
              visual={page.visuals.whatIs}
              intro={
                <SectionIntro eyebrow="Definition" title={page.whatIs.heading} fullWidth>
                  <p>{page.whatIs.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="What a studentrekening is" items={page.whatIs.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.whatIs.cards.length)}>
                {page.whatIs.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.whatIs.crossLinks.length)}>
                {page.whatIs.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="eligibility"
              tipsKey="eligibility"
              visual={page.visuals.eligibility}
              intro={
                <SectionIntro eyebrow="Eligibility" title={page.eligibility.heading} fullWidth>
                  <p>{page.eligibility.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Who usually opens" items={page.eligibility.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.eligibility.cards.length)}>
                {page.eligibility.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <InfoTable
                columns={compareColumns}
                rows={page.eligibility.rows.map((row) => ({
                  setting: row.setting,
                  homeHabit: row.homeHabit,
                  dutchNorm: row.dutchNorm,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.eligibility.crossLinks.length)}>
                {page.eligibility.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="perks"
              tipsKey="perks"
              visual={page.visuals.perks}
              intro={
                <SectionIntro eyebrow="Package perks" title={page.perks.heading} fullWidth>
                  <p>{page.perks.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Perk orientation" items={page.perks.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.perks.cards.length)}>
                {page.perks.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <WarningPanel
                title="Perks change — verify live pages"
                items={[
                  "Marketing lists expire and differ by bank",
                  "A free first debit card is common — confirm replacement fees",
                  "Partner discounts are not guarantees — check start and end dates",
                ]}
              />
              <div className={guidePremiumCardGridClass(page.perks.crossLinks.length)}>
                {page.perks.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="vs-regular"
              tipsKey="vsRegular"
              visual={page.visuals.vsRegular}
              intro={
                <SectionIntro eyebrow="Vs regular" title={page.vsRegular.heading} fullWidth>
                  <p>{page.vsRegular.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Student vs regular habits" items={page.vsRegular.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.vsRegular.cards.length)}>
                {page.vsRegular.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <InfoTable
                columns={compareColumns}
                rows={page.vsRegular.rows.map((row) => ({
                  setting: row.setting,
                  homeHabit: row.homeHabit,
                  dutchNorm: row.dutchNorm,
                  tip: row.tip,
              ...
              />
              <div className={guidePremiumCardGridClass(page.vsRegular.crossLinks.length)}>
                {page.vsRegular.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="international"
              tipsKey="international"
              visual={page.visuals.international}
              intro={
                <SectionIntro eyebrow="International" title={page.international.heading} fullWidth>
                  <p>{page.international.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Document habits" items={page.international.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.international.cards.length)}>
                {page.international.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <ChecklistBlock title="International student document checklist" items={page.international.checklistItems} columns={2} />
              <div className={guidePremiumCardGridClass(page.international.crossLinks.length)}>
                {page.international.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="graduation"
              tipsKey="graduation"
              visual={page.visuals.graduation}
              intro={
                <SectionIntro eyebrow="Graduation" title={page.graduation.heading} fullWidth>
                  <p>{page.graduation.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Graduation habits" items={page.graduation.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.graduation.cards.length)}>
                {page.graduation.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <Timeline steps={page.graduation.timeline} eyebrow="Graduation" />
              <WarningPanel
                title="Plan the fee uplift early"
                items={[
                  "Student pricing usually ends when enrolment ends",
                  "Ask whether your IBAN stays the same for rent and payroll",
                  "Verify conversion terms on your bank’s official pages",
                ]}
              />
              <div className={guidePremiumCardGridClass(page.graduation.crossLinks.length)}>
                {page.graduation.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            >
              <ScenarioTable title="Student account stories" rows={page.scenarios.rows} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title={page.mistakes.heading} fullWidth>
                  <p>These patterns create avoidable student-account friction — calm graduation planning beats rushed fee surprises.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Ready for student" title={page.checklist.heading} fullWidth>
                  <p>Work through this list before you rely on a student IBAN for rent or tuition.</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Student readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Tools" title={page.tools.heading} fullWidth>
                  <p>Use tools for account and cost planning — then verify student products and fees on each bank’s official pages.</p>
                </SectionIntro>
              }
            >
            ...
            <section id={page.recommendedOptions.sectionId} className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                <BankingRecommendedOptionsSection
                  ...
                  utmReferrerPath={STUDENT_BANK_ACCOUNTS_PATH}
                />
              </div>
            </section>
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about Dutch studentrekeningen and student package banking.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related banking guides" fullWidth>
                  <p>Live cluster siblings stay linkable — Types of accounts, Joint bank accounts, Open a bank account, Student visa and Best banks.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Banking hub and cluster cards" fullWidth>
                  <p>Return to the Banking hub or jump to Types of accounts, Joint bank accounts, Open a bank account, and live cluster peers.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next money step" tone="onDark" fullWidth>
                  <p>Learn account types, open an account, shortlist banks, check the student visa path, or model costs.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm student product rules and fees with banks — tariffs and eligibility change.</p>
                </SectionIntro>
