...
import { ProviderCard } from "@/src/components/monetization/ProviderCard";
...
const snapshotIcons = [Scale, ArrowLeftRight, Smartphone, Landmark] as const;
const orientationIcons = [ClipboardList, Landmark, Scale, ShieldCheck] as const;
  ...
  return (
    <article className={cn(cardClass, "p-5 sm:p-6")}>
      ...
      <ul className="mt-4 space-y-2">
        {signals.map((signal) => (
          <li key={signal} className="flex gap-2 text-sm leading-relaxed text-foreground-muted">
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for this decision</h3>
...
function ProviderDiscoverySection() {
  const utmContent = utmContentFromPath(CANONICAL);
  const cards = page.providers.order
    .map((providerId) => {
      const provider = loadProvider(providerId);
      if (!provider) return null;
      const note = page.providers.notes.find((entry) => entry.id === providerId);
  ...
  return (
    <div className="space-y-5">
      <div className={guidePremiumCardGridClass(Math.max(cards.length, 2))}>
        {cards.map(({ provider, note }) => (
          <ProviderCard
            key={provider.id}
            name={provider.name}
            logo={
              provider.logo?.src
                ? { src: provider.logo.src, alt: provider.logo.alt }
                : { src: "", alt: provider.name }
            }
            description={note?.editorialAngle ?? provider.tagline}
            tags={[...provider.badges]}
            bestFor={provider.highlights.slice(0, 2).join(" · ") || provider.tagline}
            priceHint="Verify live fees and plans on the provider site (2026 orientation)."
            ctaLabel={provider.cta.label}
            href={withPartnerReferralUtms(provider.cta.href, {
              partnerSlug: provider.id,
              utmContent,
            })}
            isAffiliate={provider.cta.isAffiliate}
            disclosureText={provider.disclosure}
            tone="copilot"
          />
        ))}
      </div>
      <AffiliateDisclosureNote className="text-xs text-foreground-muted">
        {DEFAULT_MONETIZATION_DISCLOSURE} {page.providers.disclaimer} Indicative fee and FX framing on this page is for
        2026 orientation only — verify live quotes on provider sites.
      </AffiliateDisclosureNote>
      <WarningPanel
        title="Verify before you fund an app"
        items={[
          "Eligibility and plan features change — read today’s product pages",
          "Editorial order is not a ranking or award",
          "Dutch salary/rent acceptance is decided by counterparties, not this guide",
        ]}
      />
    </div>
  );
...
export function WiseVsRevolutView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Decision file — keep these together" items={page.safetyFileChecklist} columns={2} />
              <ScenarioTable title="Where people usually start" rows={page.introScenarios} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Wise vs Revolut at a glance" fullWidth>
                  <p>Six cards cover the decision map. Deeper sections expand each theme without crowning a winner.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="roles"
              tipsKey="roles"
              visual={page.visuals.roles}
              intro={
                <SectionIntro eyebrow="Roles" title={page.roles.heading} fullWidth>
                  <p>{page.roles.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="How to read these products" items={page.roles.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.roles.cards.length)}>
                {page.roles.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.roles.crossLinks.length)}>
                {page.roles.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="dutch-layer"
              tipsKey="dutchLayer"
              visual={page.visuals.dutchLayer}
              intro={
                <SectionIntro eyebrow="Layers" title={page.dutchLayer.heading} fullWidth>
                  <p>{page.dutchLayer.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Layer planning checklist" items={page.dutchLayer.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.dutchLayer.cards.length)}>
                {page.dutchLayer.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.dutchLayer.crossLinks.length)}>
                {page.dutchLayer.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="wise-fit"
              tipsKey="wiseFit"
              visual={page.visuals.wiseFit}
              intro={
                <SectionIntro eyebrow="Wise" title={page.wiseFit.heading} fullWidth>
                  <p>{page.wiseFit.lead}</p>
                </SectionIntro>
              }
            >
              <FitSignalCard title={page.wiseFit.fit.title} body={page.wiseFit.fit.body} signals={page.wiseFit.fit.signals} />
              <div className={guidePremiumCardGridClass(page.wiseFit.cards.length)}>
                {page.wiseFit.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="revolut-fit"
              tipsKey="revolutFit"
              visual={page.visuals.revolutFit}
              intro={
                <SectionIntro eyebrow="Revolut" title={page.revolutFit.heading} fullWidth>
                  <p>{page.revolutFit.lead}</p>
                </SectionIntro>
              }
            >
              <FitSignalCard
                title={page.revolutFit.fit.title}
                body={page.revolutFit.fit.body}
                signals={page.revolutFit.fit.signals}
              />
              <div className={guidePremiumCardGridClass(page.revolutFit.cards.length)}>
                {page.revolutFit.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="fees-fx"
              tipsKey="feesFx"
              visual={page.visuals.feesFx}
              intro={
                <SectionIntro eyebrow="Costs" title={page.feesFx.heading} fullWidth>
                  <p>{page.feesFx.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Cost-stack habits" items={page.feesFx.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.feesFx.cards.length)}>
                {page.feesFx.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <p className="text-xs text-foreground-muted md:hidden" role="note">
                On small screens the table becomes easier to scroll horizontally — same columns.
              </p>
              <InfoTable
                columns={compareColumns}
                rows={page.feesFx.compareRows.map((row) => ({
              ...
              />
              <WarningPanel
                title="Indicative 2026 framing only"
                items={[
                  "No live tariffs or guaranteed savings on this page",
                  "Re-run official calculators for your corridor and amount today",
                  "Plan limits and weekend rules can change the real bill",
                ]}
              />
              <div className={guidePremiumCardGridClass(page.feesFx.crossLinks.length)}>
                {page.feesFx.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.snapshot}
              intro={
                <SectionIntro eyebrow="Side by side" title={page.compare.heading} fullWidth>
                  <p>{page.compare.lead}</p>
                </SectionIntro>
              }
            >
              <InfoTable
              ...
              />
              <AffiliateDisclosureNote className="text-xs text-foreground-muted">
                {DEFAULT_MONETIZATION_DISCLOSURE} Table cells describe typical editorial patterns — not live pricing or
                awards.
              </AffiliateDisclosureNote>
            </PremiumGuideSection>
            ...
            >
              <ScenarioTable title="Expat money jobs" rows={page.scenarios.rows} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Ready to decide" title={page.checklist.heading} fullWidth>
            ...
            >
              <div className="space-y-4">
                {page.checklist.groups.map((group) => (
                  <ChecklistBlock key={group.title} title={group.title} items={group.items} columns={2} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.roles}
              intro={
                <SectionIntro eyebrow="Providers" title={page.providers.heading} fullWidth>
                  <p>{page.providers.lead}</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about Wise vs Revolut in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related banking guides" fullWidth>
                  <p>
                    Live cluster siblings and deeper banking guides stay linkable — International transfers covers send
                    mechanics; debit and credit pages cover card culture.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Banking hub and cluster cards" fullWidth>
                  <p>Return to the Banking hub or jump to live peers for onboarding, transfers and bank shortlists.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next money step" tone="onDark" fullWidth>
                  <p>Deepen transfers, open a Dutch account, widen the bank shortlist, or model costs.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm fees, plans and eligibility on provider sites — products change.</p>
                </SectionIntro>
