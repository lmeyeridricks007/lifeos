...
import {
  ...
  financialChecklistNetherlandsPage as page,
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for money setup order</h3>
...
export function FinancialChecklistNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Money ops file — keep these together" items={page.safetyFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Financial checklist at a glance" fullWidth>
                  <p>Six cards cover money setup order for the first months. Deeper sections expand each op without filing your taxes for you.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Definition" title={page.whatItIs.heading} fullWidth>
                  <p>{page.whatItIs.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Boundary habits" items={page.whatItIs.bullets} columns={2} />
            ...
            <PremiumGuideSection
              id="first-30"
              tipsKey="first30"
              visual={page.visuals.first30}
              intro={
                <SectionIntro eyebrow="First 30 days" title={page.first30.heading} fullWidth>
                  <p>{page.first30.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="30-day priorities" items={page.first30.bullets} columns={2} />
              <InfoTable
                columns={timelineColumns}
                rows={page.first30.rows.map((row) => ({
                  window: row.window,
                  focus: row.focus,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.first30.cards.length)}>
                {page.first30.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="first-90"
              tipsKey="first90"
              visual={page.visuals.first90}
              intro={
                <SectionIntro eyebrow="Days 31–90" title={page.first90.heading} fullWidth>
                  <p>{page.first90.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="90-day priorities" items={page.first90.bullets} columns={2} />
              <InfoTable
                columns={timelineColumns}
                rows={page.first90.rows.map((row) => ({
                  window: row.window,
                  focus: row.focus,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.first90.cards.length)}>
                {page.first90.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            >
              <ChecklistBlock title="Banking setup habits" items={page.banking.bullets} columns={2} />
            ...
            >
              <ChecklistBlock title="Insurance setup habits" items={page.insurance.bullets} columns={2} />
            ...
            <PremiumGuideSection
              id="budget"
              tipsKey="budget"
              visual={page.visuals.budget}
              intro={
                <SectionIntro eyebrow="Budget" title={page.budget.heading} fullWidth>
                  <p>{page.budget.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Budget setup habits" items={page.budget.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.budget.cards.length)}>
                {page.budget.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.budget.crossLinks.length)}>
                {page.budget.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="tax"
              tipsKey="tax"
              visual={page.visuals.tax}
              intro={
                <SectionIntro eyebrow="Tax orientation" title={page.tax.heading} fullWidth>
                  <p>{page.tax.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Tax orientation habits" items={page.tax.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.tax.cards.length)}>
                {page.tax.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.tax.crossLinks.length)}>
                {page.tax.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="buffers"
              tipsKey="buffers"
              visual={page.visuals.buffers}
              intro={
                <SectionIntro eyebrow="Buffers" title={page.buffers.heading} fullWidth>
                  <p>{page.buffers.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Buffer habits" items={page.buffers.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.buffers.cards.length)}>
                {page.buffers.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.buffers.crossLinks.length)}>
                {page.buffers.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            >
              <ScenarioTable title="Money setup stories" rows={page.scenarios.rows} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Full list" title={page.checklist.heading} fullWidth>
                  <p>{page.checklist.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Financial checklist board" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Follow these orientation steps to sequence money ops — then verify bank, insurer and tax rules with official sources.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Tools" title={page.tools.heading} fullWidth>
                  <p>Use tools while assumptions settle — then verify live banking, insurance and budget numbers.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about money setup in the first 30 and 90 days.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.intro}
              intro={
                <SectionIntro eyebrow="Related" title="Related money and moving guides" fullWidth>
                  <p>Cluster siblings, banking/tax guides, and moving boundary peers stay linkable without comingSoon labels.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Money hub and cluster cards" fullWidth>
                  <p>Return to the Money hub or jump to Cheap cities, Hidden costs, Banking and Tax guides.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next money step" tone="onDark" fullWidth>
                  <p>Size surprise lines, compare place stacks, or deepen banking and insurance next.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm banking, insurance and tax steps with current official sources — this page is orientation only.</p>
                </SectionIntro>
