...
import {
  ...
  MONEY_HUB_PATH,
  costOfLivingNetherlandsPage as page,
  type AffiliateBlockConfig,
...
const snapshotIcons = [Home, ShieldCheck, Calculator, Wallet] as const;
const orientationIcons = [Home, ShieldCheck, ShoppingCart, Calculator] as const;
...
const cityBandColumns = [
  { key: "city", label: "City frame" },
  { key: "singleModest", label: "Single · modest (planning)" },
  { key: "coupleComfortable", label: "Couple · comfortable (planning)" },
  { key: "note", label: "Note" },
] as const;
...
function SectionAffiliateBlock({ block }: { block: AffiliateBlockConfig }) {
  return (
    <BankingRecommendedOptionsSection
      placementId={block.placementId}
      analyticsPageContext={block.analyticsPageContext}
      boundaryNote={block.boundaryNote}
      categoryLinks={[...block.categoryLinks]}
      browseLabel={block.browseLabel}
      utmReferrerPath={COST_OF_LIVING_NETHERLANDS_PATH}
      surfaceTone="muted"
    />
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Dutch cost planning</h3>
...
export function CostOfLivingNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Cost-of-living planning file — keep these together" items={page.safetyFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Cost of living at a glance" fullWidth>
                  <p>Six cards summarise what usually moves a Dutch monthly total. Deeper sections expand each driver with indicative 2026 planning bands.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              id="drivers"
              tipsKey="drivers"
              visual={page.visuals.drivers}
              intro={
                <SectionIntro eyebrow="Drivers" title={page.drivers.heading} fullWidth>
                  <p>{page.drivers.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Driver stack" items={page.drivers.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.drivers.cards.length)}>
                {page.drivers.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.drivers.crossLinks.length)}>
                {page.drivers.crossLinks.map((item, idx) => (
            ...
            >
              ...
              <InfoTable
                columns={costColumns}
                rows={page.housing.indicativeRows.map((row) => ({
                  setting: row.setting,
                  band: row.band,
                  tip: row.tip,
                }))}
              />
              <WarningPanel title="Housing figures are planning ranges" items={page.housing.warningItems} />
              <div className={guidePremiumCardGridClass(page.housing.crossLinks.length)}>
                {page.housing.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
              <SectionAffiliateBlock block={page.housing.affiliateBlock} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Insurance" title={page.insurance.heading} fullWidth>
                  <p>{page.insurance.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Insurance habits" items={page.insurance.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.insurance.cards.length)}>
                {page.insurance.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <InfoTable
              ...
              />
              <div className={guidePremiumCardGridClass(page.insurance.crossLinks.length)}>
                {page.insurance.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
              <SectionAffiliateBlock block={page.insurance.affiliateBlock} />
            </PremiumGuideSection>
            ...
            >
              <ChecklistBlock title="Food & shopping habits" items={page.groceries.bullets} columns={2} />
              ...
              <InfoTable
                columns={costColumns}
                rows={page.groceries.indicativeRows.map((row) => ({
                  setting: row.setting,
                  band: row.band,
                  tip: row.tip,
                }))}
              ...
              <SectionAffiliateBlock block={page.groceries.affiliateBlock} />
            </PremiumGuideSection>
            ...
            >
              <ChecklistBlock title="Getting-around habits" items={page.transport.bullets} columns={2} />
              ...
              <InfoTable
                columns={costColumns}
                rows={page.transport.indicativeRows.map((row) => ({
                  setting: row.setting,
                  band: row.band,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.transport.crossLinks.length)}>
                {page.transport.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
              <SectionAffiliateBlock block={page.transport.affiliateBlock} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Childcare" title={page.childcare.heading} fullWidth>
                  <p>{page.childcare.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Family cost habits" items={page.childcare.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.childcare.cards.length)}>
                {page.childcare.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <InfoTable
                columns={costColumns}
                rows={page.childcare.indicativeRows.map((row) => ({
                  setting: row.setting,
                  band: row.band,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.childcare.crossLinks.length)}>
                {page.childcare.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
              <SectionAffiliateBlock block={page.childcare.affiliateBlock} />
            </PremiumGuideSection>

            <section id="banking-setup" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Banking" title={page.bankingSetup.heading} fullWidth>
                  <p>{page.bankingSetup.lead}</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
                <SectionAffiliateBlock block={page.bankingSetup.affiliateBlock} />
              </div>
            </section>
            ...
            <PremiumGuideSection
              id="city-bands"
              tipsKey="cityBands"
              visual={page.visuals.cityBands}
              intro={
                <SectionIntro eyebrow="City bands" title={page.cityBands.heading} fullWidth>
                  <p>{page.cityBands.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="City framing habits" items={page.cityBands.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.cityBands.cards.length)}>
                {page.cityBands.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <InfoTable
                columns={cityBandColumns}
                rows={page.cityBands.rows.map((row) => ({
                  city: row.city,
                  singleModest: row.singleModest,
                  coupleComfortable: row.coupleComfortable,
                  note: row.note,
                }))}
              />
              <WarningPanel
                title="Bands are orientation only"
                items={[
                  "Not quotes, guarantees or landlord offers",
                  "Always match household size and housing type",
                  "Re-check with live listings and the calculator",
                ]}
              />
              <div className={guidePremiumCardGridClass(page.cityBands.crossLinks.length)}>
                {page.cityBands.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="how-to-read"
              tipsKey="howToRead"
              visual={page.visuals.howToRead}
              intro={
                <SectionIntro eyebrow="How to read" title={page.howToRead.heading} fullWidth>
                  <p>{page.howToRead.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Reading rules" items={page.howToRead.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.howToRead.cards.length)}>
                {page.howToRead.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <Timeline steps={page.howToRead.steps} eyebrow="Step" />
              <div className={guidePremiumCardGridClass(page.howToRead.crossLinks.length)}>
                {page.howToRead.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="calculator"
              tipsKey="calculator"
              visual={page.visuals.calculator}
              intro={
                <SectionIntro eyebrow="Calculator" title={page.calculator.heading} fullWidth>
                  <p>{page.calculator.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Calculator habits" items={page.calculator.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.calculator.cards.length)}>
                {page.calculator.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className="rounded-3xl border border-brand/20 bg-gradient-to-br from-copilot-bg-soft via-white to-cyan-50/60 p-6 shadow-card ring-1 ring-copilot-primary/10">
                <p className="text-xs font-bold uppercase tracking-[0.14em] text-brand-strong">Primary tool handoff</p>
                <h3 className="mt-2 text-2xl font-bold tracking-tight text-foreground">Model your monthly bands</h3>
                <p className="mt-2 max-w-2xl text-sm leading-relaxed text-foreground-muted">
                  Open the interactive cost-of-living calculator with the same city and household assumptions you used on this page.
                </p>
                <Link href={COST_OF_LIVING_CALCULATOR_PATH} className={cn(primaryCtaClass, "mt-5")}>
                  Open cost of living calculator
                  <ArrowRight className="h-4 w-4" aria-hidden />
                </Link>
              </div>
              <div className={guidePremiumCardGridClass(page.calculator.crossLinks.length)}>
                {page.calculator.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            >
              <ScenarioTable title="Cost-of-living stories" rows={page.scenarios.rows} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title={page.mistakes.heading} fullWidth>
                  <p>These patterns create avoidable planning friction — calm housing-led checks beat optimistic listicle averages.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Ready to plan" title={page.checklist.heading} fullWidth>
                  <p>{page.checklist.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Cost-of-living readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about Dutch living costs, city bands and how to use planning figures.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related money & living guides" fullWidth>
                  <p>Cluster siblings, the calculator, housing, insurance and banking plumbing that sit beside cost-of-living planning.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Money hub and cluster cards" fullWidth>
                  <p>Return to the Money hub or jump to the calculator, budget sibling, saving sibling and banking hub.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next money step" tone="onDark" fullWidth>
                  <p>Model estimates in the calculator, template a month, find saving levers, check net pay, or ground your housing stack.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm live prices and rules with landlords, insurers and official sources — bands on this page are planning orientation only.</p>
                </SectionIntro>
