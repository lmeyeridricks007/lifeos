...
import {
  CREDIT_CARDS_PATH,
  creditCardsPage as page,
  type CreditCardsLink,
  type MistakeCard as MistakeCardData,
} from "./creditCardsPageModel";
...
const snapshotIcons = [CreditCard, WalletCards, Landmark, ShieldCheck] as const;
const orientationIcons = [Store, CreditCard, ClipboardList, WalletCards] as const;
...
function LinkCard({ item, iconIndex = 0, tone = "default" }: { item: CreditCardsLink; iconIndex?: number; tone?: "default" | "onDark" }) {
  const Icon = iconPool[iconIndex % iconPool.length];
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for Dutch credit</h3>
    ...
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Credit cards", item: new URL(CREDIT_CARDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Credit readiness file — keep these together" items={page.safetyFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Credit cards at a glance" fullWidth>
                  <p>Six cards cover how credit fits Dutch payment culture. Deeper sections expand each theme without ranking issuers.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Culture fit" title={page.culture.heading} fullWidth>
                  <p>{page.culture.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Culture-fit habits" items={page.culture.bullets} columns={2} />
            ...
            <PremiumGuideSection
              id="availability"
              tipsKey="availability"
              visual={page.visuals.availability}
              intro={
                <SectionIntro eyebrow="Availability" title={page.availability.heading} fullWidth>
                  <p>{page.availability.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="What expats usually need to know" items={page.availability.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.availability.cards.length)}>
                {page.availability.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <Timeline steps={page.availability.timeline} eyebrow="Availability" />
              <div className={guidePremiumCardGridClass(page.availability.crossLinks.length)}>
                {page.availability.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="when-useful"
              tipsKey="whenUseful"
              visual={page.visuals.whenUseful}
              intro={
                <SectionIntro eyebrow="When useful" title={page.whenUseful.heading} fullWidth>
                  <p>{page.whenUseful.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Where credit often helps" items={page.whenUseful.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.whenUseful.cards.length)}>
                {page.whenUseful.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <InfoTable
                columns={compareColumns}
                rows={page.whenUseful.rows.map((row) => ({
                  setting: row.setting,
                  debit: row.debit,
                  credit: row.credit,
                  tip: row.tip,
              ...
              />
              <div className={guidePremiumCardGridClass(page.whenUseful.crossLinks.length)}>
                {page.whenUseful.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="fees"
              tipsKey="fees"
              visual={page.visuals.fees}
              intro={
                <SectionIntro eyebrow="Fees" title={page.fees.heading} fullWidth>
                  <p>{page.fees.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="What to verify with issuers" items={page.fees.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.fees.cards.length)}>
                {page.fees.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <InfoTable
                columns={compareColumns}
                rows={page.fees.indicativeRows.map((row) => ({
                  setting: row.setting,
                  debit: row.debit,
                  credit: row.credit,
                  tip: row.tip,
              ...
              />
              <WarningPanel title="Fees are orientation only" items={page.fees.warningItems} />
              <div className={guidePremiumCardGridClass(page.fees.crossLinks.length)}>
                {page.fees.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="vs-debit"
              tipsKey="vsDebit"
              visual={page.visuals.vsDebit}
              intro={
                <SectionIntro eyebrow="Vs debit" title={page.vsDebit.heading} fullWidth>
                  <p>{page.vsDebit.lead}</p>
                </SectionIntro>
              }
            >
              <InfoTable
                columns={compareColumns}
                rows={page.vsDebit.rows.map((row) => ({
                  setting: row.setting,
                  debit: row.debit,
                  credit: row.credit,
                  tip: row.tip,
              ...
              />
              <div className={guidePremiumCardGridClass(page.vsDebit.crossLinks.length)}>
                {page.vsDebit.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="acceptance"
              tipsKey="acceptance"
              visual={page.visuals.acceptance}
              intro={
                <SectionIntro eyebrow="Acceptance" title={page.acceptance.heading} fullWidth>
                  <p>{page.acceptance.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Acceptance habits" items={page.acceptance.bullets} columns={2} />
              <div className={guidePremiumCardGridClass(page.acceptance.cards.length)}>
                {page.acceptance.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.acceptance.crossLinks.length)}>
                {page.acceptance.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            >
              <ScenarioTable title="Tourist and expat credit stories" rows={page.scenarios.rows} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title={page.mistakes.heading} fullWidth>
                  <p>These patterns create avoidable credit friction — calm backups beat awkward declines.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Ready for credit" title={page.checklist.heading} fullWidth>
                  <p>Work through this list before you apply for or rely on a credit card in Dutch daily life.</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Credit readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Tools" title={page.tools.heading} fullWidth>
                  <p>Use tools for account and cost planning — then verify credit products and fees on each issuer’s official pages.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about credit cards in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related banking guides" fullWidth>
                  <p>Live cluster siblings stay linkable — Debit cards, Cash vs card and Wise vs Revolut.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Banking hub and cluster cards" fullWidth>
                  <p>Return to the Banking hub or jump to Debit cards, Cash vs card, payments rails, and live cluster peers.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next money step" tone="onDark" fullWidth>
                  <p>Master debit culture, till cash pockets, payment rails, open an account, or shortlist banks.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm acceptance habits and fees with merchants and banks — rules change.</p>
                </SectionIntro>
