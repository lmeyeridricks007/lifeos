...
import {
  ...
  monthlyBudgetNetherlandsPage as page,
  type MonthlyBudgetLink,
  type MistakeCard as MistakeCardData,
} from "./monthlyBudgetNetherlandsPageModel";
...
const costColumns = [
  { key: "setting", label: "Category line" },
...
const phaseColumns = [
  ...
  { key: "steadyState", label: "Steady-state" },
  { key: "tip", label: "Tip" },
] as const;

const exampleMonthColumns = [
  { key: "category", label: "Category" },
  { key: "singleBand", label: "Single · modest (planning)" },
  { key: "coupleBand", label: "Couple · comfortable (planning)" },
  { key: "note", label: "Note" },
] as const;
...
function LinkCard({ item, iconIndex = 0, tone = "default" }: { item: MonthlyBudgetLink; iconIndex?: number; tone?: "default" | "onDark" }) {
  const Icon = iconPool[iconIndex % iconPool.length];
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits for a Dutch monthly budget</h3>
...
export function MonthlyBudgetNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Monthly budget", item: new URL(MONTHLY_BUDGET_NETHERLANDS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Monthly budget planning file — keep these together" items={page.safetyFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Monthly budget at a glance" fullWidth>
                  <p>Six cards summarise the template: categories, first-month cash, workflow order and tool handoffs. Deeper sections expand each with indicative 2026 planning bands.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Template" title={page.categories.heading} fullWidth>
                  <p>{page.categories.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Category lines" items={page.categories.bullets} columns={2} />
              <InfoTable
                columns={costColumns}
                rows={page.categories.indicativeRows.map((row) => ({
                  setting: row.setting,
                  band: row.band,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.categories.cards.length)}>
                {page.categories.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.categories.crossLinks.length)}>
                {page.categories.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="first-month"
              tipsKey="firstMonth"
              visual={page.visuals.firstMonth}
              intro={
                <SectionIntro eyebrow="Phases" title={page.firstMonth.heading} fullWidth>
                  <p>{page.firstMonth.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="First-month awareness" items={page.firstMonth.bullets} columns={2} />
              <InfoTable
                columns={phaseColumns}
                rows={page.firstMonth.phaseRows.map((row) => ({
                  line: row.line,
                  firstMonth: row.firstMonth,
                  steadyState: row.steadyState,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.firstMonth.cards.length)}>
                {page.firstMonth.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <WarningPanel title="Keep cash events labelled" items={page.firstMonth.warningItems} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="workflow"
              tipsKey="workflow"
              visual={page.visuals.workflow}
              intro={
                <SectionIntro eyebrow="Workflow" title={page.workflow.heading} fullWidth>
                  <p>{page.workflow.lead}</p>
                </SectionIntro>
              }
            >
              <Timeline steps={page.workflow.steps} eyebrow="Step" />
              <div className={guidePremiumCardGridClass(page.workflow.cards.length)}>
                {page.workflow.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.workflow.crossLinks.length)}>
                {page.workflow.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="example-month"
              tipsKey="exampleMonth"
              visual={page.visuals.exampleMonth}
              intro={
                <SectionIntro eyebrow="Examples" title={page.exampleMonth.heading} fullWidth>
                  <p>{page.exampleMonth.lead}</p>
                </SectionIntro>
              }
            >
              <InfoTable
                columns={exampleMonthColumns}
                rows={page.exampleMonth.rows.map((row) => ({
                  category: row.category,
                  singleBand: row.singleBand,
                  coupleBand: row.coupleBand,
                  note: row.note,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.exampleMonth.cards.length)}>
                {page.exampleMonth.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <WarningPanel title="How to read these sketches" items={page.exampleMonth.warningItems} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Tools" title={page.calculator.heading} fullWidth>
                  <p>{page.calculator.lead}</p>
                </SectionIntro>
              }
            >
              <ChecklistBlock title="Who owns what" items={page.calculator.bullets} columns={2} />
              <div className="rounded-3xl border border-brand/15 bg-gradient-to-br from-copilot-bg-soft via-white to-cyan-50/50 p-5 ring-1 ring-copilot-primary/10">
                <p className="text-sm font-bold text-foreground">Ready for interactive estimates?</p>
                <p className="mt-2 text-sm leading-relaxed text-foreground-muted">
                  Structure the month here, then model city and household totals in the cost-of-living calculator — keep assumptions matched.
                </p>
            ...
            >
              <ScenarioTable title="Budget stories" rows={page.scenarios.rows} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title={page.mistakes.heading} fullWidth>
                  <p>These patterns create avoidable planning friction — a calm net-pay template beats optimistic listicle averages.</p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Monthly budget readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about Dutch monthly budgets, first-month cash and how to use planning figures.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Related" title="Related money & living guides" fullWidth>
                  <p>Cluster siblings, the calculator, housing, insurance and banking plumbing that sit beside monthly budget planning.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Hub" title="Money hub and cluster cards" fullWidth>
                  <p>Return to the Money hub or jump to cost of living, saving, the calculator and banking hub.</p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next money step" tone="onDark" fullWidth>
                  <p>Model estimates in the calculator, read cost drivers, find saving levers, check net pay, or ground your housing line.</p>
                </SectionIntro>
              }
            >
