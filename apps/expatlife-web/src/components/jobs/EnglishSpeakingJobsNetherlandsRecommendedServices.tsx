...
import { englishSpeakingJobsNetherlandsPage as page, ENGLISH_SPEAKING_JOBS_NETHERLANDS_PATH } from "./englishSpeakingJobsNetherlandsPageModel";
...
function RecommendedProviderCard({ service }: { service: PageRecommendedProviderCard }) {
  ...
  if (external) {
    const outbound = buildTrackedOutboundLink(service.url, {
      pagePath: ENGLISH_SPEAKING_JOBS_NETHERLANDS_PATH,
...
export function EnglishSpeakingJobsNetherlandsRecommendedServices() {
  ...
  return (
    <div className="space-y-8">
      ...
      <ProviderCardSection
        ...
        description="Useful when work-permit routes, recognised sponsors or relocation timing still need scoped professional help alongside official IND guidance — not as a English-employer ranking."
        cards={visaPermits}
      />
      <ProviderCardSection
        eyebrow="Relocation"
        title="Relocation support when English-role offers lead to a move"
      ...
      <AffiliateDisclosure
        variant="copilot"
        text="ExpatCopilot may earn a commission from some partners. Listings here support job-search and relocation planning — not pay-to-rank English employers or job boards and not job or visa guarantees. Confirm scope, credentials and pricing with any provider."
      />
    </div>
  );
}
