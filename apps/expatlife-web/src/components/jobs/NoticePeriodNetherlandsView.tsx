...
import {
  NoticePeriodHsmAffiliateSupport,
  NoticePeriodJobChangeAffiliateSupport,
  NoticePeriodMortgageAffiliateSupport,
  NoticePeriodNetherlandsRecommendedServices,
  NoticePeriodResignationAffiliateSupport,
  NoticePeriodTaxAffiliateSupport,
  NoticePeriodTransitionAffiliateSupport,
} from "./NoticePeriodNetherlandsRecommendedServices";
import {
...
function LinkCard({ item, iconIndex = 0, tone = "default" }: { item: NoticePeriodLink; iconIndex?: number; tone?: "default" | "onDark" }) {
  const Icon = cardIcons[iconIndex % cardIcons.length];
  ...
  return (
    <aside className="relative flex flex-col overflow-hidden rounded-3xl bg-slate-950 p-5 text-white shadow-expatos-xl ring-1 ring-black/20 sm:p-6">
      ...
      <div className="relative flex flex-col">
        ...
        <h3 className="mt-2 text-lg font-bold tracking-tight">Connect notice periods to your wider career picture</h3>
        <p className="mt-3 text-sm leading-relaxed text-slate-300">
          Notice timing sits inside your employment agreement — use these guides alongside HR confirmation, not instead of reading your contract.
        </p>
...
function NoticeTransitionFlowBand({ className }: { className?: string }) {
  const items = [
    { label: "Find notice clauses", body: "Locate employee notice, employer notice and any probation-specific notice in your contract.", Icon: FileText },
    { label: "Confirm last working day", body: "Ask HR in writing how notice is calculated — month-end rules vary by employer.", Icon: MessageCircle },
    { label: "Align your next steps", body: "Model visa, housing and new-role start dates against realistic notice end dates.", Icon: Clock },
  ] as const;
  ...
  return (
    <aside className={cn("relative w-full overflow-hidden rounded-3xl bg-slate-950 p-5 text-white shadow-expatos-xl ring-1 ring-black/20 sm:p-6", className)}>
      ...
      <div className="relative">
        <p className="text-xs font-bold uppercase tracking-[0.14em] text-cyan-200">Transition flow</p>
        <h3 className="mt-2 text-xl font-bold tracking-tight">Three moves that reduce notice-period uncertainty</h3>
...
export function NoticePeriodNetherlandsView() {
  const breadcrumbCrumbs = [
    { name: "Home", item: new URL("/", baseUrl).toString() },
    { name: "Netherlands", item: new URL("/netherlands/", baseUrl).toString() },
    { name: "Jobs", item: new URL(JOBS_HUB_PATH, baseUrl).toString() },
    { name: "Notice period", item: new URL(page.path, baseUrl).toString() },
  ];
      ...
      <main className={sitePageCanvasClass}>
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
              ...
              <div className="grid gap-8 lg:grid-cols-[minmax(0,0.95fr)_minmax(360px,0.85fr)] lg:items-center">
                <div>
                  <nav aria-label="Breadcrumb" className="flex flex-wrap gap-2 text-xs text-foreground-muted">
                    ...
                    <span aria-hidden>/</span>
                    <span className="text-foreground" aria-current="page">Notice period</span>
                  </nav>
          ...
          <div className="sticky top-3 z-20 mt-6 overflow-x-auto rounded-2xl border border-slate-200/80 bg-white/90 p-2 shadow-card ring-1 ring-slate-900/[0.04] backdrop-blur-xl">
            <nav aria-label="Notice period guide sections" className="flex min-w-max gap-2">
          ...
          <div className={sectionStackClass}>
            <section id="intro" className={sectionClass}>
              ...
              <div className="mt-6">
                <SectionIntro eyebrow="Key points" title="What to know about notice periods" />
              ...
              <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.72fr)] lg:items-start">
                <ChecklistBlock title="Three planning moves before you resign or accept a start date" items={page.introPlanningSteps} columns={2} />
                <NoticePlanningPanel />
              </div>
              <NoticeTransitionFlowBand className="mt-6" />
              <ScenarioTable title="When notice periods affect real plans" rows={page.intro.scenarios} className="mt-6" />
              <NoticePeriodTransitionAffiliateSupport />
              <VisualFigure visual={page.visuals.intro} className={sectionVisualMtClass} />
            </section>

            <section id="snapshot" className={sectionClass}>
              <SectionIntro eyebrow="At a glance" title="Notice Periods at a Glance">
                <p>Practical orientation on contract-defined notice, job-change timing and expat planning before you resign or negotiate a start date.</p>
              </SectionIntro>
              ...
              <div className="mt-8">
                <SectionIntro eyebrow="Timing comparison" title={page.noticeTimingHeading} fullWidth>
                  {page.noticeTimingParagraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
                </SectionIntro>
                <div className="mt-4 w-full">
                  <InfoTable
                    rows={page.noticeTimingRows.map((row) => ({
                      label: row.label,
              ...
              <div className="mt-8">
                <SectionIntro eyebrow="Worked examples" title={page.noticeWorkedExamplesHeading} fullWidth>
                  {page.noticeWorkedExamplesParagraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
                </SectionIntro>
                <div className="mt-4 w-full">
                  <InfoTable
                    rows={page.noticeWorkedExamples.map((row) => ({
                      profile: row.profile,
                      situation: row.situation,
                      likelyLastDay: row.likelyLastDay,
                      whatToConfirm: row.whatToConfirm,
                    }))}
                    columns={[
                      { key: "profile", label: "Profile" },
                      { key: "situation", label: "Situation" },
                      { key: "likelyLastDay", label: "Likely last day" },
                      { key: "whatToConfirm", label: "Confirm with HR" },
                    ]}
                  />
                </div>
              </div>
              <ScenarioTable title="Notice timing examples expats often see" rows={page.snapshotScenarios} className="mt-8" />
            ...
            <section id="why-notice" className={sectionClass}>
              <SectionIntro eyebrow="Why notice" title={page.whyNoticeHeading} fullWidth>
                {page.whyNoticeParagraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
              </SectionIntro>
              <div className="mt-6 grid gap-6 lg:grid-cols-2">
                <BulletPanel title="Why employers use notice periods" items={page.whyNoticeEmployerPoints} />
                <BulletPanel title="Why employees benefit from notice" items={page.whyNoticeEmployeePoints} />
              </div>
              <ChecklistBlock title="Use notice as planning time — not a trap" items={page.whyNoticeChecklist} columns={2} className="mt-6" />
              <ScenarioTable title="When notice periods support smoother transitions" rows={page.whyNoticeScenarios} className="mt-8" />
              <VisualFigure visual={page.visuals.whyNotice} className={sectionVisualMtClass} />
            </section>

            <section id="employee-vs-employer" className={sectionClass}>
              <div className="grid w-full gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.8fr)] lg:items-start lg:gap-8">
                <div className="flex min-w-0 flex-col gap-6">
                  <SectionIntro eyebrow="Employee vs employer" title={page.employeeVsEmployerHeading} fullWidth>
                    {page.employeeVsEmployerParagraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
                  </SectionIntro>
                  <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2")}>
                    {page.employeeVsEmployerCards.map((item, idx) => (
                      <FeatureCard key={item.title} title={item.title} body={item.body} iconIndex={idx} />
                    ))}
                  </div>
                  <VisualFigure visual={page.visuals.employeeVsEmployer} className="lg:hidden" />
                </div>
                <VisualFigure visual={page.visuals.employeeVsEmployer} className="hidden lg:block lg:sticky lg:top-24" />
              </div>
              <div className="mt-8 w-full">
                <InfoTable
                  rows={page.noticeObligationRows.map((row) => ({
                    topic: row.topic,
                    employeeNotice: row.employeeNotice,
                    employerNotice: row.employerNotice,
                  }))}
                  columns={[
                    { key: "topic", label: "Topic" },
                    { key: "employeeNotice", label: "Employee context" },
                    { key: "employerNotice", label: "Employer context" },
                  ]}
                />
              </div>
              <ChecklistBlock title="Compare both notice clauses before you act" items={page.employeeVsEmployerChecklist} columns={2} className="mt-6" />
              <ScenarioTable title="Employee vs employer notice examples" rows={page.employeeVsEmployerScenarios} className="mt-8" />
            </section>

            <section id="contracts" className={sectionClass}>
              <div className="grid w-full gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.8fr)] lg:items-start lg:gap-8">
                <div className="flex min-w-0 flex-col gap-6">
                  <SectionIntro eyebrow="Contracts" title={page.contractsHeading} fullWidth>
                    {page.contractsParagraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
                  </SectionIntro>
                  <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2")}>
                    {page.contractsClauseCards.map((item, idx) => (
                      <FeatureCard key={item.title} title={item.title} body={item.body} iconIndex={idx} />
                    ))}
                  </div>
                  <GuideLinkAside
                    title="Employment contract guide"
                    description="Read notice alongside proeftijd, contract duration and salary lines before you sign or plan a transition."
                    href={EMPLOYMENT_CONTRACT_NETHERLANDS_PATH}
                    linkLabel="Open employment contract guide"
                  />
                  <VisualFigure visual={page.visuals.contracts} className="lg:hidden" />
                </div>
                <VisualFigure visual={page.visuals.contracts} className="hidden lg:block lg:sticky lg:top-24" />
              </div>
              <ProcessPanel
                className="mt-8"
                eyebrow="Contract review"
                title="Four steps when reading notice clauses"
                rows={page.contractsReviewFlow.map((item, idx) => ({
                  label: item.title,
                  body: item.body,
                  Icon: [FileText, Clock, MessageCircle, ShieldCheck][idx % 4],
                }))}
                rowsLayout="wide"
                note="These steps support orientation only — your contract, CAO and employer policy determine exact notice rules."
              />
              <ScenarioTable title="Contract and notice review examples" rows={page.contractsScenarios} className="mt-8 w-full" />
              <div className="mt-8 grid gap-6 lg:grid-cols-2">
                <BulletPanel title="Where to find notice in your contract" items={page.contractsPoints} />
                <ChecklistBlock title="Contract review tips" items={page.contractsReviewTips} columns={1} />
              </div>
              <NoticePeriodTaxAffiliateSupport />
            </section>

            <section id="probation-vs-notice" className={sectionClass}>
              <div className="grid w-full gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.8fr)] lg:items-start lg:gap-8">
                <div className="flex min-w-0 flex-col gap-6">
                  <SectionIntro eyebrow="Probation vs notice" title={page.probationVsNoticeHeading} fullWidth>
                    {page.probationVsNoticeParagraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
                  </SectionIntro>
                  <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2")}>
                    {page.probationVsNoticeCards.map((item, idx) => (
                      <FeatureCard key={item.title} title={item.title} body={item.body} iconIndex={idx} />
                    ))}
                  </div>
                  <GuideLinkAside
                    title="Probation period guide"
                    description="Proeftijd applies at employment start — read it separately from notice and resignation timing."
                    href={PROBATION_PERIOD_NETHERLANDS_PATH}
                    linkLabel="Open probation period guide"
                  />
                  <VisualFigure visual={page.visuals.probationVsNotice} className="lg:hidden" />
                </div>
                <VisualFigure visual={page.visuals.probationVsNotice} className="hidden lg:block lg:sticky lg:top-24" />
              </div>
              <div className="mt-8 w-full">
                <InfoTable
                  rows={page.probationVsNoticeRows.map((row) => ({
                    label: row.label,
                  ...
                  columns={[
                    { key: "label", label: "Topic" },
                    { key: "probationPeriod", label: "Probation (proeftijd)" },
                    { key: "noticePeriod", label: "Notice period" },
                  ]}
                />
              </div>
              <ScenarioTable title="Probation vs notice — practical examples" rows={page.probationVsNoticeScenarios} className="mt-8" />
            </section>

            <section id="changing-jobs" className={sectionClass}>
              <SectionIntro eyebrow="Job changes" title={page.changingJobsHeading} fullWidth>
                {page.changingJobsParagraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
              </SectionIntro>
              <BulletPanel title="How notice affects employer changes" items={page.changingJobsPoints} />
              <ChecklistBlock title="Changing jobs checklist" items={page.changingJobsChecklist} columns={2} className="mt-6" />
              <ScenarioTable title="Job-change scenarios expats often navigate" rows={page.changingJobsScenarios} className="mt-8" />
              <NoticePeriodJobChangeAffiliateSupport />
              <VisualFigure visual={page.visuals.changingJobs} className={sectionVisualMtClass} />
            </section>
            ...
            <section id="hsm" className={sectionClass}>
              ...
              <div className="mt-6 grid gap-6 lg:grid-cols-2">
                <BulletPanel title="Notice points for sponsored routes" items={page.hsmPoints} />
                <GuideLinkAside
              ...
              <ScenarioTable title="Highly skilled migrant notice scenarios" rows={page.hsmScenarios} className="mt-6" />
              <ChecklistBlock title="HSM and notice checklist" items={page.hsmChecklist} columns={2} className="mt-6" />
              <NoticePeriodHsmAffiliateSupport />
              <VisualFigure visual={page.visuals.hsm} className={sectionVisualMtClass} />
            </section>

            <section id="international-relocation" className={sectionClass}>
              <SectionIntro eyebrow="Leaving the Netherlands" title={page.internationalRelocationHeading} fullWidth>
                {page.internationalRelocationParagraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
              </SectionIntro>
              <BulletPanel title="Coordinate notice with your move plan" items={page.internationalRelocationPoints} />
              <ScenarioTable title="International relocation and notice examples" rows={page.internationalRelocationScenarios} className="mt-8" />
              <NoticePeriodTransitionAffiliateSupport />
              <VisualFigure visual={page.visuals.internationalRelocation} className={sectionVisualMtClass} />
            </section>

            <section id="mortgages" className={sectionClass}>
              <SectionIntro eyebrow="Financial planning" title={page.mortgagesHeading} fullWidth>
                {page.mortgagesParagraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
              </SectionIntro>
              <div className="mt-6 grid gap-6 lg:grid-cols-2">
                <BulletPanel title="Career transitions and financial planning" items={page.mortgagesPoints} />
                <GuideLinkAside
                  title="Mortgages for expats"
                  description="Lender policy varies by employment continuity and contract type — confirm requirements with a mortgage adviser."
                  href={MORTGAGES_NETHERLANDS_EXPATS_PATH}
                  linkLabel="Open mortgages for expats guide"
                />
              </div>
              <ScenarioTable title="Notice, job changes and mortgage scenarios" rows={page.mortgageScenarios} className="mt-6" />
              <ChecklistBlock title="Financial planning during transitions" items={page.mortgagePlanningTips} columns={2} className="mt-6" />
              <NoticePeriodMortgageAffiliateSupport />
            ...
            <section id="misconceptions" className={sectionClass}>
              <SectionIntro eyebrow="Myths" title="Common Notice Period Myths">
                <p>These myths often create avoidable stress when expats plan resignations or job changes in the Netherlands.</p>
              </SectionIntro>
              <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <ChecklistBlock title="Reality check before you act" items={page.mythRealityChecks} columns={2} className="mt-6" />
            ...
            <section id="transition-tips" className={sectionClass}>
              <SectionIntro eyebrow="Transition tips" title="Managing Job Transitions Smoothly">
                <p>Practical habits that reduce uncertainty when notice periods intersect with your next role or relocation.</p>
              </SectionIntro>
              <ProcessPanel
                eyebrow="Action checklist"
                title="Eight habits that help during employment transitions"
                rows={page.transitionTips.map((item, idx) => ({
                  label: item.title,
                  body: item.body,
                ...
                rowsLayout="wide"
              />
              <ScenarioTable title="Transition tips in real situations" rows={page.transitionScenarios} className="mt-8" />
              <VisualFigure visual={page.visuals.transitionTips} className={sectionVisualMtClass} />
            </section>

            <section id="mistakes" className={sectionClass}>
              <SectionIntro eyebrow="Avoidable mistakes" title="Mistakes Expats Commonly Make">
                <p>Use these cards to spot patterns that create avoidable stress around notice and job changes.</p>
              </SectionIntro>
            ...
            <section id="questions" className={sectionClass}>
              <SectionIntro eyebrow="Ask in writing" title="Questions to Ask HR and Your Manager">
                <p>Use these prompts before resigning or accepting a start date — written answers reduce uncertainty more than verbal assurances alone. For quick orientation answers, see the cards below.</p>
              </SectionIntro>
            ...
            <section id="related-guides" className={sectionClass}>
              <SectionIntro eyebrow="Career guides" title="Related Career Guides">
                <p>Connect notice planning to contracts, probation, job search, salary benchmarks and permit routes.</p>
              </SectionIntro>
              <div className={cn(sectionStackClass, "grid gap-4 sm:grid-cols-2 xl:grid-cols-3")}>
              ...
              <ChecklistBlock
                ...
                items={[
                  "Start with the employment contract guide if notice, proeftijd and duration still need review.",
                  "Open the probation period guide when you are unsure whether notice rules differ during proeftijd.",
                  "Use the finding jobs and highly skilled migrant guides when notice timing affects search or permit planning.",
                ]}
                columns={2}
                className="mt-6"
              />
            ...
            <section id="services" className={sectionClass}>
              ...
              <ScenarioTable title="When expats typically seek support during job transitions" rows={page.serviceScenarios} className="mt-8" />
            ...
            <section id="faq" className={sectionClass}>
              <SectionIntro eyebrow="FAQ" title="Frequently Asked Questions">
                <p>These answers help you identify what still needs verification — notice timing, job changes, visas and contracts.</p>
              </SectionIntro>
            ...
            >
              ...
              <div className="mt-6">
                <SectionIntro eyebrow="Explore next" title="Plan the Next Step" tone="onDark">
                  <p>Move from notice orientation into contracts, probation, salary guides, mortgage planning and visa routes.</p>
                </SectionIntro>
