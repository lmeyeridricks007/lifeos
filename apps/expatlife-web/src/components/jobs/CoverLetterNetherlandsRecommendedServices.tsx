...
import { coverLetterNetherlandsPage as page, COVER_LETTER_NETHERLANDS_PATH } from "./coverLetterNetherlandsPageModel";
...
function RecommendedProviderCard({ service }: { service: PageRecommendedProviderCard }) {
  ...
  if (external) {
    const outbound = buildTrackedOutboundLink(service.url, {
      pagePath: COVER_LETTER_NETHERLANDS_PATH,
...
export function CoverLetterNetherlandsRecommendedServices() {
  ...
  return (
    <div className="space-y-8">
      ...
      <ProviderCardSection
        ...
        title="Immigration support when applications depend on sponsorship"
        description="Useful when work-permit routes, recognised sponsors or relocation timing still need scoped professional help alongside official IND guidance — not as a cover-letter writing service."
        cards={visaPermits}
      />
      <ProviderCardSection
        eyebrow="Relocation"
        title="Relocation support when interviews lead to a move"
      ...
      <AffiliateDisclosure
        variant="copilot"
        text="ExpatCopilot may earn a commission from some partners. Listings here support application and relocation planning — not pay-to-rank cover-letter writers and not job or visa guarantees. Confirm scope, credentials and pricing with any provider."
      />
    </div>
  );
}
