...
import { interviewTipsNetherlandsPage as page, INTERVIEW_TIPS_NETHERLANDS_PATH } from "./interviewTipsNetherlandsPageModel";
...
function RecommendedProviderCard({ service }: { service: PageRecommendedProviderCard }) {
  ...
  if (external) {
    const outbound = buildTrackedOutboundLink(service.url, {
      pagePath: INTERVIEW_TIPS_NETHERLANDS_PATH,
...
export function InterviewTipsNetherlandsRecommendedServices() {
  ...
  return (
    <div className="space-y-8">
      ...
      <ProviderCardSection
        ...
        title="Immigration support when interviews depend on sponsorship"
        description="Useful when work-permit routes, recognised sponsors or relocation timing still need scoped professional help alongside official IND guidance — not as interview coaching."
        cards={visaPermits}
      />
      <ProviderCardSection
        eyebrow="Relocation"
        title="Relocation support when late-stage interviews look serious"
        description="Useful once city, start date and family logistics need planning alongside a likely offer — confirm what your employer covers."
        cards={relocation}
      />
      <ProviderCardSection
        eyebrow="Post-offer setup"
        title="Banking, health insurance and housing search"
        description="Many international hires line up accounts, mandatory insurance and housing in parallel with final interviews and contract signing — scope and fees differ by provider."
        cards={setup}
      />
      <AffiliateDisclosure
        variant="copilot"
        text="ExpatCopilot may earn a commission from some partners. Listings here support interview-to-relocation planning — not pay-to-rank interview coaches and not job or visa guarantees. Confirm scope, credentials and pricing with any provider."
      />
    </div>
  );
}
