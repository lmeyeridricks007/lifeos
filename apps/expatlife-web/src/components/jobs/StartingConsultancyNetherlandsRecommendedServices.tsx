...
import {
  getStartingConsultancyNetherlandsSupportCardGroups,
...
import {
  CAREER_COACHES_PATH,
  STARTING_CONSULTANCY_NETHERLANDS_PATH,
...
function RecommendedProviderCard({ service }: { service: PageRecommendedProviderCard }) {
  ...
  if (external) {
    const outbound = buildTrackedOutboundLink(service.url, {
      pagePath: STARTING_CONSULTANCY_NETHERLANDS_PATH,
...
export function StartingConsultancyTaxAffiliateSupport() {
  const { taxPayroll } = getStartingConsultancyNetherlandsSupportCardGroups();

  return (
    <div className="mt-6 space-y-4">
      <ProviderCardSection
        eyebrow="Tax & accounting"
        title="Tax advisors when consultancy setup gets complex"
...
export function StartingConsultancyVisaAffiliateSupport() {
  const { visaPermits } = getStartingConsultancyNetherlandsSupportCardGroups();

  return (
    <div className="mt-6 space-y-4">
      <ProviderCardSection
        eyebrow="Visa & permits"
        title="Immigration support when permits intersect with consultancy"
...
export function StartingConsultancyNetherlandsRecommendedServices() {
  const { visaPermits, taxPayroll, relocation, setup } = getStartingConsultancyNetherlandsSupportCardGroups();
  ...
  return (
    <div className="mt-8 space-y-8">
      ...
      <ProviderCardSection
        ...
        description="Many consultants coordinate accounts and insurance around registration and first client payments."
        cards={setup}
      />
      <SectionAffiliateDisclosure />
    </div>
