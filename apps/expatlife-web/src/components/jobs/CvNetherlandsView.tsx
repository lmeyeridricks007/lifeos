...
import { CvNetherlandsRecommendedServices } from "./CvNetherlandsRecommendedServices";
import {
  CV_NETHERLANDS_PATH,
  ...
  cvNetherlandsPage as page,
  type CvNetherlandsLink,
...
function LinkCard({ item, iconIndex = 0, tone = "default" }: { item: CvNetherlandsLink; iconIndex?: number; tone?: "default" | "onDark" }) {
...
export function CvNetherlandsView() {
    ...
    <div className={sitePageCanvasClass}>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "CV Netherlands", item: `${baseUrl}${CV_NETHERLANDS_PATH}` },
        ]}
      />
      <Container className={siteGuideColumnPadYClass}>
        <div className="mx-auto w-full max-w-6xl">
          ...
          <div className="sticky top-3 z-20 mt-6 overflow-x-auto rounded-2xl border border-slate-200/80 bg-white/90 p-2 shadow-card ring-1 ring-slate-900/[0.04] backdrop-blur-xl">
            <nav aria-label="CV guide sections" className="flex min-w-max gap-2">
          ...
          <div className={sectionStackClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                  ...
                  <GuideLinkAside
                    ...
                    description="Offer talks after interviews — not part of the CV rewrite."
                    href={SALARY_NEGOTIATION_NETHERLANDS_PATH}
                    linkLabel="Open Salary Negotiation"
                  />
                </div>
                <ScenarioTable title="When CV localisation changes the plan" rows={page.intro.scenarios} />
              </div>
            </section>

            <section id="snapshot" className={sectionClass}>
              <SectionIntro eyebrow="At a glance" title="Dutch CV Snapshot">
                <p>Practical orientation on length, photos, motivation letters, LinkedIn and recruiter scan habits before you rewrite.</p>
              </SectionIntro>
            ...
            <section id="photo" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Photo" title={page.photoHeading} fullWidth>
                  {page.photoParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Photo orientation" items={page.photoPoints} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.photo} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <ScenarioTable title="Photo decisions by situation" rows={page.photoScenarios} />
              </div>
            </section>

            <section id="motivation" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Sollicitatiebrief" title={page.motivationHeading} fullWidth>
                  {page.motivationParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Motivation letter principles" items={page.motivationPoints} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.motivation} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <div className="grid gap-4 sm:grid-cols-2">
                  {page.motivationStructure.map((item, idx) => (
                    <FeatureCard key={item.title} title={item.title} body={item.body} iconIndex={idx} />
                  ))}
                </div>
                <ChecklistBlock title="Motivation letter checklist" items={page.motivationChecklist} columns={2} />
              </div>
            </section>

            <section id="linkedin" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="LinkedIn" title={page.linkedinHeading} fullWidth>
                  {page.linkedinParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="LinkedIn tips" items={page.linkedinTips} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.linkedin} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <ChecklistBlock title="LinkedIn alignment checklist" items={page.linkedinChecklist} columns={2} />
              </div>
            </section>

            <section id="recruiter-scan" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Screening" title={page.recruiterScanHeading} fullWidth>
                  {page.recruiterScanParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Scan tips" items={page.recruiterScanTips} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.recruiterScan} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <InfoTable
                  rows={page.recruiterScanRows.map((row) => ({
                    whatTheyScan: row.whatTheyScan,
                    whyItMatters: row.whyItMatters,
                    howToShowIt: row.howToShowIt,
                  }))}
                  columns={[
                    { key: "whatTheyScan", label: "What they scan" },
                    { key: "whyItMatters", label: "Why it matters" },
                    { key: "howToShowIt", label: "How to show it" },
                  ]}
                />
              </div>
            </section>
            ...
            <section id="checklist" className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                ...
                <ChecklistBlock title="Application package checklist" items={page.applicationChecklist} columns={2} />
              </div>
            </section>

            <section id="scenarios" className={sectionClass}>
              <SectionIntro eyebrow="Scenarios" title={page.scenariosHeading}>
                <p>Your CV emphasis changes depending on how you enter the Dutch labour market.</p>
              </SectionIntro>
              <GuidePremiumVisualFigure visual={page.visuals.scenarios} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <ScenarioTable title="Expat CV scenarios" rows={page.scenarios} />
              </div>
            </section>

            <section id="related-guides" className={sectionClass}>
              <SectionIntro eyebrow="Career cluster" title="Related Guides">
                <p>Connect CV localisation to finding jobs, interviews, salary negotiation, contracts and workplace culture.</p>
              </SectionIntro>
            ...
            <section id="services" className={sectionClass}>
              <SectionIntro eyebrow="Professional support" title="Providers That May Help">
                <p>Career, relocation, immigration and tax support may help with specific steps — this page does not rank CV writers or guarantee interviews. Some links may be affiliate or referral links.</p>
              </SectionIntro>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                ...
                <CvNetherlandsRecommendedServices />
            ...
            <section id="faq" className={sectionClass}>
              <SectionIntro eyebrow="FAQ" title="Frequently Asked Questions">
                <p>These answers help you decide what to edit next — length, photos, letters, LinkedIn and authorization wording.</p>
              </SectionIntro>
            ...
            >
              ...
              <div className="mt-6">
                <SectionIntro eyebrow="Explore next" title="Explore Next" tone="onDark">
                  <p>Move from CV localisation into channels, interviews, offer talks and workplace culture.</p>
                </SectionIntro>
