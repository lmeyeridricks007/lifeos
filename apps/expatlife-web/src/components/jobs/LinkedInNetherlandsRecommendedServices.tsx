...
import { linkedInNetherlandsPage as page, LINKEDIN_NETHERLANDS_PATH } from "./linkedinNetherlandsPageModel";
...
function RecommendedProviderCard({ service }: { service: PageRecommendedProviderCard }) {
  ...
  if (external) {
    const outbound = buildTrackedOutboundLink(service.url, {
      pagePath: LINKEDIN_NETHERLANDS_PATH,
...
export function LinkedInNetherlandsRecommendedServices() {
  ...
  return (
    <div className="space-y-8">
      ...
      <ProviderCardSection
        ...
        title="Immigration support when LinkedIn search depends on sponsorship"
        description="Useful when work-permit routes, recognised sponsors or relocation timing still need scoped professional help alongside official IND guidance — not as a LinkedIn Premium or profile-writing service."
        cards={visaPermits}
      />
      <ProviderCardSection
        eyebrow="Relocation"
        title="Relocation support when outreach leads to a move"
      ...
      <AffiliateDisclosure
        variant="copilot"
        text="ExpatCopilot may earn a commission from some partners. Listings here support job-search and relocation planning — not pay-to-rank LinkedIn coaches or Premium sellers and not job or visa guarantees. Confirm scope, credentials and pricing with any provider."
      />
    </div>
  );
}
