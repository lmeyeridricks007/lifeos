...
import { networkingNetherlandsPage as page, NETWORKING_NETHERLANDS_PATH } from "./networkingNetherlandsPageModel";
...
function RecommendedProviderCard({ service }: { service: PageRecommendedProviderCard }) {
  ...
  if (external) {
    const outbound = buildTrackedOutboundLink(service.url, {
      pagePath: NETWORKING_NETHERLANDS_PATH,
...
export function NetworkingNetherlandsRecommendedServices() {
  ...
  return (
    <div className="space-y-8">
      ...
      <ProviderCardSection
        ...
        description="Useful when work-permit routes, recognised sponsors or relocation timing still need scoped professional help alongside official IND guidance — not as a networking-coach or meetup service."
        cards={visaPermits}
      />
      <ProviderCardSection
        eyebrow="Relocation"
        title="Relocation support when relationships lead to a move"
      ...
      <AffiliateDisclosure
        variant="copilot"
        text="ExpatCopilot may earn a commission from some partners. Listings here support job-search and relocation planning — not pay-to-rank networking coaches or meetup apps and not job or visa guarantees. Confirm scope, credentials and pricing with any provider."
      />
    </div>
  );
}
