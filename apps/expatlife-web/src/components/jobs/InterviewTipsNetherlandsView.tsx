...
import { InterviewTipsNetherlandsRecommendedServices } from "./InterviewTipsNetherlandsRecommendedServices";
import {
  CV_NETHERLANDS_PATH,
  ...
  interviewTipsNetherlandsPage as page,
  type InterviewTipsNetherlandsLink,
} from "./interviewTipsNetherlandsPageModel";
...
function LinkCard({ item, iconIndex = 0, tone = "default" }: { item: InterviewTipsNetherlandsLink; iconIndex?: number; tone?: "default" | "onDark" }) {
...
export function InterviewTipsNetherlandsView() {
    ...
    <div className={sitePageCanvasClass}>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Interview Tips", item: `${baseUrl}${INTERVIEW_TIPS_NETHERLANDS_PATH}` },
        ]}
      />
      <Container className={siteGuideColumnPadYClass}>
        <div className="mx-auto w-full max-w-6xl">
              ...
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:items-center">
                <div>
                  ...
                  <p className="mt-5 max-w-3xl rounded-2xl border border-amber-200/70 bg-amber-50/70 px-4 py-3 text-sm leading-relaxed text-amber-950 shadow-sm ring-1 ring-amber-200/40">
                    This guide is practical orientation only — not career, immigration or recruitment advice. Interview norms vary by employer and sector. Some links on this page may be affiliate or referral links.
                  </p>
                </div>
                <HeroImage />
              </div>
          ...
          <div className="sticky top-3 z-20 mt-6 overflow-x-auto rounded-2xl border border-slate-200/80 bg-white/90 p-2 shadow-card ring-1 ring-slate-900/[0.04] backdrop-blur-xl">
            <nav aria-label="Interview tips guide sections" className="flex min-w-max gap-2">
          ...
          <div className={sectionStackClass}>
            <section id="intro" className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                ...
                <ChecklistBlock title="Three orientation moves before your next interview" items={page.introPlanningSteps} columns={2} />
                <div className="grid gap-4 sm:grid-cols-3">
                  <GuideLinkAside
                    title="CV Netherlands"
                    description="Localise CV, motivation letter and LinkedIn before the loop."
                    href={CV_NETHERLANDS_PATH}
                    linkLabel="Open CV Netherlands"
                  />
                  <GuideLinkAside
                    title="Finding jobs"
                    description="Channels, recruiters and sponsorship strategy alongside interviews."
                    href={FINDING_JOBS_NETHERLANDS_PATH}
                    linkLabel="Open Finding Jobs"
                  />
                  <GuideLinkAside
                    title="Salary negotiation"
                    description="Offer talks after interviews — not during early screens."
                    href={SALARY_NEGOTIATION_NETHERLANDS_PATH}
                    linkLabel="Open Salary Negotiation"
                  />
                </div>
                <ScenarioTable title="When interview prep changes the plan" rows={page.intro.scenarios} />
              </div>
            </section>

            <section id="snapshot" className={sectionClass}>
              <SectionIntro eyebrow="At a glance" title="Dutch Interview Snapshot">
                <p>Practical orientation on tone, evidence, stages, formats, follow-up and authorization honesty before you prepare.</p>
              </SectionIntro>
            ...
            <section id="culture" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Culture" title={page.cultureHeading} fullWidth>
                  {page.cultureParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Culture principles" items={page.culturePoints} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.culture} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <div className="grid gap-4 sm:grid-cols-2">
                  {page.cultureCards.map((item, idx) => (
                    <FeatureCard key={item.title} title={item.title} body={item.body} iconIndex={idx} />
                  ))}
                </div>
              </div>
            ...
            <section id="prepare" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Prepare" title={page.prepareHeading} fullWidth>
                  {page.prepareParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Preparation principles" items={page.preparePoints} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.prepare} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <ChecklistBlock title="Preparation checklist" items={page.prepareChecklist} columns={2} />
              </div>
            </section>

            <section id="questions" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Questions" title={page.questionsHeading} fullWidth>
                  {page.questionsParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Answer tips" items={page.questionsTips} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.questions} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <InfoTable
                  rows={page.questionRows.map((row) => ({
                    question: row.question,
                    whatTheyProbe: row.whatTheyProbe,
                    howToAnswer: row.howToAnswer,
                  }))}
                  columns={[
                    { key: "question", label: "Question theme" },
                    { key: "whatTheyProbe", label: "What they probe" },
                    { key: "howToAnswer", label: "How to answer" },
                  ]}
                />
              </div>
            </section>

            <section id="formats" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Formats" title={page.formatsHeading} fullWidth>
                  {page.formatsParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Format checklist" items={page.formatsChecklist} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.formats} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <InfoTable
                  rows={page.formatRows.map((row) => ({
                    format: row.format,
            ...
            <section id="follow-up" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Follow-up" title={page.followUpHeading} fullWidth>
                  {page.followUpParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Follow-up principles" items={page.followUpPoints} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.followUp} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <ScenarioTable title="Follow-up scenarios" rows={page.followUpScenarios} />
              </div>
            </section>

            <section id="mistakes" className={sectionClass}>
              <SectionIntro eyebrow="Pitfalls" title={page.mistakesHeading}>
                <p>Fixing these issues improves clarity — it still does not guarantee offers.</p>
              </SectionIntro>
            ...
            <section id="checklist" className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                ...
                <ChecklistBlock title="Interview loop checklist" items={page.applicationChecklist} columns={2} />
              </div>
            </section>

            <section id="scenarios" className={sectionClass}>
              <SectionIntro eyebrow="Scenarios" title={page.scenariosHeading}>
                <p>Your prep emphasis changes depending on how you enter the Dutch labour market.</p>
              </SectionIntro>
              <GuidePremiumVisualFigure visual={page.visuals.scenarios} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <ScenarioTable title="Expat interview scenarios" rows={page.scenarios} />
              </div>
            </section>

            <section id="related-guides" className={sectionClass}>
              <SectionIntro eyebrow="Career cluster" title="Related Guides">
                <p>Connect interview prep to CV localisation, finding jobs, salary negotiation, contracts and workplace culture.</p>
              </SectionIntro>
            ...
            <section id="services" className={sectionClass}>
              <SectionIntro eyebrow="Professional support" title="Providers That May Help">
                <p>Career, relocation, immigration and tax support may help with specific steps — this page does not rank interview coaches or guarantee offers. Some links may be affiliate or referral links.</p>
              </SectionIntro>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                ...
                <InterviewTipsNetherlandsRecommendedServices />
            ...
            <section id="faq" className={sectionClass}>
              <SectionIntro eyebrow="FAQ" title="Frequently Asked Questions">
                <p>These answers help you decide what to prepare next — tone, language, salary timing, formats and follow-up.</p>
              </SectionIntro>
            ...
            >
              ...
              <div className="mt-6">
                <SectionIntro eyebrow="Explore next" title="Explore Next" tone="onDark">
                  <p>Move from interview prep into documents, channels, offer talks and workplace culture.</p>
                </SectionIntro>
