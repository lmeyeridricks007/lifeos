...
function RecommendedProviderCard({ service }: { service: PageRecommendedProviderCard }) {
  ...
  if (external) {
    const outbound = buildTrackedOutboundLink(service.url, {
      pagePath: RECRUITMENT_AGENCIES_NETHERLANDS_PATH,
...
export function RecruitmentAgenciesNetherlandsRecommendedServices() {
  ...
  return (
    <div className="space-y-8">
      ...
      <ProviderCardSection
        ...
        title="Immigration support when agency roles depend on sponsorship"
        description="Useful when work-permit routes, recognised sponsors or relocation timing still need scoped professional help alongside official IND guidance — not as a recruitment-agency ranking or placement guarantee."
        cards={visaPermits}
      />
      <ProviderCardSection
        eyebrow="Relocation"
        title="Relocation support when agency interviews lead to a move"
        description="Useful once city, start date and family logistics need planning alongside your new role — confirm what your employer or agency-client covers."
        cards={relocation}
      />
      <ProviderCardSection
        eyebrow="Post-offer setup"
        title="Banking, health insurance and housing search"
        description="Many international hires line up accounts, mandatory insurance and housing in parallel with contract signing after recruiter-led processes — scope and fees differ by provider."
        cards={setup}
      />
      <AffiliateDisclosure
        variant="copilot"
        text="ExpatCopilot may earn a commission from some partners. Listings here support job-search and relocation planning — not pay-to-rank recruitment agencies, uitzendbureaus or headhunters and not job or visa guarantees. Confirm scope, credentials and pricing with any provider."
      />
    </div>
  );
}
