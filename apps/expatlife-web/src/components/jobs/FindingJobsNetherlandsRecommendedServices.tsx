...
import { findingJobsNetherlandsPage as page, FINDING_JOBS_NETHERLANDS_PATH } from "./findingJobsNetherlandsPageModel";
...
const categoryLinks = [
  ...
  { href: "/netherlands/services/highly-skilled-migrant-sponsors/", label: "HSM sponsors" },
...
export function FindingJobsVisaAffiliateSupport() {
  const { visaPermits } = getFindingJobsNetherlandsSupportCardGroups();

  return (
    <div className="mt-6 space-y-4">
      <ProviderCardSection
        eyebrow="Visa & permits"
        title="Immigration support when sponsorship is part of your search"
        description="Useful when employer-sponsored routes, recognised sponsors or permit timing still need scoped professional help alongside official IND guidance — not as a job placement service."
        cards={visaPermits}
      />
      <AffiliateDisclosure
        variant="copilot"
...
export function FindingJobsNetherlandsRecommendedServices() {
  ...
  return (
    <div className="space-y-8">
      ...
      <ProviderCardSection
        ...
        description="Helpful when work-permit routes, recognised sponsors or relocation timing still feel unclear before you accept an offer."
        cards={visaPermits}
      />
      <ProviderCardSection
        eyebrow="Relocation"
        title="Relocation support when an offer leads to a move"
      ...
      <AffiliateDisclosure
        variant="copilot"
        text="ExpatCopilot may earn a commission from some partners. Listings here support job-search planning — not pay-to-rank placement and not job or visa guarantees. Confirm scope, credentials and pricing with any provider."
      />
    </div>
  );
}
