...
import { LinkedInNetherlandsRecommendedServices } from "./LinkedInNetherlandsRecommendedServices";
import {
...
export function LinkedInNetherlandsView() {
    ...
    <div className={sitePageCanvasClass}>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "LinkedIn Netherlands", item: `${baseUrl}${LINKEDIN_NETHERLANDS_PATH}` },
        ]}
      />
      <Container className={siteGuideColumnPadYClass}>
        <div className="mx-auto w-full max-w-6xl">
              ...
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:items-center">
                <div>
                  ...
                  <p className="mt-5 max-w-3xl rounded-2xl border border-amber-200/70 bg-amber-50/70 px-4 py-3 text-sm leading-relaxed text-amber-950 shadow-sm ring-1 ring-amber-200/40">
                    This guide is practical orientation only — not career, immigration or recruitment advice. LinkedIn and hiring norms vary by employer and sector. Some links on this page may be affiliate or referral links.
                  </p>
                </div>
                <HeroImage />
              </div>
          ...
          <div className="sticky top-3 z-20 mt-6 overflow-x-auto rounded-2xl border border-slate-200/80 bg-white/90 p-2 shadow-card ring-1 ring-slate-900/[0.04] backdrop-blur-xl">
            <nav aria-label="LinkedIn guide sections" className="flex min-w-max gap-2">
          ...
          <div className={sectionStackClass}>
            <section id="intro" className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                ...
                <ChecklistBlock title="Three orientation moves before you rewrite" items={page.introPlanningSteps} columns={2} />
                <div className="grid gap-4 sm:grid-cols-3">
                  ...
                  <GuideLinkAside
                    title="Finding jobs"
                    description="Multi-channel strategy beyond LinkedIn alone."
                    href={FINDING_JOBS_NETHERLANDS_PATH}
                    linkLabel="Open Finding Jobs"
                  />
                  <GuideLinkAside
                    title="Cover letter"
                    description="Motivatiebrief norms when portals require a letter."
                    href={COVER_LETTER_NETHERLANDS_PATH}
                    linkLabel="Open Cover Letter"
                  />
                </div>
                <ScenarioTable title="When LinkedIn decisions change" rows={page.intro.scenarios} />
              </div>
            </section>

            <section id="snapshot" className={sectionClass}>
              <SectionIntro eyebrow="At a glance" title="LinkedIn Netherlands Snapshot">
                <p>Practical orientation on headline, About, Open to Work, language, messaging and CV alignment before you rewrite.</p>
              </SectionIntro>
            ...
            <section id="profile" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Foundations" title={page.profileHeading} fullWidth>
                  {page.profileParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Profile foundations" items={page.profilePoints} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.profile} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <InfoTable
                  rows={page.profileRows.map((row) => ({ section: row.section, include: row.include, tip: row.tip }))}
                  columns={[
                    { key: "section", label: "Element" },
                    { key: "include", label: "Include" },
                    { key: "tip", label: "Tip" },
                  ]}
                />
                <ChecklistBlock title="Profile foundation checklist" items={page.profileChecklist} columns={2} />
              </div>
            </section>

            <section id="headline-about" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Positioning" title={page.headlineAboutHeading} fullWidth>
                  {page.headlineAboutParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Headline and About principles" items={page.headlineAboutPoints} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.headlineAbout} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <div className="grid gap-4 sm:grid-cols-2">
                  {page.headlineAboutCards.map((item, idx) => (
                    <FeatureCard key={item.title} title={item.title} body={item.body} iconIndex={idx} />
                  ))}
                </div>
                <BulletPanel title="Headline and About tips" items={page.headlineAboutTips} />
              </div>
            </section>

            <section id="experience" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Evidence" title={page.experienceHeading} fullWidth>
                  {page.experienceParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Experience principles" items={page.experiencePoints} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.experience} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <InfoTable
                  rows={page.experienceRows.map((row) => ({ section: row.section, include: row.include, tip: row.tip }))}
                  columns={[
                ...
                />
                <ChecklistBlock title="Experience checklist" items={page.experienceChecklist} columns={2} />
                <GuideLinkAside
                  title="Need the CV document guide?"
                  description="LinkedIn Experience should match your localised Dutch CV — keep titles and dates consistent."
                  href={CV_NETHERLANDS_PATH}
            ...
            <section id="open-to-work" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Signals" title={page.openToWorkHeading} fullWidth>
                  {page.openToWorkParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Open to Work principles" items={page.openToWorkPoints} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.openToWork} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <InfoTable
                  rows={page.openToWorkRows.map((row) => ({
                  ...
                  columns={[
                    { key: "situation", label: "Situation" },
                    { key: "letterNeeded", label: "Visibility approach" },
                    { key: "tip", label: "Tip" },
                  ]}
                />
              </div>
            ...
            <section id="dutch-norms" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Culture" title={page.dutchNormsHeading} fullWidth>
                  {page.dutchNormsParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Dutch LinkedIn norms" items={page.dutchNormsPoints} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.dutchNorms} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <InfoTable
                  rows={page.dutchNormsRows.map((row) => ({ topic: row.topic, dutchNorm: row.dutchNorm, tip: row.tip }))}
                  columns={[
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  />
                  <GuideLinkAside
                    title="Dutch Directness at Work"
            ...
            <section id="messaging" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Outreach" title={page.messagingHeading} fullWidth>
                  {page.messagingParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Messaging principles" items={page.messagingPoints} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.messaging} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <div className="grid gap-4 sm:grid-cols-2">
                  {page.messagingCards.map((item, idx) => (
                    <FeatureCard key={item.title} title={item.title} body={item.body} iconIndex={idx} />
                  ))}
                </div>
                <BulletPanel title="Messaging tips" items={page.messagingTips} />
              </div>
            </section>

            <section id="applying" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Channels" title={page.applyingHeading} fullWidth>
                  {page.applyingParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="Applying principles" items={page.applyingPoints} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.applying} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <ScenarioTable title="LinkedIn application paths" rows={page.applyingRows} />
                <div className="grid gap-4 sm:grid-cols-2">
                  <GuideLinkAside
                    title="Cover Letter Netherlands"
                    description="When company portals still require a motivatiebrief."
                    href={COVER_LETTER_NETHERLANDS_PATH}
                    linkLabel="Open Cover Letter"
                  />
                  <GuideLinkAside
                    title="Finding Jobs Netherlands"
                    description="Multi-channel strategy beyond Easy Apply."
                    href={FINDING_JOBS_NETHERLANDS_PATH}
                    linkLabel="Open Finding Jobs"
                  />
                </div>
            ...
            <section id="networking" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="Platform layer" title={page.networkingOnLiHeading} fullWidth>
                  {page.networkingOnLiParagraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
                </SectionIntro>
                <BulletPanel title="LinkedIn networking moves" items={page.networkingOnLiPoints} />
              </div>
              <GuidePremiumVisualFigure visual={page.visuals.networkingOnLi} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <ScenarioTable title="Platform networking scenarios" rows={page.networkingOnLiScenarios} />
              </div>
            </section>
            ...
            <section id="checklist" className={sectionClass}>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                ...
                <ChecklistBlock title="LinkedIn job-search package checklist" items={page.applicationChecklist} columns={2} />
              </div>
            </section>

            <section id="scenarios" className={sectionClass}>
              <SectionIntro eyebrow="Scenarios" title={page.scenariosHeading}>
                <p>Your profile emphasis changes depending on how you enter the Dutch labour market.</p>
              </SectionIntro>
              <GuidePremiumVisualFigure visual={page.visuals.scenarios} className={guidePremiumVisualSpacingClass} />
              <div className={guidePremiumSectionDetailStackClass}>
                <ScenarioTable title="Expat LinkedIn scenarios" rows={page.scenarios} />
              </div>
            </section>

            <section id="related-guides" className={sectionClass}>
              <SectionIntro eyebrow="Career cluster" title="Related Guides">
                <p>Connect LinkedIn profile work to CV localisation, cover letters, finding jobs, interviews, salary negotiation and workplace culture.</p>
              </SectionIntro>
            ...
            <section id="services" className={sectionClass}>
              <SectionIntro eyebrow="Professional support" title="Providers That May Help">
                <p>
                  Career, relocation, immigration and tax support may help with specific steps — this page does not rank LinkedIn coaches,
                  Premium sellers or recruiters. Some links may be affiliate or referral links.
                </p>
              </SectionIntro>
              ...
              <div className={guidePremiumSectionDetailStackClass}>
                ...
                <LinkedInNetherlandsRecommendedServices />
            ...
            <section id="faq" className={sectionClass}>
              <SectionIntro eyebrow="FAQ" title="Frequently Asked Questions">
                <p>These answers help you decide what to edit next — Open to Work, language, Premium, photos, messaging and Easy Apply.</p>
              </SectionIntro>
            ...
            >
              ...
              <div className="mt-6">
                <SectionIntro eyebrow="Explore next" title="Explore Next" tone="onDark">
                  <p>Move from LinkedIn profile work into CV polish, cover letters, channels, interviews, offer talks and workplace culture.</p>
                </SectionIntro>
                ...
                <div className={guidePremiumSectionDetailStackClass}>
                  ...
                  <aside className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/10 p-5 ring-1 ring-white/10">
                    ...
                    <Link href={INTERVIEW_TIPS_NETHERLANDS_PATH} className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-cyan-200 hover:text-white">
                      Interview tips guide <ArrowRight className="h-4 w-4" aria-hidden />
                    </Link>
                  </aside>
                </div>
              </div>
