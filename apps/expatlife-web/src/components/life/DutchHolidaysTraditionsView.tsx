...
import {
  ...
  dutchHolidaysTraditionsPage as page,
  DUTCH_CULTURE_PATH,
  ...
  type LifeGuideLink,
  type MistakeCard,
} from "./dutchHolidaysTraditionsPageModel";
...
const schoolHolidayColumns = [
  { key: "name", label: "Break" },
  { key: "duration", label: "Duration" },
  { key: "travelImpact", label: "Travel impact" },
  { key: "note", label: "Region note" },
] as const;
...
const regionalTraditionColumns = [
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Three moves in your first year</h3>
        ...
        return (
          ...
          >
            ...
            <dl className="mt-3 space-y-2 text-sm">
              <div>
                <dt className="font-semibold text-foreground">King&apos;s Day</dt>
                <dd className="text-foreground-muted">{city.kingsDay}</dd>
              </div>
              <div>
                <dt className="font-semibold text-foreground">Christmas</dt>
                <dd className="text-foreground-muted">{city.christmas}</dd>
              </div>
              <div>
                <dt className="font-semibold text-foreground">Carnival</dt>
                <dd className="text-foreground-muted">{city.carnival}</dd>
              </div>
              <div>
                <dt className="font-semibold text-foreground">Markets</dt>
                <dd className="text-foreground-muted">{city.markets}</dd>
              </div>
              <div>
                <dt className="font-semibold text-foreground">Family atmosphere</dt>
                <dd className="text-foreground-muted">{city.familyAtmosphere}</dd>
              </div>
            </dl>
...
export function DutchHolidaysTraditionsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Dutch Holidays and Traditions", item: new URL(DUTCH_HOLIDAYS_TRADITIONS_PATH, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
              ...
              <div className="grid gap-8 lg:grid-cols-[minmax(0,0.95fr)_minmax(360px,0.85fr)] lg:items-center">
                <div>
                  <nav aria-label="Breadcrumb" className="flex flex-wrap gap-2 text-xs text-foreground-muted">
                    ...
                    <span aria-hidden>/</span>
                    <span className="text-foreground" aria-current="page">Dutch Holidays and Traditions</span>
                  </nav>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <GuideCrossLink
                  ...
                  description="Everyday etiquette for parties, visits and gift exchanges during holiday seasons."
                  linkLabel="Open Social Norms"
                />
                <GuideCrossLink
                  href={COMMUNITY_BASICS_NETHERLANDS_PATH}
                  title="Community Basics"
                  description="Friends, neighbours, clubs and integration routes beyond calendar events."
            ...
            <PremiumGuideSection
              ...
              tipsKey="snapshot"
              intro={
                <SectionIntro eyebrow="At a glance" title="Dutch Holidays at a Glance" fullWidth>
                  <p>Six orientation signals — then pick one celebration per season to experience first.</p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              id="annual-calendar"
              tipsKey="annualCalendar"
              intro={
                <SectionIntro title="Annual Holiday Calendar" fullWidth>
                  <p>Month-by-month overview of major Dutch holidays, commemorations and family seasons — confirm exact dates yearly.</p>
                </SectionIntro>
              }
              visual={page.visuals.annualCalendar}
            >
              <SectionTable
                title="Month-by-month overview"
                description="Use this as orientation — confirm parade dates and market openings on municipal sites."
                rows={page.annualCalendar.map((r) => ({ month: r.month, holidays: r.holidays, note: r.note }))}
                columns={[
              ...
              <SectionTable
                ...
                description="Paid day off for many employees — verify your contract for Good Friday and Liberation Day."
                rows={page.publicHolidays.map((r) => ({
                  name: r.name,
                  datePattern: r.datePattern,
                  paidDayOff: r.paidDayOff,
                  note: r.note,
                }))}
                columns={[
                  { key: "name", label: "Holiday" },
                  { key: "datePattern", label: "Date" },
                  { key: "paidDayOff", label: "Paid day off" },
                  { key: "note", label: "Note" },
                ]}
              />
              <SectionTable
                title="Employer-dependent or regional dates"
                description="These shape work and travel but are not always national paid holidays."
                rows={page.variableHolidayRows.map((r) => ({
                  name: r.name,
                  datePattern: r.datePattern,
                  paidDayOff: r.paidDayOff,
                  note: r.note,
            ...
            <PremiumGuideSection
              ...
              tipsKey="kingsDay"
              intro={
                <SectionIntro title={page.kingsDayHeading}>
                  {page.kingsDayParagraphs.map((p) => <p key={p}>{p}</p>)}
                </SectionIntro>
              }
              visual={page.visuals.kingsDay}
            >
              <TopicsTable rows={page.kingsDayTopics} />
              <ChecklistBlock title="King's Day checklist" items={page.kingsDayTips} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              tipsKey="sinterklaas"
              intro={
                <SectionIntro title={page.sinterklaasHeading}>
                  {page.sinterklaasParagraphs.map((p) => <p key={p}>{p}</p>)}
                </SectionIntro>
              }
              visual={page.visuals.sinterklaas}
            >
              <TopicsTable rows={page.sinterklaasTopics} />
              <ChecklistBlock title="Sinterklaas checklist" items={page.sinterklaasTips} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              tipsKey="christmas"
              intro={
                <SectionIntro title={page.christmasHeading}>
                  {page.christmasParagraphs.map((p) => <p key={p}>{p}</p>)}
                </SectionIntro>
              }
              visual={page.visuals.christmas}
            >
              <TopicsTable rows={page.christmasTopics} />
              <ChecklistBlock title="Christmas checklist" items={page.christmasTips} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              tipsKey="newYear"
              intro={
                <SectionIntro title={page.newYearHeading}>
                  {page.newYearParagraphs.map((p) => <p key={p}>{p}</p>)}
                </SectionIntro>
              }
              visual={page.visuals.newYear}
            >
              <TopicsTable rows={page.newYearTopics} />
              <ChecklistBlock title="New Year checklist" items={page.newYearTips} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              tipsKey="easter"
              intro={
                <SectionIntro title={page.easterHeading}>
                  {page.easterParagraphs.map((p) => <p key={p}>{p}</p>)}
                </SectionIntro>
              }
              visual={page.visuals.easter}
            >
              <TopicsTable rows={page.easterTopics} />
              <ChecklistBlock title="Easter checklist" items={page.easterTips} columns={2} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="liberation-remembrance"
              tipsKey="liberationRemembrance"
              intro={
                <SectionIntro title={page.liberationRemembranceHeading}>
                  {page.liberationRemembranceParagraphs.map((p) => <p key={p}>{p}</p>)}
                </SectionIntro>
              }
              visual={page.visuals.liberationRemembrance}
            >
              <TopicsTable rows={page.liberationRemembranceTopics} />
              <ChecklistBlock title="May 4–5 etiquette checklist" items={page.liberationRemembranceTips} columns={2} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="ascension-pentecost"
              tipsKey="ascensionPentecost"
              intro={
                <SectionIntro title={page.ascensionPentecostHeading}>
                  {page.ascensionPentecostParagraphs.map((p) => <p key={p}>{p}</p>)}
                </SectionIntro>
              }
              visual={page.visuals.ascensionPentecost}
            >
              <TopicsTable rows={page.ascensionPentecostTopics} />
              <ChecklistBlock title="Long weekend planning" items={page.ascensionPentecostTips} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              tipsKey="carnival"
              intro={
                <SectionIntro title={page.carnivalHeading}>
                  {page.carnivalParagraphs.map((p) => <p key={p}>{p}</p>)}
                </SectionIntro>
              }
              visual={page.visuals.carnival}
            >
              <TopicsTable rows={page.carnivalTopics} />
              <ChecklistBlock title="Carnival checklist" items={page.carnivalTips} columns={2} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="school-holidays"
              tipsKey="schoolHolidays"
              intro={
                <SectionIntro title={page.schoolHolidaysHeading}>
                  {page.schoolHolidaysParagraphs.map((p) => <p key={p}>{p}</p>)}
                </SectionIntro>
              }
              visual={page.visuals.schoolHolidays}
            >
              <SectionTable
                title="Regional school breaks"
                description="North, middle and south regions stagger dates — international schools may differ."
                rows={page.schoolHolidayRows.map((r) => ({ ...r }))}
                columns={[...schoolHolidayColumns]}
              />
              <ChecklistBlock title="School holiday planning" items={page.schoolHolidayTips} columns={2} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="regional-traditions"
              tipsKey="regionalTraditions"
              intro={
                <SectionIntro title="Regional Traditions" fullWidth>
                  <p>Beyond national holidays — regional events reward short trips outside your home city. Combine with a weekend plan.</p>
                </SectionIntro>
              }
              visual={page.visuals.regionalTraditions}
            >
              <SectionTable
                title="Regional events worth a day trip"
                rows={page.regionalTraditionRows.map((r) => ({ ...r }))}
                columns={[...regionalTraditionColumns]}
              />
              <FeatureGrid items={page.regionalTraditions} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              tipsKey="holidayFood"
              intro={
                <SectionIntro title="Seasonal Holiday Foods" fullWidth>
                  <p>Taste the calendar — seasonal treats appear in shops weeks before each celebration.</p>
                </SectionIntro>
              }
              visual={page.visuals.holidayFood}
            >
              <SectionTable
                title="What to try and when"
                rows={page.holidayFoods.map((food) => ({
                  name: food.name,
                  season: food.season,
                  note: food.note,
                }))}
              ...
              />
              <ChecklistBlock title="Taste the calendar" items={page.holidayFoodTips} columns={2} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="expat-expectations"
              tipsKey="expatExpectations"
              intro={
                <SectionIntro title="What Expats Should Expect" fullWidth>
                  <p>Public holidays affect work, shops and travel — plan ahead like mini peak seasons.</p>
                </SectionIntro>
              }
              visual={page.visuals.expatExpectations}
            >
              <SituationCardGrid items={page.expatScenarioCards} />
              <ChecklistBlock title="Expat planning checklist" items={page.expatExpectationsChecklist} columns={2} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="experience-traditions"
              tipsKey="experienceTraditions"
              intro={
                <SectionIntro title="How to Experience Dutch Traditions" fullWidth>
                  <p>Showing up locally beats only watching tourist highlights — start with one event per season.</p>
                </SectionIntro>
              }
              visual={page.visuals.experienceTraditions}
            >
              <FeatureGrid items={page.experienceIdeas} />
              <ChecklistBlock title="Participation checklist" items={page.experienceTraditionsChecklist} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro title="Holiday Celebrations by City">
                  <p>Choose cities by the holiday you care about — Carnival south, King&apos;s Day canals, Christmas markets north.</p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              id="common-questions"
              tipsKey="commonQuestions"
              intro={
                <SectionIntro title="Common Questions">
                  <p>Quick clarifiers on shops, work days, public holiday status and school calendars.</p>
                </SectionIntro>
              }
              visual={page.visuals.commonQuestions}
            >
              <FeatureGrid items={page.commonQuestionCards} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro title="Common Mistakes">
                  <p>Calendar gaps that catch expats off guard — adjust plans before concluding holidays are impossible to navigate.</p>
                </SectionIntro>
              }
            ...
            >
              ...
              <SectionTable
                title="Quick recovery guide"
                description="Use this table when a planning gap catches you off guard."
                rows={page.mistakeRecoveryRows.map((r) => ({ ...r }))}
                columns={[
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro title="Frequently Asked Questions">
                  <p>Confirm dates and employer policies locally — holiday rules vary by sector and municipality.</p>
                </SectionIntro>
              }
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro title="Related Guides">
                  <p>Continue from holidays into culture, social norms, community and city guides.</p>
                </SectionIntro>
              }
            ...
            <section id="explore-next" className={cn(CITIES_FUNNEL_SECTION_SCROLL_MARGIN, "scroll-mt-24 overflow-hidden rounded-[2rem] bg-slate-950 p-6 text-white shadow-expatos-xl sm:p-8 lg:p-10")}>
              ...
              <div className="mt-6">
                <SectionIntro eyebrow="Explore next" title="Plan the Next Step" tone="onDark" fullWidth>
                  <p>Move from holiday orientation into social norms, community integration and city guides.</p>
                </SectionIntro>
