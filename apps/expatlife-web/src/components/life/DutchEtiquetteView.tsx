...
import {
  ...
  dutchEtiquettePage as page,
  LIFE_HUB_PATH,
  type LifeGuideLink,
  type MistakeCard,
  type MythCard,
  type SituationCard,
} from "./dutchEtiquettePageModel";
...
export function DutchEtiquetteView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Dutch Etiquette", item: new URL(page.path, baseUrl).toString() },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
              ...
              <div className="grid gap-8 lg:grid-cols-[minmax(0,0.95fr)_minmax(360px,0.85fr)] lg:items-center">
                <div>
                  <nav aria-label="Breadcrumb" className="flex flex-wrap gap-2 text-xs text-foreground-muted">
                    ...
                    <span aria-hidden>/</span>
                    <span className="text-foreground" aria-current="page">Dutch etiquette</span>
                  </nav>
          ...
          <div className={sectionStackClass}>
            <PremiumGuideSection id="intro" tipsKey="intro" intro={<SectionIntro eyebrow="Quick answer" title={page.quickAnswer.heading} fullWidth>{page.introParagraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>} visual={page.visuals.intro}>
              <QuickAnswerBox />
              <FeatureGrid items={page.etiquetteCategories} />
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <GuideCrossLink href={DUTCH_SOCIAL_NORMS_PATH} title="Dutch Social Norms" description="Broader unwritten rules and social values beyond specific manners." linkLabel="Open Social Norms" />
              </div>
            </PremiumGuideSection>

            <PremiumGuideSection id="snapshot" tipsKey="snapshot" intro={<SectionIntro eyebrow="At a glance" title="Etiquette at a Glance" fullWidth><p>Six orientation signals — then verify with your neighbourhood and colleagues.</p></SectionIntro>} visual={page.visuals.snapshot}>
            ...
            <PremiumGuideSection id="greetings" tipsKey="greetings" intro={<SectionIntro title={page.greetingsHeading}>{page.greetingsParagraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>} visual={page.visuals.greetings}>
              <SectionTable title="Useful Dutch phrases" description="Match the greeting to the time of day and setting." rows={page.greetingPhrases.map((r) => ({ situation: r.situation, dutch: r.dutch, english: r.english, note: r.note }))} columns={[{ key: "situation", label: "Situation" }, { key: "dutch", label: "Dutch phrase" }, { key: "english", label: "English" }, { key: "note", label: "Note" }]} />
              <SectionTable title="Greetings by setting" rows={page.greetingExamples.map((r) => ({ setting: r.setting, hello: r.hello, goodbye: r.goodbye, note: r.note }))} columns={[{ key: "setting", label: "Setting" }, { key: "hello", label: "Hello" }, { key: "goodbye", label: "Goodbye" }, { key: "note", label: "Note" }]} />
              <GuideCrossLink href={DUTCH_SOCIAL_NORMS_PATH} title="Dutch Social Norms" description="Greetings, punctuality and visiting customs in broader social context." linkLabel="Open Social Norms" />
            </PremiumGuideSection>

            <PremiumGuideSection id="visiting-home" tipsKey="visitingHome" intro={<SectionIntro title={page.visitingHomeHeading}>{page.visitingHomeParagraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>} visual={page.visuals.visitingHome}>
              <InfoTable rows={page.visitFlowSteps.map((r) => ({ step: r.step, detail: r.detail }))} columns={[{ key: "step", label: "Step" }, { key: "detail", label: "What to do" }]} />
              <BulletPanel title="House guest reminders" items={page.visitingHomeTips} />
            </PremiumGuideSection>

            <PremiumGuideSection id="dining" tipsKey="dining" intro={<SectionIntro title={page.diningHeading}>{page.diningParagraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>} visual={page.visuals.dining}>
              <InfoTable rows={page.diningExamples.map((r) => ({ context: r.context, practice: r.practice, note: r.note }))} columns={[{ key: "context", label: "Context" }, { key: "practice", label: "Common practice" }, { key: "note", label: "Note" }]} />
              <ChecklistBlock title="Table manners" items={page.diningTableManners} columns={2} />
            </PremiumGuideSection>

            <PremiumGuideSection id="gift-giving" tipsKey="giftGiving" intro={<SectionIntro title={page.giftGivingHeading}>{page.giftGivingParagraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>} visual={page.visuals.giftGiving}>
              <InfoTable rows={page.giftExamples.map((r) => ({ occasion: r.occasion, gift: r.gift, note: r.note }))} columns={[{ key: "occasion", label: "Occasion" }, { key: "gift", label: "Appropriate gift" }, { key: "note", label: "Note" }]} />
              <FeatureGrid items={page.giftAvoid} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection id="workplace" tipsKey="workplace" intro={<SectionIntro title={page.workplaceHeading}>{page.workplaceParagraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>} visual={page.visuals.workplace}>
              <SectionTable title="Workplace scenarios" description="What to expect and how to respond in common professional settings." rows={page.workplaceScenarios.map((r) => ({ situation: r.situation, expectation: r.expectation, tip: r.tip }))} columns={[{ key: "situation", label: "Situation" }, { key: "expectation", label: "What to expect" }, { key: "tip", label: "Practical tip" }]} />
              <BulletPanel title="Workplace etiquette" items={page.workplaceTips} />
              <GuideCrossLink href={DUTCH_WORKPLACE_CULTURE_PATH} title="Dutch Workplace Culture" description="Meetings, feedback, borrels, emails and professional hierarchy in depth." linkLabel="Open Workplace Culture" />
            </PremiumGuideSection>

            <PremiumGuideSection id="neighbour" tipsKey="neighbour" intro={<SectionIntro title={page.neighbourHeading}>{page.neighbourParagraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>} visual={page.visuals.neighbour}>
              <InfoTable rows={page.neighbourScenarios.map((r) => ({ situation: r.situation, action: r.action, note: r.note }))} columns={[{ key: "situation", label: "Situation" }, { key: "action", label: "Helpful action" }, { key: "note", label: "Note" }]} />
              <GuideCrossLink href={COMMUNITY_BASICS_NETHERLANDS_PATH} title="Community Basics" description="Neighbours, clubs, buurt apps and integration routes." linkLabel="Open Community Basics" />
            </PremiumGuideSection>

            <PremiumGuideSection id="public" tipsKey="public" intro={<SectionIntro title={page.publicHeading}>{page.publicParagraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>} visual={page.visuals.public}>
              <InfoTable rows={page.publicScenarios.map((r) => ({ scenario: r.scenario, behaviour: r.behaviour, note: r.note }))} columns={[{ key: "scenario", label: "Scenario" }, { key: "behaviour", label: "Expected behaviour" }, { key: "note", label: "Note" }]} />
              <ChecklistBlock title="Cycling etiquette" items={page.cyclingTips} columns={2} />
              <BulletPanel title="Public transport tips" items={page.publicTransportTips} />
            </PremiumGuideSection>

            <PremiumGuideSection id="shopping" tipsKey="shopping" intro={<SectionIntro title={page.shoppingHeading}>{page.shoppingParagraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>} visual={page.visuals.shopping}>
              <SectionTable title="Shopping scenarios" description="How to navigate common Dutch retail settings with confidence." rows={page.shoppingScenarios.map((r) => ({ setting: r.setting, do: r.do, note: r.note }))} columns={[{ key: "setting", label: "Setting" }, { key: "do", label: "What to do" }, { key: "note", label: "Note" }]} />
              <ChecklistBlock title="Shopping courtesy" items={page.shoppingTips} columns={2} />
            </PremiumGuideSection>

            <PremiumGuideSection id="online" tipsKey="online" intro={<SectionIntro title={page.onlineHeading}>{page.onlineParagraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>} visual={page.visuals.online}>
              <SectionTable title="Digital channels" description="Tone and response expectations vary by platform — match the channel." rows={page.onlineScenarios.map((r) => ({ channel: r.channel, expectation: r.expectation, tip: r.tip }))} columns={[{ key: "channel", label: "Channel" }, { key: "expectation", label: "Expectation" }, { key: "tip", label: "Tip" }]} />
              <ChecklistBlock title="Digital communication" items={page.onlineTips} columns={2} />
            </PremiumGuideSection>

            <PremiumGuideSection id="family" tipsKey="family" intro={<SectionIntro title={page.familyHeading}>{page.familyParagraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>} visual={page.visuals.family}>
              <SectionTable title="Family gathering scenarios" description="Birthdays, dinners and school events each have their own rhythm." rows={page.familyScenarios.map((r) => ({ occasion: r.occasion, expectation: r.expectation, tip: r.tip }))} columns={[{ key: "occasion", label: "Occasion" }, { key: "expectation", label: "What to expect" }, { key: "tip", label: "Practical tip" }]} />
              <ChecklistBlock title="Family gathering tips" items={page.familyTips} columns={2} />
              <GuideCrossLink href={DUTCH_BIRTHDAY_TRADITIONS_PATH} title="Dutch Birthday Traditions" description="Circle parties, family congratulations, gifts and workplace birthday customs." linkLabel="Open Birthday Traditions" />
            </PremiumGuideSection>

            <PremiumGuideSection id="cultural-differences" tipsKey="culturalDifferences" intro={<SectionIntro title={page.culturalDifferencesHeading}>{page.culturalDifferencesParagraphs.map((p) => <p key={p}>{p}</p>)}</SectionIntro>} visual={page.visuals.culturalDifferences}>
              <FeatureGrid items={page.culturalDifferenceCards} />
              <SectionTable title="When something surprises you" description="Try these responses before assuming the interaction is personal." rows={page.culturalScripts.map((r) => ({ surprise: r.surprise, trySaying: r.trySaying, note: r.note }))} columns={[{ key: "surprise", label: "What may surprise you" }, { key: "trySaying", label: "Try saying" }, { key: "note", label: "Why" }]} />
            </PremiumGuideSection>

            <PremiumGuideSection id="situations" tipsKey="situations" intro={<SectionIntro title={page.situationsHeading}><p>Practical advice for everyday settings — observe locally before assuming intent.</p></SectionIntro>} visual={page.visuals.situations}>
              <SituationCardGrid items={page.situations} />
            </PremiumGuideSection>

            <PremiumGuideSection id="dos-donts" tipsKey="dosDonts" intro={<SectionIntro title={page.dosAndDontsHeading}><p>Quick reference for common social settings in the Netherlands.</p></SectionIntro>} visual={page.visuals.dosDonts}>
              <DosDontsGrid rows={page.dosAndDonts} />
            </PremiumGuideSection>

            <PremiumGuideSection id="mistakes" tipsKey="mistakes" intro={<SectionIntro title="Common Expat Mistakes"><p>Common adaptation gaps — not permanent mismatches.</p></SectionIntro>} visual={page.visuals.mistakes}>
              <MistakeCardGrid items={page.mistakeCards} />
            </PremiumGuideSection>

            <PremiumGuideSection id="checklist" tipsKey="checklist" intro={<SectionIntro title={page.checklistHeading}><p>Work through over your first months — consistency beats perfection.</p></SectionIntro>} visual={page.visuals.checklist}>
              <SectionTable title="Habits with detail" description="What each checklist item looks like in practice." rows={page.checklistWithDetail.map((r) => ({ habit: r.habit, detail: r.detail }))} columns={[{ key: "habit", label: "Habit" }, { key: "detail", label: "What it looks like in practice" }]} />
              <SectionTable title="Essential Dutch phrases" description="Start with these — they cover most everyday etiquette moments." rows={page.essentialPhrases.map((r) => ({ dutch: r.dutch, english: r.english, when: r.when }))} columns={[{ key: "dutch", label: "Dutch" }, { key: "english", label: "English" }, { key: "when", label: "When to use" }]} />
              <ChecklistBlock title="Everyday etiquette checklist" items={page.checklist} columns={2} />
            </PremiumGuideSection>

            <PremiumGuideSection id="myths" tipsKey="myths" intro={<SectionIntro title="Common Misconceptions"><p>Balanced explanations — individuals and cities vary widely.</p></SectionIntro>} visual={page.visuals.myths}>
              <MythCardGrid items={page.myths} />
            </PremiumGuideSection>

            <PremiumGuideSection id="faq" tipsKey="faq" intro={<SectionIntro title="Frequently Asked Questions"><p>Confirm specifics with neighbours and colleagues — customs vary by region.</p></SectionIntro>} visual={page.visuals.faq}>
            ...
            <PremiumGuideSection id="related-guides" tipsKey="relatedGuides" intro={<SectionIntro title="Related Guides"><p>Continue from etiquette into culture, community and workplace guides.</p></SectionIntro>} visual={page.visuals.relatedGuides}>
            ...
            <PremiumGuideSection id="culture-cluster" tipsKey="cultureCluster" intro={<SectionIntro eyebrow="Culture cluster" title="Explore More Dutch Culture" fullWidth><p>Navigate the full Dutch culture guide cluster from this etiquette hub.</p></SectionIntro>} visual={page.visuals.cultureCluster}>
            ...
            <section id="explore-next" className={cn(CITIES_FUNNEL_SECTION_SCROLL_MARGIN, "scroll-mt-24 overflow-hidden rounded-[2rem] bg-slate-950 p-6 text-white shadow-expatos-xl sm:p-8 lg:p-10")}>
              ...
              <div className="mt-6">
                <SectionIntro eyebrow="Explore next" title="Plan the Next Step" tone="onDark" fullWidth>
                  <p>Move from etiquette orientation into social norms, community integration and language learning.</p>
                </SectionIntro>
