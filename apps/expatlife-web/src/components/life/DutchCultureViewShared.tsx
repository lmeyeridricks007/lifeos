...
import { ArrowRight, BriefcaseBusiness, Building2, CheckCircle2, Clock, Gift, Globe2, Heart, Home, MessageCircle, ShieldCheck, Users } from "lucide-react";
...
export const sectionStackClass = "mt-8 space-y-6 sm:space-y-8 md:space-y-9";
const cardClass = cn(
  CITIES_FUNNEL_SOFT_COPILOT_SURFACE,
...
export function PremiumGuideSection({
  id,
  intro,
  introPanel,
  visual,
...
export function OrientationFlowBand({ className }: { className?: string }) {
  return (
    <aside
      className={cn(
...
export function MilestoneStatGrid({ items }: { items: readonly { label: string; value: string; note: string }[] }) {
  return (
...
export function BulletPanel({ title, items }: { title: string; items: readonly string[] }) {
  return (
    <aside
      className={cn(
...
export function FeatureGrid({ items }: { items: ReadonlyArray<{ title: string; body: string }> }) {
  return (
...
export function TopicsTable({ rows }: { rows: readonly { topic: string; detail: string }[] }) {
  return (
    <InfoTable
...
export function SituationCardGrid({ items }: { items: readonly MistakeCard[] }) {
  return (
    <div className={guidePremiumCardGridClass(items.length)}>
      {items.map((item, index) => {
...
export function ValueCardGrid({ items }: { items: readonly ValueCard[] }) {
  return (
    <div className={guidePremiumCardGridClass(items.length)}>
      {items.map((item, index) => {
        const Icon = iconPool[index % iconPool.length];
...
export function RegionalCardGrid({ items }: { items: readonly RegionalCard[] }) {
  return (
    <div className={guidePremiumCardGridClass(items.length)}>
      {items.map((item, index) => {
        const Icon = iconPool[index % iconPool.length];
        return (
          <article key={item.region} className={cardClass}>
            <div className={cn("absolute inset-x-0 top-0 h-1.5", movingNlSignatureGradientClass)} aria-hidden />
            <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-copilot-bg-soft to-white text-brand-strong shadow-sm ring-1 ring-copilot-primary/10">
              <Icon className="h-5 w-5" aria-hidden />
            </span>
            <h3 className="mt-4 text-lg font-bold tracking-tight text-foreground">{item.region}</h3>
            <p className="mt-2 text-sm font-semibold text-brand-strong">{item.pace}</p>
            <p className="mt-2 text-sm leading-relaxed text-foreground-muted">{item.traits}</p>
...
export function TimelineGrid({ items }: { items: readonly { era: string; title: string; detail: string }[] }) {
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {items.map((item, index) => (
        <article key={item.title} className={cardClass}>
          <div className={cn("absolute inset-x-0 top-0 h-1.5", movingNlSignatureGradientClass)} aria-hidden />
          <p className="text-xs font-bold uppercase tracking-[0.12em] text-brand-strong">{item.era}</p>
          <h3 className="mt-2 text-lg font-bold tracking-tight text-foreground">{item.title}</h3>
...
export function MythCardGrid({ items }: { items: readonly MythCard[] }) {
  return (
    <div className={guidePremiumCardGridClass(items.length)}>
      {items.map((item, index) => (
...
export function GuideCrossLink({
  href,
  title,
  description,
  linkLabel,
