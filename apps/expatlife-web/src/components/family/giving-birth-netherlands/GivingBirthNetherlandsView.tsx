...
import {
  ...
  givingBirthNetherlandsPage as page,
  type GivingBirthLink,
  type MistakeCard as MistakeCardData,
} from "./givingBirthNetherlandsPageModel";
...
const iconPool = [Baby, HeartPulse, ClipboardList, Clock, Home, Hospital, Phone, Users, MapPin, ShieldCheck] as const;
const snapshotIcons = [Home, Phone, ClipboardList, Baby] as const;
const orientationIcons = [MapPin, Phone, ClipboardList, Users] as const;
...
const placeColumns = [
  ...
  { key: "feel", label: "What it feels like" },
  { key: "whoSupports", label: "Who supports" },
  { key: "tip", label: "Tip" },
] as const;
...
function DecisionFlowStrip({ labels, eyebrow = "Birth day" }: { labels: readonly string[]; eyebrow?: string }) {
  return (
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 text-xl font-bold tracking-tight">Four habits before birth day</h3>
...
export function GivingBirthNetherlandsView() {
  return (
    <>
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: "Family", item: absoluteUrlFromPath(PARENTING_NETHERLANDS_PATH) },
          { name: "Giving birth", item: absoluteUrlFromPath(GIVING_BIRTH_NETHERLANDS_PATH) },
        ]}
      />
        ...
        <Container className={cn("w-full max-w-screen-2xl", siteGuideColumnPadYClass)}>
          ...
          <div className={sectionStackClass}>
            ...
            >
              ...
              <ChecklistBlock title="Birth file checklist" items={page.safetyFileChecklist} columns={2} />
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Snapshot" title="Giving birth at a glance" fullWidth>
                  <p>
                    Six cards cover place of birth, call cues, packing, partner role, transfer awareness and the first
                    hours — deeper sections expand each theme without medical advice.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Places" title={page.birthPlaces.heading} fullWidth>
            ...
            >
              <InfoTable
                ...
                rows={page.birthPlaces.rows.map((row) => ({
                  place: row.place,
                  feel: row.feel,
                  whoSupports: row.whoSupports,
                  tip: row.tip,
                }))}
              />
              <div className={guidePremiumCardGridClass(page.birthPlaces.cards.length)}>
                {page.birthPlaces.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.birthPlaces.crossLinks.length)}>
                {page.birthPlaces.crossLinks.map((item, idx) => (
            ...
            <PremiumGuideSection
              id="call-midwife"
              tipsKey="callMidwife"
              visual={page.visuals.callMidwife}
              intro={
                <SectionIntro eyebrow="Call cues" title={page.callMidwife.heading} fullWidth>
                  <p>{page.callMidwife.intro}</p>
                  {page.callMidwife.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <InfoTable
                columns={callColumns}
                rows={page.callMidwife.rows.map((row) => ({
                  cue: row.cue,
                  urgency: row.urgency,
                  action: row.action,
                }))}
              />
              <BulletPanel title="Call tips" items={page.callMidwife.tips} />
              <aside
                className={cn(
              ...
              >
                ...
                <p className="mt-3 text-sm leading-relaxed text-amber-950">
                  Call 112 for life-threatening emergencies. Your midwife’s call sheet still applies for labour cues —
                  when unsure, call the on-call line.
                </p>
              </aside>
            </PremiumGuideSection>
            ...
            >
              <div className="grid gap-4 lg:grid-cols-3">
                <ChecklistBlock title="Parent bag" items={page.packing.parentBag} />
                <ChecklistBlock title="Baby kit" items={page.packing.babyBag} />
                <ChecklistBlock title="Partner kit" items={page.packing.partnerBag} />
              </div>
            ...
            <PremiumGuideSection
              id="partner"
              tipsKey="partner"
              visual={page.visuals.partner}
              intro={
                <SectionIntro eyebrow="Support" title={page.partner.heading} fullWidth>
                  <p>{page.partner.intro}</p>
                  {page.partner.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.partner.cards.length)}>
                {page.partner.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <BulletPanel title="Partner tips" items={page.partner.tips} />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="labour-day"
              tipsKey="labourDay"
              visual={page.visuals.labourDay}
              intro={
                <SectionIntro eyebrow="Labour day" title={page.labourDay.heading} fullWidth>
                  <p>{page.labourDay.intro}</p>
                  {page.labourDay.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <DecisionFlowStrip labels={page.labourDay.flowLabels} eyebrow="Labour day flow" />
              <Timeline steps={page.labourDay.timeline} eyebrow="Stage" />
            </PremiumGuideSection>

            <PremiumGuideSection
              id="first-hours"
              tipsKey="firstHours"
              visual={page.visuals.firstHours}
              intro={
                <SectionIntro eyebrow="After birth" title={page.firstHours.heading} fullWidth>
                  <p>{page.firstHours.intro}</p>
                  {page.firstHours.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <div className={guidePremiumCardGridClass(page.firstHours.cards.length)}>
                {page.firstHours.cards.map((card, idx) => (
                  <FeatureCard key={card.title} title={card.title} body={card.body} iconIndex={idx} />
                ))}
              </div>
              <div className={guidePremiumCardGridClass(page.firstHours.crossLinks.length)}>
                {page.firstHours.crossLinks.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              id="plans-change"
              tipsKey="plansChange"
              visual={page.visuals.plansChange}
              intro={
                <SectionIntro eyebrow="Transfers" title={page.plansChange.heading} fullWidth>
                  <p>{page.plansChange.intro}</p>
                  {page.plansChange.paragraphs.map((paragraph) => (
                    <p key={paragraph.slice(0, 48)}>{paragraph}</p>
                  ))}
                </SectionIntro>
              }
            >
              <ScenarioTable title="When plans change" rows={page.plansChange.scenarios} />
              <BulletPanel title="Transfer tips" items={page.plansChange.tips} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Avoid" title="Common birth-day mistakes" fullWidth>
                  <p>These patterns create avoidable stress — calm call cues and packing beat last-minute improvisation.</p>
                </SectionIntro>
              }
            >
            ...
            >
              <ChecklistBlock title="Birth-day readiness checklist" items={page.checklist.items} columns={2} />
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="How-to" title={page.howTo.heading} fullWidth>
                  <p>Five orientation steps for expats preparing for birth day with a Dutch midwife or obstetric team.</p>
                </SectionIntro>
              }
            >
            ...
            <section id="faq" className={sectionClass}>
              <div className={guidePremiumIntroStackClass}>
                <SectionIntro eyebrow="FAQ" title="Frequently asked questions" fullWidth>
                  <p>Short answers for the searches expats ask most often about giving birth in the Netherlands.</p>
                </SectionIntro>
              </div>
              <div className={guidePremiumSectionDetailStackClass}>
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.intro}
              intro={
                <SectionIntro eyebrow="Related" title="Related family and health guides" fullWidth>
                  <p>
                    Maternity care is the system companion. Healthcare for Children and Parenting continue after birth.
                    Pets is the family cluster sibling.
                  </p>
                </SectionIntro>
              }
            >
            ...
            <PremiumGuideSection
              ...
              visual={page.visuals.snapshot}
              intro={
                <SectionIntro eyebrow="Hub" title="Family hub cards" fullWidth>
                  <p>Stay inside the family cluster — giving birth, parenting, children’s healthcare, maternity care and pets.</p>
                </SectionIntro>
              }
            >
              <BulletPanel title="Hub tips" items={page.familyHubTips} />
              <div className={guidePremiumCardGridClass(page.familyHubCards.length)}>
                {page.familyHubCards.map((item, idx) => (
                  <LinkCard key={`${item.href}-${item.label}`} item={item} iconIndex={idx} />
                ))}
              </div>
            </PremiumGuideSection>
            ...
            <PremiumGuideSection
              ...
              intro={
                <SectionIntro eyebrow="Explore next" title="Pick your next family step" tone="onDark" fullWidth>
                  <p>Continue to maternity pathways, children’s healthcare, parenting culture or the pets sibling guide.</p>
                </SectionIntro>
              }
            >
              ...
              <div id="sources" className="mt-8 space-y-6">
                <SectionIntro eyebrow="Trust" title="Official sources" tone="onDark" fullWidth>
                  <p>Confirm personal clinical guidance with your midwife or obstetric team — these sources are orientation only.</p>
                </SectionIntro>
